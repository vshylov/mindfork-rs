...
| C. Семантические ворота черт | `add_traits` предупреждает о почти-дубле черты (эмбеддинг новой ∩ существующие), зеркало ворот `add_insight` | 2b (отложено) |
...
### Ярус 2 — статус: A+B реализованы, оценка зонда — ручной шаг

Реализованы **A (инъекция по релевантности)** и **B (граф над наблюдениями)**; **C
(семантические ворота черт) отложена**. Журнал — [CLAUDE.md](../CLAUDE.md), карта —
[architecture.md](architecture.md) §9. Гейт зелёный (716 тестов, 20 `#[ignore]`).
...
модель; связывает ли наблюдения (`self_model_graph_e2e_live` печатает поведение).
Критерий go/no-go — переходить ли к Ярусу 3 (связывание органов памяти self↔user↔RAG)
и/или C (ворота черт).
