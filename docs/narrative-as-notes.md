...
- **C** — ворота почти-дублей черт `user_model` (зеркало ворот `add_insight`):
  `add_traits` для каждой реально добавленной черты ищет ближайшую среди прежних
  выше порога `TRAIT_SIMILARITY=0.85` и предупреждает о почти-дубле с подсказкой
  интегрировать через `remove_traits`. У черт нет хранимых векторов — эмбеддинг
  считается на лету (новые + прежние одним запросом); мягкая деградация без
  эмбеддера. Мягкие ворота (предупреждение, не запрет — решение за моделью). Черты
  остаются `Vec<String>` (без миграции).
...
**Осталось (ручной шаг пользователя):** живой мульти-сессионный прогон — всплывает
ли старое релевантное наблюдение, когда тема возвращается, и **пользуется** ли им
модель; срабатывают ли ворота черт на живой модели (объединяет ли почти-дубль).
Критерий go/no-go — переходить ли к Ярусу 3 (связывание органов памяти
self↔user↔RAG).
