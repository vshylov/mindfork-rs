...
## Ярус 3 — статус: Пути 1 (кросс-органные связи) и 2 (смешение выдачи) реализованы, GO

Исходная дальняя цель «связности» — **связывание органов памяти** (self-заметки ↔
пользовательские заметки ↔ RAG). Реализованы **Путь 1** (кросс-органные рёбра) и
**Путь 2** (смешение выдачи `[о себе]`-маркером, за тумблером). Путь 3 (RAG ↔ заметки)
**отложен**.
...
**Отложено (следующие заходы):** Путь 3 (RAG ↔ заметки), обзор self-консолидации,
vec0 при росте числа заметок.
