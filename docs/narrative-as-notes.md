...
## Ярус 3 — статус: Путь 1 (кросс-органные связи) реализован, GO

Исходная дальняя цель «связности» — **связывание органов памяти** (self-заметки ↔
пользовательские заметки ↔ RAG). Развилка (подтверждена пользователем) — **Путь 1:
кросс-органные рёбра**. Пути 2 (смешение выдачи `[о себе]`-маркером в общем
`note_recall` — реверс сокрытия Яруса 1) и 3 (RAG ↔ заметки) **отложены**.
...
- **Смоук — GO** (`cross_organ_link_e2e_live`, Gemma 4 31B + bge-m3): модель записала
  факт «о собеседнике» (`note_save`) и наблюдение «о себе» (`add_insight`), затем
  через `get_self_model` + `note_recall` (взяла id заметки) вызвала `note_link` →
...
**Отложено (следующие заходы):** Путь 2 (`[о себе]`-маркер в общем recall, за
тумблером), Путь 3 (RAG ↔ заметки), обзор self-консолидации, vec0 при росте числа
заметок.
