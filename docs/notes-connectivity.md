...
- **Консолидация** `consolidate_notes` + авто-задача «сон» (фоновое слияние дублей/
  прунинг, по образцу авто-рефлексии).
- **Перенос связей при merge** на объединённую заметку (сейчас связи замещённых не
  переносятся — соседи их просто не показывают).
- **vec0 для заметок** (если их станет очень много) — задел; пока brute-force косинус.
