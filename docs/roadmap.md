...
- **Озвучивание сообщений (TTS)** — **этап 1 сделан** (команда `/tts`, речевой
  экстрактор, плеер `rodio`, провайдеры OpenAI/Gemini/external, вкладка
  «Озвучивание» в настройках); план и решения — [research/tts.md](research/tts.md).
  Осталось: **этап 2** — локальный сайдкар (sherpa-onnx + piper-голоса ru/en,
  `mindfork tts setup` по образцу ADR 0005; спайк кириллицы на Windows в начале
  этапа) и **ADR** по итогам направления. Заделы: SSE-стриминг внутри чанка, кэш
  аудио, авто-выбор голоса по языку сообщения, маршрутизация кириллица/латиница по
  двум голосам, чтение таблиц построчно, озвучка выбранного сообщения из ленты
  (ждёт per-message выделения — пункт выше), мульти-спикерная озвучка диалога
  (Gemini), режим ОС-TTS (WinRT/speech-dispatcher).
