...
- **Авто-обновление** — self-update, musl-static и arm64-сборки, уведомление
  ...
  Releases) уже есть.
- **Озвучивание сообщений (TTS)** — реализовано, **откатано** (реверт `15dc2d0`/
  `839df19`/`6fd8529`), затем **пере-исследовано спайком** (2026-07-22/23,
  [research/tts.md §13](research/tts.md)). Проверены 6 локальных + 3 облачных
  кандидата на живом железе. **Решение: основной движок — OpenAI TTS**
  (`gpt-4o-mini-tts`): ударения — 1 ошибка на абзац (лучше всех локальных),
  англицизмы нативно (транслитерация не нужна), дрейфа нет; главное — **не новый
  vendor** (ключ OpenAI уже хранится, ADR 0008). Локальные (Qwen3-TTS/Supertonic 3
  — NO-GO по русскому; vosk-tts — GO, но 22 кГц + свой Rust-фронтенд) и
  ElevenLabs/Azure (своя регистрация) — **задел**. **Следующий шаг — этап
  `feat/tts` объёмом OpenAI + Gemini + external** (без локального сайдкара): поднять
  отревертнутую инфраструктуру (`git show 19ead9f` — экстрактор `pulldown-cmark`,
  команда `/tts`, плеер `rodio`, точки остановки, вкладка настроек, клиенты) +
  one-shot для сообщений < 4096 символов. Оговорка: OpenAI TTS требует ключа
  OpenAI — для offline/не-OpenAI аудитории локальный vosk остаётся заделом.
