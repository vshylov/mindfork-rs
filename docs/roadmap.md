...
- **Mermaid: расширение whitelist** — flowchart/sequence уже рендерятся
  ...
  обещан мейнтейнеру).
- **Широкие глифы на conhost: половина фона** (апстрим). Хвостовая ячейка широкого
  эмодзи несёт дефолтный стиль и помечена `skip`, поэтому в diff не попадает —
  locally не чинится (полной перерисовке туда писать нельзя, сама сдвинет ряд).
  Тот же корень, что открытый ratatui#2651 (`last_pos` без учёта ширины глифа):
  чинится либо наследованием стиля глифа хвостовой ячейкой, либо ведением `last_pos`
  по ширине — второй путь заодно снимает наши обходные пути (полная перерисовка на
  смену экрана / закрытие попапов, замена VS16-эмодзи в сетке). Механика и
