...
- **Авто-обновление** — self-update, musl-static и arm64-сборки, уведомление
  ...
  Releases) уже есть.
- **Озвучивание сообщений (TTS)** — реализовано и **откатано** (реверт `15dc2d0`/
  `839df19`/`6fd8529`): ни один движок не дал приемлемого качества — OpenAI читал
  монотонно и не тем голосом, piper звучал «без поставленной речи», Gemini гулял
  голосом между чанками и падал с перегрузкой. Инфраструктура (речевой экстрактор
  по `pulldown-cmark`, команда `/tts`, плеер `rodio`, точки остановки, вкладка
  настроек) провал не затронула и поднимается из `git show 19ead9f`. Разбор причин
  и новые кандидаты (Qwen3-TTS через MIT-сайдкар `qwentts.cpp` с
  OpenAI-совместимым `/v1/audio/speech`; Supertonic 3 как детерминированный
  фолбэк) — [research/tts.md §13](research/tts.md). **Следующий шаг — спайк на
  звучание, до всякого кода** (прошлый заход умер потому, что качество проверяли
  после реализации двух этапов). Не решено ни одним кандидатом: ударения и
  омографы (RUAccent — MIT, но только Python).
