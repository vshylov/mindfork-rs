...
- **Озвучивание сообщений (TTS)** — направление **исследовано, развилки приняты**
  (2026-07-21), реализация не начиналась: [research/tts.md](research/tts.md).
  Команда `/tts` (последнее) / `/tts N` / `/tts all` / `/tts stop`; пропуск кода,
  mermaid, таблиц и формул при озвучке (голосовые пометки на языке профиля);
  провайдеры — OpenAI `/v1/audio/speech`, Gemini `generateContent`+AUDIO, любой
  OpenAI-совместимый TTS-сервер (external), этапом 2 — локальный сайдкар
  sherpa-onnx + piper-голоса; воспроизведение — `rodio` (цена: ALSA на Linux,
  MPL-2.0 в `deny.toml`). Заделы направления: SSE-стриминг внутри чанка, кэш
  аудио, авто-выбор голоса по языку сообщения, чтение таблиц построчно, озвучка
  выбранного сообщения из ленты (ждёт per-message выделения — пункт выше),
  мульти-спикерная озвучка диалога (Gemini), режим ОС-TTS (WinRT/speech-dispatcher).
