...
- Post-M9: one Russian word for the sub-agent (done)
...
### Post-M9: one Russian word for the sub-agent (done)
- **One concept, three spellings in the `ru` bundle** (branch
  `fix/ru-subagent-wording`). Transliterated: `sab-agent` — 23 occurrences,
...
- **The scope is exactly one file.** A repo-wide grep for the Cyrillic stem
  finds nothing else to fix: `docs/legal/DISCLAIMER.ru.md` already used the
  solid form, and the remaining hits are `dictionaries/ru_RU.dic` (unrelated
  words that end in the same letters) and the unrelated "agentic loop" /
  "main agent" strings. No Rust source is touched: every one of these strings
  reaches the screen through `Locale::t`/`tf`, which is why a wording change
  is a one-file change — and the source-language gate is what keeps it that
  way.
- **Left alone deliberately:** the `en` bundle has the same split in its own
  terms — "sub-agent" in the prose and the tool results, "subagent" in the
  settings fields, the tool group and the tool label. Unifying that is a
  separate call, because English is the source language: it would also touch
  spec/architecture/journal prose and ADR 0010's own vocabulary. Recorded here
  so the next pass finds it rather than rediscovering it.
- **Tests:** none added — this is bundle text, and no assertion quotes it (the
  i18n tests assert on keys and on `Lang`-parametrised round trips), so
  nothing needed rewording. **2676 unit tests green, 126 `#[ignore]`**, clippy
  `-D warnings` / fmt clean, `cyrillic_scan.py` clean. No live run: interface
  text only — no engine, memory or tool behaviour involved.
