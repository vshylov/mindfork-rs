...
- **Deliberately out of scope:** English prose outside the bundle — README, the
  site copy, spec/architecture, ADR 0010, the code comments — and
  ...
  tool schema; the prose is a separate, larger pass with a different blast
  radius. Also untouched: `Step.summary` in `shared/storage/schema.rs`, which
  spells "sub-agent" but is migration-log text, explicitly not user-facing.
