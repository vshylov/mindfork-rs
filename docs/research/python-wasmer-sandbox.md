...
вердикт GO** (см. §9). Заказ: у `python_exec` должно быть два режима — **локальный
интерпретатор** (как сейчас) и **изолированная среда** на Wasmer с предустановленными
пакетами (numpy, requests и т.п.) для относительно сложных задач; **по умолчанию —
режим Wasmer**. Код изолированной среды не должен жить в основном исполняемом файле —
отдельная dll/so рядом с бинарником.
...
### 3.1 Отдельная библиотека `mindfork-sandbox` (cdylib)
...
- **Фаза 0 — спайк (go/no-go, без кода в main).** Руками, Windows + Linux:
  `wasmer run python/python` (холодный/тёплый старт — замер), numpy (matmul),
  `requests.get("https://…")` с `--net` и **без** (должен быть понятный отказ),
  `while True: pass` + прерывание. Мини-спайк embedding в ветке: cdylib +
  `WasiEnvBuilder`/runner + `Pipe`-захват вывода + terminate по таймеру. Выход:
  таблица «работает/нет» по пунктам, решение по прерыванию (§4.1) и по дефолту сети
  (§7.2).
- **Фаза 1 — каркас.** Workspace; крейт `crates/mindfork-sandbox` (ABI v1);
  `shared/sandbox.rs` (клиент за трейтом + mock); `python.rs` — режимы
  Wasmer/Local + конфиг + UI-поля + описания; graceful-ошибки «песочница не
  установлена». Юнит-тесты: диспетчер режимов, разбор ответов ABI, ошибки; живое —
  `#[ignore]`.
- **Фаза 2 — ассеты и сеть.** `mindfork sandbox setup` (lock-список, sha256);
  кэш скомпилированного модуля; `PYTHONPATH`/tmpfs; `python_net_enabled`;
  баннер первого запуска. `#[ignore]`-смоуки: numpy, requests (с сетью и без),
  кириллица, таймаут.
- **Фаза 3 — прочность и полировка.** Решение по прерыванию (metering или воркер за
  тем же ABI); лимит памяти tunables; пересмотр дефолта `python_enabled` (§7.1);
  доки (install.md, spec §9.3/§13.2, CLAUDE.md, + ADR «Python-песочница на
  Wasmer/WASIX в отдельной cdylib» — решение архитектурного уровня, достойно
  docs/decisions/0005).
...
| Место на диске | 0 | ~150–250 МБ (dll + python.webc + пакеты) |
...
принципиально нужна именно dll. **Нужно решение пользователя.**
