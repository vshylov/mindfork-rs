...
**Статус:** направление **завершено** (этапы 1 `feat/tts-core` + 2
`feat/tts-sidecar`), итог — **[ADR 0009](../decisions/0009-tts-speech-synthesis.md)**.
Развилки Р1–Р9 (§10) приняты пользователем 2026-07-21 (Р4/Р7 — подтверждены,
Р5/Р6/Р8 — с поправками, остальные — по рекомендациям); **Р2 уточнена спайком
этапа 2**: CLI-сайдкар sherpa-onnx оказался непригоден для кириллицы на Windows
(ANSI-argv, §4), взят подвариант (в) — бинарь `piper` через stdin. Поведение —
spec §11.9, журнал — CLAUDE.md. Данные о движках/API проверены веб-исследованием по первоисточникам
...
**Посадка (этап 2, реализовано)**: провизия по паттерну ADR 0005 —
...
долгоживущий протокол `--json-input` — задел.
...
- **Р2. Локальный движок сайдкара (этап 2)** — *по рекомендации*: **sherpa-onnx**
  ...
  (`--output-raw`/`--json-input`), но мёртвый апстрим. **Итог спайка (2026-07-21):
  (а) непригоден для кириллицы на Windows (ANSI-argv, §4) → решением пользователя
  взят (в)**; (б) отклонён (линковка C++ в наш бинарь не окупает выигрыша).
  Отслушивание русского — **GO** (dmitri/denis, CC0). Supertonic 3 отслушан,
  оставлен заделом; Silero/XTTS — не берём (NC-лицензии), llama-server — TTS не
  умеет (§4).
...
1. **Этап 1 `feat/tts-core`** — **сделано** (одно отступление от эскиза §8:
   ...
   оверлей помощи.
2. **Этап 2 `feat/tts-sidecar`** — **сделано**: managed-режим на бинаре `piper`
   (спайк кириллицы — **GO**, но он же закрыл подвариант (а): CLI sherpa-onnx
   уродует кириллицу через ANSI-argv на Windows) + `mindfork tts setup` (провизия
   по lock-списку sha256, паттерн ADR 0005; общая механика — `features/provision.rs`)
   + голоса ru/en с permissive-лицензиями.
3. **Итог** — **[ADR 0009](../decisions/0009-tts-speech-synthesis.md)** «Озвучивание
   сообщений (TTS)»: провайдеры, локальный движок, воспроизведение, границы.
