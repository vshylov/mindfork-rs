...
**Этап `feat/tts` реализован (2026-07-23):** OpenAI + Gemini + external восстановлены
из отревертнутого этапа 1 (`git show 19ead9f`), дефолт голоса — `onyx`; локальный
managed-сайдкар не входит (задел). 1258 юнит-тестов зелёные, 58 `#[ignore]`; живой смоук OpenAI — GO.
