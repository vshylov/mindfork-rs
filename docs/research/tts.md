...
**Статус:** исследование готово; развилки Р1–Р9 (§10) ждут решения пользователя.
Реализация не начиналась. Данные о движках/API проверены веб-исследованием по
первоисточникам (июль 2026; три параллельных обзора: локальные движки, облачные
API, воспроизведение аудио в Rust).
...
```
/tts            озвучить последнее содержательное сообщение (обычно ответ ассистента)
/tts 5          озвучить последние 5 содержательных сообщений (пользователя и модели)
/tts all        озвучить всю переписку чата
/tts stop       остановить воспроизведение
```

- **«Содержательное»** = user/assistant с непустым текстом (system/tool
  пропускаются — те же правила, что в `F5`-экспорте). Порядок — хронологический.
- При `N > 1`/`all` каждое сообщение предваряется голосовым префиксом роли
  («Пользователь.» / «Ассистент.») на языке профиля; при одиночном — без префикса.
- Новая команда `/tts` **прерывает** текущее воспроизведение и начинает новое;
  `/tts stop` просто останавливает. Переключение чата/генерация воспроизведение
  **не** прерывают (звук не конфликтует с текстом; остановить всегда одна команда).
...
Рекомендация (Р7): **четвёртая вкладка «Озвучивание»** в таб-стрипе секции
«Модель» (Ассистент · Имперсонация · Эмбеддинги · Озвучивание) — TTS это ещё один
«серверный слот» с теми же mode-driven полями (селектор режима, модель, голос,
скорость; у external — URL; ключи — общие поля-статусы ADR 0008). Альтернатива —
группа в «Интерфейсе» — хуже: поля движковые, а не оформительские.

## 10. Развилки (требуют решения пользователя)

- **Р1. Объём MVP (этап 1).** Рекомендация: команда `/tts` + экстрактор + плеер +
  режимы **OpenAi / Gemini / External** (всё — тонкие HTTP-клиенты поверх готовой
  инфраструктуры ключей). Локальный сайдкар (managed) — этапом 2: он независим и
  вдвое дороже по объёму (провизия бинаря+голосов, спавн, кириллица).
- **Р2. Локальный движок сайдкара (этап 2).** Рекомендация: **sherpa-onnx**
...
- **Р3. Воспроизведение.** Рекомендация: `rodio` в процессе (§6) — цена: системная
  C-зависимость ALSA на Linux (apt в CI, `libasound2`/`alsa-lib` в пакетах) +
  MPL-2.0 (Symphonia) в `deny.toml`. Альтернатива «сайдкар-плеер» отклонена
  (на Windows нет пригодного плеера, отмена/стыки хуже).
- **Р4. Фильтрация Markdown.** Рекомендация — таблица §5: код/mermaid/таблицы/
  display-math/URL пропускаются **с короткой голосовой пометкой**; inline-код и
  inline-math читаются текстом; «мысли» и tool-блоки не озвучиваются никогда.
  Альтернатива — пропускать молча (слушатель не узнает, что часть выпала).
- **Р5. Синтаксис команды.** Рекомендация: `/tts` + `N`/`all`/`stop` (§7).
  Альтернативные имена: `/speak`, `/say`.
- **Р6. Роли при N>1.** Рекомендация: голосовые префиксы «Пользователь./Ассистент.»
  на языке профиля; одиночное сообщение — без префикса. Альтернатива — пауза без
  префиксов (дешевле, но реплики сливаются).
- **Р7. Настройки в UI.** Рекомендация: вкладка «Озвучивание» в секции «Модель» (§9).
- **Р8. Поведение воспроизведения.** Рекомендация: новая `/tts` прерывает прежнюю;
  переключение чата/генерация — не прерывают; `Esc` не трогаем (занят генерацией/
  списком) — остановка только `/tts stop`.
- **Р9. Режим «ОС-TTS без моделей»** (WinRT «Microsoft Irina» на Windows,
  speech-dispatcher на Linux). Рекомендация: **не в объёме** — качество legacy,
  на Linux болото зависимостей; вернуться, если после этапов 1–2 останется спрос
  (реализация на Windows — прямой WinRT-синтез в WAV-буфер в нашу же очередь).

## 11. План этапов

1. **Этап 1 `feat/tts-core`** (после решения развилок): конфиг+UI-вкладка, команда
   `/tts`, речевой экстрактор, плеер rodio, клиенты OpenAi/Gemini/External, чип
   статуса, i18n (ui.tts.* + speak.*), CI/packaging-правки (ALSA, deny.toml),
