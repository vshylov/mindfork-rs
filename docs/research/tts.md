...
по рекомендациям). Реализация не начиналась; следующий шаг — этап 1 `feat/tts-core`
(§11). Данные о движках/API проверены веб-исследованием по первоисточникам
...
Дефолт `speak_roles` пользователем не задан — предлагается **выключен**: самый
частый сценарий (`/tts` на последний ответ) префикса не требует, а включение одним
тумблером сразу покрывает все три варианта, как и просил пользователь. Уточнить
на этапе 1, если ожидание иное.
...
1. **Этап 1 `feat/tts-core`** (развилки решены — можно начинать): конфиг + вкладка
