...
дополнительно нужны заголовки ALSA — воспроизведение речи (`/tts`) собирается
через `rodio`/`cpal`:
...
тянут рантайм-библиотеку сами (`libasound2` / `alsa-lib`).
...
|---|---|
| `managed` | голос (например `ru_RU-dmitri-medium`) — **локальный движок, без сети и ключей**; ставится `mindfork-rs tts setup` (ниже) |
| `openai` | модель (`gpt-4o-mini-tts`), голос (`marin`, `cedar`, …) — ключ **общий с чатом** |
...
Скорость у модели `gpt-4o-mini-tts` задаётся **словами** в поле «Указания»
(«говори по-русски, спокойно, чуть медленнее») — параметр `speed` эта модель
де-факто игнорирует; у локального движка и большинства сторонних серверов
работает обычное поле «Скорость».

### 4.3.1 Локальный движок (`managed`): `mindfork-rs tts setup`

```
mindfork-rs tts setup                              # piper + голоса ru/en (~130 МБ)
mindfork-rs tts setup --voice ru_RU-denis-medium   # добавить запасной русский голос
mindfork-rs tts setup --force                      # перекачать заново
```

Команда скачивает в `data/tts/` бинарь **piper** (MIT-релиз 2023.11.14-2) и
голоса с HuggingFace — всё по lock-списку с проверкой sha256, идемпотентно. По
умолчанию ставятся `ru_RU-dmitri-medium` (русский, данные CC0) и
`en_US-amy-medium`; запасной русский `ru_RU-denis-medium` — по `--voice`. Голоса
с несвободными лицензиями (`ru_RU-ruslan` — CC BY-NC-SA, `ru_RU-irina` —
лицензия неизвестна) в набор не входят. После установки достаточно выбрать режим
`managed` во вкладке «Озвучивание» — синтез идёт офлайн на CPU (быстрее реального
времени; на каждый кусок текста запускается процесс, поэтому куски укрупняются).

Установленный вручную piper тоже годится: укажите путь в поле «Бинарь piper»
(или переменной `MINDFORK_TTS_PIPER`), а в поле «Голос» — полный путь к `*.onnx`
(рядом должен лежать `*.onnx.json`).
...
Смоуки озвучивания (`/tts`) берут свои переменные и без них тихо пропускаются —
`MINDFORK_OPENAI_KEY`, `MINDFORK_GEMINI_KEY` (облака), `MINDFORK_TTS_URL`
(сторонний OpenAI-совместимый сервер) и `MINDFORK_TTS_DIR` (локальный движок:
каталог `data/tts/` после `tts setup`); опционально `MINDFORK_TTS_MODEL` /
`MINDFORK_TTS_VOICE`. Они проверяют **синтез** (клип непустой, формат ожидаемый);
само звучание — ручная проверка на живой машине.
