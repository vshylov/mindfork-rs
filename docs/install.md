...
- **`default_language`** — язык **служебного каркаса** (промпты, каркас «модели себя»,
  результаты инструментов), на котором создаётся первый профиль и новые профили:
  `ru` (по умолчанию) или `en`. Это **не** язык интерфейса и **не** язык ответа модели
  (его задаёт системное сообщение профиля). Инсталлятор заполнит это поле по выбору
  языка при установке. См. [docs/i18n.md](i18n.md).
- Нет файла или он пуст → **портативный** режим + язык `ru`. Для обратной
  совместимости читается и старый файл **`location.json`** (только режим хранения),
  если `defaults.json` отсутствует.
- Повреждённый JSON — ошибка запуска (чтобы опечатка в пути не привела к работе с
  пустым набором данных не там, где надо).
...
- **Правка существующего языка.** Положите `data/locales/en.json` (или `ru.json`) —
  ...
  ```
- **Новый язык.** Положите `data/locales/<code>.json`, где `<code>` — код языка из
  строчных латинских букв (`de`, `fr`, `uk`…). Язык появится в селекторах «Язык
  каркаса» (профиль, ось A) и «Язык интерфейса» (ось B). Недостающие ключи
  деградируют к русскому (референс) + запись в лог; переведите постепенно.
  Зарезервированный ключ `ui.lang.name` задаёт название языка в его же написании для
  селектора (иначе показывается код). Полный список ключей — вшитые бандлы в репозитории
  (`locales/ru.json`, `locales/en.json`), их удобно взять референсом.
- **Отказоустойчивость.** Битый/нечитаемый файл или недопустимое имя — предупреждение
  в лог (`logs/`) и **пропуск** (вшитые тексты остаются рабочими). Изменения файлов
  применяются при **перезапуске** приложения.
