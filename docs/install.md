...
### Где задаётся сервер/режим

Источник истины — **`settings.json`** (правится на экране настроек, `Ctrl+,`,
секция «Модель/сервер»). Приложение само поднимает managed-сервер или подключается
к external по сохранённому конфигу.

> **Важно:** при обычном запуске (`cargo run` / релизный бинарник) переменные
> окружения `MINDFORK_*` **не применяются** — иначе оставшийся в окружении
> `MINDFORK_ENGINE_URL` молча перебивал бы сохранённый режим при каждом старте
> (исторический баг: режим «съезжал» на external). Эти переменные читают только
> смоук-тесты (`#[ignore]`, см. §7), напрямую через `OpenAiClient::from_env`.

Эмбеддинги для RAG — **выделенный** сервер (ADR 0002): режим/URL/бинарник/модель
тоже задаются в настройках (секция «Инструменты»). Если не настроен — RAG отдаёт
ошибку, но приложение не падает.
