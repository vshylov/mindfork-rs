...
де-факто игнорирует. Локальный движок без сервера (managed-сайдкар) — этап 2
направления; сейчас локально работает режим `external` с любым из
...
`MINDFORK_OPENAI_KEY`, `MINDFORK_GEMINI_KEY` (облака) и `MINDFORK_TTS_URL`
(сторонний OpenAI-совместимый сервер); опционально `MINDFORK_TTS_MODEL` /
