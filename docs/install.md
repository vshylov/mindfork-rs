...
По умолчанию данные хранятся **рядом с бинарником** (портативно). Это можно
изменить файлом-маркером `location.json` **рядом с исполняемым файлом** (сам маркер
всегда остаётся рядом с бинарником — он про установку, а не пользовательские данные;
в резервную копию не входит):

```jsonc
{ "mode": "portable" }                 // данные рядом с бинарником (по умолчанию)
...
- **`path`** → указанный каталог (создаётся при необходимости).
- Нет файла-маркера или он пуст → **портативный** режим (обратная совместимость).
