...
- **Тема**: цвета (заголовки/ссылки/маркеры списков) берутся из `Palette`;
  ...
  сохранена через `syntect` + `ansi-to-tui` (паритет с `tui-markdown`).
- **LaTeX — delimiter-scoped** (как в .NET `LaTeXConverter`): `normalize_delimiters`
  ...
  `latex_to_unicode` (`\frac`, `\sqrt`, `\text{}`-обёртки, индексы, расширенная
  таблица символов). Разделители снимает парсер; «голые» команды вне `$…$` не
  трогаются.
...
- Горизонтальный скролл широких таблиц (вместо клипа) и подсветка-через-тему
  кода — возможные доработки; текущий клип достаточен.
