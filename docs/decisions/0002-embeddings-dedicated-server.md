...
- Embedding-сервер поднимается **лениво** (при первом обращении инструмента к
  эмбеддингам) через тот же супервайзер, что и chat-сервер
  (`ServerHandle::launch`), и держится живым до выхода приложения (kill on drop).
- Отклонение от формулировки плана «использовать и остановить после каждого
  ...
  между вызовами. «По требованию» = ленивый старт, выгрузка = при завершении.
