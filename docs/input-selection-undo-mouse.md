...
### [Р] Fork 2 — клавиши копирования/вырезания (ГЛАВНАЯ развилка)

`Ctrl+C` в экране чата — **выход** (`ChatIntent::Quit`); занят. Варианты:

- **Вариант A (рекомендуется) — `Ctrl+C` контекстный + `Ctrl+X` вырезать.**
  `Ctrl+C` копирует, **только если есть непустое выделение** (и снимает его); иначе —
  прежний выход. `Ctrl+X` вырезает выделение (без выделения — no-op). Вставка уже
  работает (bracketed paste / коалесинг, `Ctrl+V` в справке). Плюсы: мышечная память
  редакторов, выход остаётся рабочим когда ничего не выделено, выделение делается
  осознанно (Shift-навигация/драг) — «случайно скопировать вместо выхода»
  маловероятно и обратимо (`Esc`/повторный `Ctrl+C`). `Ctrl+X` свободен во всех
  экранах.
- **Вариант B — Windows-классика: `Ctrl+Ins` копировать, `Shift+Del` вырезать,
  `Shift+Ins` вставить.** Оставляет `Ctrl+C` чистым выходом. Минус: поддержка этих
  комбинаций терминалами неровная (Windows Terminal — да; часть эмуляторов не шлёт).
- **Вариант C — выделенные свободные клавиши.** Напр. `Ctrl+X` вырезать + `Ctrl+Ins`
  копировать (комбинация A и B без перегрузки `Ctrl+C`). Компромисс: выход не
  перегружен, но копирование на менее привычной клавише.

Рекомендация — **A** (контекстный `Ctrl+C` + `Ctrl+X`), с оговоркой, что в
**однострочных** потребителях (переименование/поиск/поля настроек) `Ctrl+C` и так
не выход (у них своя семантика `Esc`/`Enter`), поэтому там `Ctrl+C`=копировать
...
### 5.2 Клавиши (по выбору Fork 2; ниже — для рекомендованного варианта A)

В `screens/chat/input.rs::handle_key`, Ctrl-разбор:

- физ. `c`: если `self.input.has_selection()` → вернуть
  `ChatIntent::CopyToClipboard(self.input.selected_text().unwrap())` (и снять выделение
  `clear_selection`); иначе — прежний `ChatIntent::Quit`.
- физ. `x`: если выделение есть → скопировать (то же намерение) + `delete_selection()`
  + `mark_input_changed()`; иначе no-op.
...
паттерном (у них `Ctrl+C` не выход, поэтому копирование безопасно). На первом шаге там
работают выделение и `delete_selection` (правкой), но не clipboard-копирование.
...
### 6.4 `Ctrl+K` как частный случай

Текущий `clear_or_restore` (буфер `cleared`) — по сути одноразовый undo очистки.
После общего undo его можно **упростить**: `Ctrl+K` при непустом поле = `record_undo
(Structural)` + `clear` (содержимого, не истории — т.е. отдельный «мягкий clear»,
который толкает undo и **не** чистит стек), тогда `Ctrl+Z` вернёт текст. Поле
`cleared` и toggle-семантика `Ctrl+K` удаляются (замена: `Ctrl+K` очищает, `Ctrl+Z`
возвращает). **Развилка сохранения поведения:** если toggle `Ctrl+K` (повторное
нажатие возвращает) ценно — оставить `cleared` как есть рядом с общим undo (они не
конфликтуют). Рекомендация — **упростить** (одна модель отмены), но отметить как
поведенческую смену (§10).
...
## 7. Этап D — мышь в поле (клик → курсор, драг → выделение)
...
- **spec §11.5** (ввод/редактирование): дополнить таблицу клавиш выделением
  (`Shift`+навигация, `Ctrl+A`), копированием/вырезанием (по Fork 2), undo/redo
  (`Ctrl+Z`/`Ctrl+Y`), мышью (клик/драг при захвате). **Поведенческая смена:** если
  Fork 4/6.4 принят — `Ctrl+K` больше не toggle-restore (заменён общим `Ctrl+Z`);
  отметить.
- **spec §11.3** (мышь): клик/драг в поле ввода — новые потребители захвата `Ctrl+W`.
- **Оверлей помощи `F1`/`?`** (`HELP_KEYS`): добавить новые клавиши.
...
- **CLAUDE.md**: журнальные записи по каждому этапу (как для п.1–7).
...
стилями орфографии/команды, этап A.2) и маппинг клика с широкими глифами/скроллом
(этап D.2); оба покрываются юнит-тестами на `TestBackend`. Развилки §2 (особенно
Fork 2 — клавиши копирования) — до старта этапа B.
