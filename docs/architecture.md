...
| Модель себя    | `get_self_model`, `reflect`, `update_self_model`, `update_user_model`, `add_insight`, `consolidate_narrative` — **опц., по умолч. выкл**: пер-профильная «модель себя» в SQLite (описание + цели + модель собеседника + нарратив), пишут напрямую через `storage` (не через `ChatEffect`). **Подробно — §9** |
...
- **`user_model`** — устойчивая, интегрированная модель собеседника:
  ...
  дедупом) + `relationship_dynamic` (свободный текст). Это **снимок текущих выводов**,
  не летопись настроения.
- **`narrative`** — «биография я во времени»: короткие инсайты/наблюдения append-only
  (включая замеченные противоречия и **шрамы ревизий** — прозой). Именно нарратив, а не
  структурные поля, хранит «что и почему менялось».

**Два режима рендера** (важное различие, выведенное живым тестом):

- `render_for_prompt(cap, n, now)` — **компактный, усечённый** (по `prompt_cap`),
  только активные цели, `n` свежих инсайтов. Для **пассивной инъекции** в системный
  промпт (экономия окна контекста).
- `render_full(now)` — **без усечения**: цели с коротким `#id` и статусом (+ недавние
  закрытые), весь нарратив с `#id`. Для **чтения инструментами**
  (`get_self_model`/`reflect`/эхо правок) — модель, читающая себя, не видит «…» и
  получает `#id`, необходимые для операций над целями/наблюдениями.

Оба режима показывают **метки возраста** (`age_label(at, now)`, сутки-гранулярность:
сегодня/вчера/N дн./нед./мес./г.) у целей (закрытые — от `Goal.closed_at`) и
наблюдений — «я во времени» получает время. Два потолка-через-интеграцию защищают от
раздувания блоба, грузимого/пишемого каждый ход: FIFO нарратива (`add_insight`
возвращает вытесненное, инструмент сообщает «нарратив N/M» и что ушло) и свёртка старых
закрытых целей в нарратив-шрам «[архив цели] …» (`fold_closed_goals`,
`SelfModelSettings.max_closed_goals`). `narrative_fill_hint` (≥80% заполнения) делает
протокол ведения data-aware — подсказка «пора consolidate_narrative» в промпте и рубрике.
...
- **Чтение — пассивное, каждый ход.** `start_generation` (§5) читает снимок, кладёт его
  в `ToolContext.self_model` и **компактно** подмешивает в `system`-промпт через
  `generation::inject_self_model` — но только если профиль **включил** `get_self_model`
  (opt-in-гейт). Так агент «видит себя» на каждом ходу без явного вызова инструмента.
...
|---|---|---|
| `get_self_model` | чтение | `render_full(now)` — целиком, с `#id` и возрастом, без усечения |
| `reflect` | чтение + рубрика | текущая модель + вопросы (цели по `#id`, merge, «мимолётное — в наблюдения», консолидация) |
| `update_self_model` | описание + цели | `summary` — **интеграция**; цели — жизненный цикл по `#id` (complete/abandon), не только add |
| `update_user_model` | собеседник | списки — **merge** (add_/remove_, дедуп); `note` → **шрам** в нарратив; напоминание при удалении без `note` |
| `add_insight` | нарратив | append наблюдения/противоречия/шрама (единственный append-орган) |
| `consolidate_narrative` | нарратив | против раздувания: убрать по `#id` + опц. одно сводное (интеграция, не потеря) |
...
1. **Нейтральный к персоне «протокол ведения»** (`generation::
   SELF_MODEL_MAINTENANCE_PROTOCOL`) подмешивается в системный промпт **поверх** персоны
   профиля: когда фиксировать изменения, «мимолётное — в наблюдения», и ключевое
   **«точность важнее угодливости»** (прямой контр-приём против лести доброй персоны).
   Тумблер `config.self_model.maintenance_protocol` (по умолчанию **вкл**);
   подмешивается даже при пустой модели (bootstrap первой записи).
...
flowchart LR
    ...
    F2["Унификация органов памяти<br/>narrative ↔ notes ↔ RAG"]
...
- **Авто-консолидация модели себя по таймеру** — как `notes.auto_consolidate_every`:
  ...
  консолидация ручная / через авто-рефлексию.
- **Унификация органов памяти** — нарратив по сути **второй, слабый экземпляр заметок**
  (append-only, FIFO-потолок, без графа). Стратегически верный дальний ход — связать/
  слить `narrative ↔ notes ↔ RAG`, а не растить на нарративе параллельный граф. Крупное
  отдельное направление (см. notes-connectivity Ярус 3).
