...
- `render_for_prompt(cap, n)` — **компактный, усечённый** (по `prompt_cap`), только
  активные цели без id, `n` свежих инсайтов. Для **пассивной инъекции** в системный
  промпт (экономия окна контекста).
- `render_full()` — **без усечения**: цели с коротким `#id` и статусом (+ недавние
...
|---|---|---|
| `get_self_model` | чтение | `render_full()` — целиком, с `#id`, без усечения |
