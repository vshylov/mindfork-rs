...
| `update_user_model` | собеседник | списки — **merge** (add_/remove_, дедуп); `note` → **шрам** @self-заметкой; напоминание при удалении черт **или замене непустой динамики** без `note`; **ворота почти-дублей черт** (Ярус 2/C): близкая к добавляемой уже есть → подсказка `remove_traits`, эмбеддинг черт на лету, зеркало ворот `add_insight` |
...
self↔self); авто-рефлексии эти инструменты даны (`REFLECT_TOOL_IDS`).
...
- **Унификация органов памяти** — нарратив был **вторым, слабым экземпляром заметок**
  ...
  GO), **семантические ворота почти-дублей черт** `user_model` (шаг C). Далее (Ярус 3) —
  связывание органов памяти self↔user↔RAG и полное смешение выдачи (`[о себе]`-маркер в
  общем `note_recall`).
