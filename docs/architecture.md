...
|---|---|---|
| `get_self_model` | чтение | `render_full(now, recent)` — целиком, цели с `#id`, наблюдения (self-заметки) с полным id, без усечения |
| `reflect` | чтение + рубрика | текущая модель + вопросы (цели по `#id`, merge, «мимолётное — в наблюдения», консолидация наблюдений через note_revise/supersede) |
| `update_self_model` | описание + цели | `summary` — **интеграция**; цели — жизненный цикл по `#id` (complete/abandon), не только add |
| `update_user_model` | собеседник | списки — **merge** (add_/remove_, дедуп); `note` → **шрам** @self-заметкой; напоминание при удалении черт **или замене непустой динамики** без `note` |
...
[narrative-as-notes.md](narrative-as-notes.md)).
...
- **Унификация органов памяти** — нарратив был **вторым, слабым экземпляром заметок**
  ...
  ворота дублей, замещение со «шрамом», «сон». Далее — связать/слить с `RAG` и полное
  смешение выдачи (`[о себе]`-маркер в общем `note_recall`) — Ярус 2/3.
