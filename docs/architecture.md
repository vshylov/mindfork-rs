...
- **Code workspace (`code.rs`)** — the root comes from the turn snapshot,
  ...
  Both path families share `reach.rs`: after containment, a resolved path inside
  `Paths::app_dirs` (the data root, the binary's directory) is refused, the walker
  filters those directories out whether the project ignores them or not, and the
  missing part of a path is checked for a dangling symbolic link before it is
  judged by its name (docs/research/safe-defaults.md D2, N1); `code_edit` and
  `code_write` also refuse anything under `.git/` (N2). Walking uses
...
- **Release**: `[profile.release]` with LTO/strip/`opt-level=3`,
  `panic=unwind`. On Windows `.cargo/config.toml` adds
  `-C target-feature=+crt-static` for `x86_64-pc-windows-msvc`: without it the
  binary imports `VCRUNTIME140.dll` from the Visual C++ redistributable, which a
  clean Windows does not carry and no artifact of ours ships — the app then fails
  to start before printing anything. It is a config file rather than a workflow
  variable so every build links the way the release does; cargo does **not**
  merge it with a `RUSTFLAGS` set in the environment, which replaces it entirely
  (docs/research/release-pipeline.md R1).
- **What a release ships beyond the binary** (`.github/workflows/release.yml`):
  README/CHANGELOG/LICENSE/DISCLAIMER/PRIVACY and their `ru` translations,
  `docs/install.md`, the spellcheck dictionaries with their licences under
  `data/dictionaries/`, the vendored grammars' licences under
  `licenses/syntaxes/`, and `THIRD-PARTY-NOTICES.md` — the licence texts of the
  whole locked graph, generated by `cargo about` (`about.toml`, `about.hbs`) from
  the release's own `Cargo.lock` and therefore **not** committed. Beside the
  archives sits **`install.sh`** (`packaging/linux/install.sh`), the Linux install
  script `releases/latest/download/install.sh` resolves to: POSIX `sh`, one
  function called on its last line (a truncated download runs nothing), the
  latest tag read off the releases page's *redirect* rather than the API, the
  archive refused unless `sha256sums.txt` lists it and agrees, unpacked beside
