...
- **Унификация органов памяти** — нарратив был **вторым, слабым экземпляром заметок**
  ...
  3, Путь 1 сделан** (кросс-органные связи, GO): рёбра self-заметка ↔ пользовательская
  заметка теперь **показываются** в выдаче с пометками `[о себе]`/`[заметка]`, а
  `note_recall` выводит id (адресуемость); механика графа была кросс-органна и раньше,
  Ярус 3 снял фильтры в выдаче. Органы остаются РАЗДЕЛЬНЫМИ по хранению/поиску — всплывает
  лишь намеренно созданное ребро. Далее — Путь 2 (`[о себе]`-маркер в общем `note_recall`,
  за тумблером) и Путь 3 (RAG ↔ заметки).
