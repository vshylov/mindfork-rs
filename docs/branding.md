...
Сейчас не покрыто **ничего**: у `.exe` нет иконки вовсе (в Проводнике/таскбаре —
дефолтная), Linux-пакеты не кладут ни `.desktop`, ни иконок темы, README —
текстовый заголовок, `artwork/` даже не в git (`?? artwork/`).
...
Развилка **Р4** — нужен ли `.desktop` консольному приложению вообще.
...
**Куда поставить.** Оверлей помощи (`F1`) уже несёт версию в титуле, что делает
его естественным «About». Но он же и тесный: высота считается по числу строк
`HELP_KEYS` и на коротком терминале список **уже** прокручивается. Поэтому
логотип рисуется **только когда есть вертикальный запас** (иначе пропускается) —
ровно та же деградация, что у скроллбара (нет переполнения → не рисуем) и у
Mermaid (не влезает → фолбэк). Точное размещение — развилка **Р2**.
...
## 7. Развилки (нужно решение до реализации)
...
| **Р2** | Логотип в TUI | (а) в шапке `F1` при наличии места; (б) отдельный экран «О программе»; (в) в пустом чате; (г) не делать | **(а)** — переиспользует уже существующий «About» (там версия), новых клавиш не вводит |
| **Р3** | Бренд-оранжевый `#c25a27` как акцент темы | (а) не трогать тему; (б) заменить `accent`; (в) новая тема «brand» | **(а)** — `accent` сейчас несёт смысл «активность» (генерация, счётчик токенов); перекраска ломает семантику ради косметики. Логотип и так рисуется фирменными цветами |
