...
знак рисуется **только когда в терминале хватает высоты и на него, и на весь
список**, а ширины попапа хватает на лockup; иначе не рисуется вовсе — клавиши не
сдвигаются и лишней прокрутки не появляется. Ширину под лockup специально **не
растягиваем**: попап меряется списком клавиш, знак либо вписывается, либо
пропускается. Та же деградация, что у скроллбара (нет переполнения → не рисуем) и
...
### 5.1 Вордмарк и горизонтальный лockup

Рядом с глифом рисуется слово `mindfork` — не одним из SVG-вордмарков (там текст
переведён в кривые, §2, растеризовать их в терминале нечем), а **собственным
пиксельным шрифтом** в `widgets/logo.rs`: матрица `#`/`.` на букву, те же
половинные блоки. Начертание повторяет бренд (JetBrains Mono ExtraBold): строчные,
выносные у `d`/`f`/`k` и **штрихи в 2 пикселя** — ровно той толщины, что бары
глифа (в его сетке они тоже по 2 единицы).

Пропорции взяты из горизонтального лockup'а (метрики — `artwork/README.md`), а не
подобраны на глаз:

| Величина | Бренд | В терминале |
|---|---|---|
| Высота слова | `0.5227 × S`, где `S` — **размер иконки** (16) | `≈ 8` пикселей = **4 строки** против 6 строк глифа |
| Базовая линия | `0.7418 × S` от верха иконки | низ слова — **на строку выше** низа глифа |
| Зазор «иконка → текст» | `0.3608 × S` от края **бокса** | `0.3608 × 16 − 3` (пустое поле справа от чернил) ≈ **3 колонки** |

Ключевая тонкость первой строки: доли берутся от **размера иконки** `S = 16`, а не
от высоты её чернил (12). Первая версия шрифта считала «половину чернил» → 6
пикселей, и в такую высоту 2-пиксельные штрихи не влезали (x-высота 4 не оставляет
просвета внутри `o`/`d`), из-за чего слово вышло светлым — при чанковом-то глифе.
Правильные 8 пикселей дают x-высоту 6: два пикселя штриха, два просвета, два
штриха.

Отдельный компромисс — **`f`**. Между её крючком (на выносной высоте) и
перекладиной (на x-высоте) в пиксельном бюджете не остаётся ни одного ряда, поэтому
2-пиксельный крючок слипался с перекладиной в сплошной блок. Крючок сделан в
**1 пиксель**: он попадает в верхнюю половину ячейки (`▀`), нижняя остаётся пустой —
просвет виден, а перекладина стоит на x-высоте, в одну строку с верхними барами
`n`/`o`/`r`. Альтернатива (опустить перекладину к середине x-высоты ради
2-пиксельного крючка) ломала бы эту общую линию.

Итог — **65 колонок × 6 строк**. Цвета: `fork` — фирменный акцент, как везде; а вот
`mind` берётся из палитры (`text`). Это **не нарушение Р3**, а его прямое следствие:
в бренде `mind` — «текст на тёмном»/«текст на светлом» (для того и заведены
раздельные `-dark`/`-light`), т.е. цвет, производный от фона. Один лockup поэтому
работает в обеих темах, а знаковые цвета остаются фирменными.

**Выравнивание — влево, по полю списка клавиш** (те же два пробела), с отбивками
сверху и снизу. Центрированный знак читался бы отдельной картинкой; прижатый влево
он работает как шапка блока и задаёт ту же вертикаль, что и клавиши ниже.

Гейтов от расхождения два: тест сверяет `GLYPH` с SVG (см. выше), а высота слова и
его отступ проверяются `const`-ассертом — правка шрифта или `WORDMARK_TOP_ROW`,
из-за которой слово перестало бы влезать в высоту глифа, **роняет сборку**, а не
молча обрезает картинку.
