...
жирные штрихи, выносные у `d`/`f`/`k`.
...
|---|---|---|
| Высота слова | `0.5227 × S` (≈ половина глифа) | 6 пикселей = **3 строки** против 6 строк глифа |
| Базовая линия | `0.7418 × S` от верха иконки | низ 3-й строки слова — **на строку выше** низа глифа |
| Зазор «иконка → текст» | `0.3608 × S` от края **бокса** | `0.3608 × 16 − 3` (пустое поле справа от чернил) ≈ **3 колонки** |

Итог — **49 колонок × 6 строк**. Цвета: `fork` — фирменный акцент, как везде; а вот
