...
> **Статус: РЕАЛИЗОВАНО** (ветка `feat/i18n-ui`, 2026-07-13). Основные UI-поверхности
> локализованы (`ui.*`-ключи в `locales/{ru,en}.json`), настройка
> `config.interface.language` + поле «Язык интерфейса» в настройках, проводка
> `&'static Locale` зеркалит `Palette`. Итог и ключевые решения — в журнале CLAUDE.md
> («Пост-M9: мультиязычность интерфейса — ось B»). **Задел** (§7 ниже дополнен):
> метки инструментов `ToolInfo.ui_label`/`ToolGroup::title()` в тумблерах профиля и
> reasons `ServerStatus::Disconnected` из супервайзера остались русскими (требуют
> правки слоёв `features/tools`/`supervisor` — вынесено в отдельный шаг).
>
> Ниже — исходный дизайн-план (развилки подтверждены 2026-07-13).
> Направление продолжает [docs/i18n.md](i18n.md): ось A (язык **агента** — промпты и
...
> (docs/i18n.md §3.2: русский UI + англоязычные агенты — законная комбинация).
...
UI **независим** (docs/i18n.md §3.2, решение 2026-07-12): один глобальный язык
интерфейса, никакого наследования между осями.

**Пересечение осей — одно место** (docs/i18n.md §1): результаты инструментов и
...
2. **Множественное число в английском** («1 line» vs «2 lines»). Варианты: (a)
   ...
   **Решение: (a)** — формат «tokens: N», «found: N chats», «N str.» без
   ...
   необходимости пересмотреть fluent, docs/i18n.md §7).
...
3. **Язык UI по умолчанию** (первый запуск / старый `settings.json` без поля).
   ...
   **Решение: (a)** — установочный `default_language` (тот же, что у первого профиля,
   docs/i18n.md §3.2) задаёт и язык UI при свежей установке. Инсталлятор ставит оба
...
**~365 живых UI-литералов** (§2.2 в docs/i18n.md давал ~600–800, но считал тесты).
...
5. **Тесты** (по ходу, `TestBackend`): per-locale структурные (рендер под каждым
   ...
   `ui.*`; смысловые пины ключевых фраз (docs/i18n.md §3.5); проверка выравнивания
...
2. **Churn тестов**: сотни UI-ассертов на русские подстроки → ключи бандла + цикл по
   локалям (docs/i18n.md §3.5). Механический, но объёмный; ru-бандл извлекается
...
**Задел (не вошло в реализацию, отдельный шаг):**
- **Метки инструментов в тумблерах профиля** — `ToolInfo.ui_label` (инлайн-hint) и
  `ToolGroup::title()` (заголовки групп) живут в слое `features/tools` (трейт/meta),
  возвращают `&'static str`. Локализация: либо `ui_label(&self, loc)` в 35 impl +
  `ToolGroup` с i18n-ключом, либо резолв в слое настроек по `info.id`/`group`
  (`Locale::get(&'static self, key)` уже позволяет динамический ключ → `&'static str`).
  ~43 короткие строки. Сам тумблер показывает английский `id` инструмента — основная
  метка уже английская.
- **Reasons `ServerStatus::Disconnected(...)`** из `supervisor.rs` (редкие ошибки
  конфигурации сервера, всплывают в чипе «чат: нет связи: {why}») — супервайзер не
  имеет доступа к `config.interface.language`; нужна отдельная проводка локали.
