...
- **Дизайн-планы направлений** (банк идей + реализованный MVP-зонд; направления
  ...
  **связность заметок** ([notes-connectivity.md](docs/history/notes-connectivity.md)) —
  ...
  «модели себя» переезжают в заметки с тегом `@self`.
- **Активный дизайн-план**: [docs/summary-as-snapshot.md](docs/summary-as-snapshot.md) —
  анти-раздувание summary «модели себя» («снимок, не летопись»): жанровая граница в
  POLICY_CORE, мягкие ворота размера, посекционные бюджеты инъекции, диета эха правок.
  **Все 4 этапа сделаны** (живой прогон `summary_gate_e2e_live` — следующий ручной
  шаг). Этап 1 — жанровая граница в текстах (`POLICY_CORE`
  маршрутизирует событийные выводы в `add_insight` даже устойчивые, summary —
  компактный снимок с шагом «прочти целиком перед правкой»; правки описаний
  `update_self_model`/`add_insight` и рубрики `reflect`). Этап 2 — мягкие ворота размера
  summary: конфиг `self_model.summary_target_chars` (дефолт 1000, санитизация пол 200),
  чистый `SelfModel::summary_fill_hint`, три точки показа (чтение `render_self_read`;
  data-aware приписка к протоколу в `inject_self_model`; строка размера в эхе
  `update_self_model` при правке summary), поле «Модель себя: ориентир описания» в
  настройках, живой смоук `summary_gate_e2e_live`. Этап 3 — посекционные бюджеты
  `render_for_prompt`: описание (summary) усекается до **половины** лимита инъекции
  (`truncate_chars_word` — по границе слова), так что раздутый summary не вытесняет из
  промпта цели/собеседника/наблюдения; финальное усечение всего блока — страховка;
  `render_full` (полное чтение) не усекается. Этап 4 — диета эха правок:
  `update_self_model`/`update_user_model` возвращают **дельты** (добавленные цели с
  `#id`, закрытые/неактуальные по `#id`, число свёрнутых в наблюдения; итоговые списки
  собеседника + подтверждение шрама) вместо полного `render_full` — экономит токены и
  снимает «заякоривание» на жанре эссе; полное чтение остаётся за `get_self_model`
  (`entities::self_model::short_id` — публичная ручка `#id` для эха).
