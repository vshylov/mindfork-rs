...
- **Тесты**: менеджер (первый подъём авто-пиннит + возвращает пин для персиста;
  ...
  clippy `-D warnings`/fmt чисты.
- **Живой прогон**: e2e `mcp_filesystem_e2e_live` перегоняется после 3b
  (TOFU-авто-пин теперь на happy path) — финальный ручной шаг перед PR; негативный
  путь TOFU (изменившийся каталог) покрыт юнит/интеграционными тестами (живая
  симуляция подмены сервера не нужна — механика чисто клиентская).
