...
Сделан весь план **M0–M9** плюс обширный пост-M9 (в `main`). **1177 юнит-тестов
...
- **Живой прогон движка не требуется** (рендеринг, без движка/памяти). **Ручной регресс
  на conhost — за пользователем**: юнит-тесты дефекты терминала не ловят, а снятие
  перерисовки со сдвига выделения — единственное место, где поведение на реальном
  conhost могло измениться.
