...
Сделан весь план **M0–M9** плюс обширный пост-M9 (в `main`). **1174 юнит-теста
...
- **Гейт переехал к механизму**: `prime_full_redraw_repaints_all_but_wide_glyph_tails`
  ...
  переведены на реальный хелпер вместо своей копии сентинела. **1174 юнит-теста
  зелёные** (+7 к `main`), 53 `#[ignore]`.
