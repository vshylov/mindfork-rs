...
- **Тесты**: db (`note_update` только свой профиль; `note_search_semantic` ранжирует и
  ...
  месте; плохой/несуществующий id); mod (`note_revise` в дефолтах). Семантические
  тесты — на `MockEmbedder` (мешок символов + L2). **639 тестов зелёные**, clippy/fmt
  чисты. Живой прогон/оценка зонда на локальной модели — ручной шаг (критерий go/no-go
  в плане).
