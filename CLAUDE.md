...
Сделан весь план **M0–M9** (в `main`). **274 теста зелёные, 6 `#[ignore]`.**
...
- **LaTeX delimiter-scoped** (по образцу .NET `LaTeXConverter`): `normalize_delimiters`
  ...
  `√(x)`, текстовые обёртки (`\text/\mathrm/\mathbb/…`), spacing-команды, защита
  `\{ \}`, снятие группирующих скобок, десятки символов.
