...
- **Активный дизайн-план**: [docs/summary-as-snapshot.md](docs/summary-as-snapshot.md) —
  ...
  **Этап 1 сделан** (жанровая граница в текстах: `POLICY_CORE` маршрутизирует событийные
  выводы в `add_insight` даже устойчивые, summary — компактный снимок с шагом «прочти
  целиком перед правкой»; правки описаний `update_self_model`/`add_insight` и рубрики
  `reflect`). Этапы 2–4 (ворота размера, посекционные бюджеты, диета эха) — впереди.
