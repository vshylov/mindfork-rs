...
Сделан весь план **M0–M9** плюс обширный пост-M9 (в `main`). **1171 юнит-тест
зелёный, 53 `#[ignore]`-смоука** (крупнейший счётчик — журнал ниже; текущее
...
- **Смежный случай (не чинили, вне объёма запроса)**: тот же механизм возможен в
  попапе подсказок орфографии (`Ctrl+G`) — пункт «➕ добавить в словарь» несёт
  широкий глиф, а выделенная строка списка рисуется `reversed` (т.е. с фоном).
  Воспроизведения не было; лечится той же строкой `request_full_redraw()` в
  `handle_suggest_key`.
...
- **Два гейта на инвариант** (оба **мутационно проверены** возвратом `❤️` в список):
  ...
  diff'а не целится во вторую половину широкого глифа. **1171 юнит-тест зелёный**
  (+4 к `main`), 53 `#[ignore]`.
