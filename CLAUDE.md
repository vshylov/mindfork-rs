...
- **[docs/decisions/](docs/decisions/)** — ADR: UI-крейты под ratatui 0.30 (0001),
  ...
  версионирование схем данных и миграции (0006), плагины: MCP-хост инструментов +
  нейтральный формат обмена импорта (0007), API-ключи в настройках с
  машинно-привязанным шифрованием (0008), озвучивание сообщений (TTS): облачный
  провайдер + свой речевой экстрактор + плеер `rodio` в процессе (0009).
...
## Статус (на 2026-07-23, версия 0.9.2)
Сделан весь план **M0–M9** плюс обширный пост-M9 (в `main`). **1264 юнит-тестов
зелёные, 58 `#[ignore]`-смоуков** (крупнейший счётчик — журнал ниже; текущее
направление — **озвучивание сообщений чата (TTS)** (команда `/tts`, движки OpenAI/
Gemini/external, речевой экстрактор Markdown, плеер `rodio`; исследование
[docs/research/tts.md](docs/research/tts.md), решение 2026-07-23 — основной движок
OpenAI `gpt-4o-mini-tts`/`onyx`) — **завершено** (без локального сайдкара — задел);
прежнее — **API-ключи в настройках** (ввод в окне настроек + машинно-привязанное
шифрование в конфиге; исследование
[docs/research/api-key-storage.md](docs/research/api-key-storage.md), развилки Р1–Р6
приняты 2026-07-20, итог — **ADR 0008**) — **завершено** (этапы 1 «ядро» + 2 «UI»);
прежнее — **айдентика** (логотип/вордмарк: ассеты, иконка `.exe`, запись в меню
Linux, логотип в справке; план [docs/branding.md](docs/branding.md)) — **завершено**;
ещё прежнее — **система плагинов** — **завершено** (исследование
`docs/research/plugin-system.md`, развилки Р1–Р8 приняты 2026-07-17, итог —
**ADR 0007**): **этап 1 «generic-импорт»** (`feat/generic-import` — нейтральный
формат `mindfork-import` + `docs/import-format.md` + CLI `import`,
`import-lamellama` удалён — знание о непубличной LameLLaMA уезжает в приватный
внешний конвертер); **этап 2 «зонд MCP-клиента» — GO** (`spike/mcp-client`:
мини-клиент `shared/mcp.rs` + живой смоук — Gemma 4 31B вызвала инструмент
реального `server-filesystem` первым раундом, 14 инструментов ≈ 7 КиБ схем,
протокол 2025-11-25); **этап 3 «MCP-хост»** (`feat/mcp-host`, части 3a+3b:
клиент в бинаре + `McpManager` + обёртка `McpTool` + `config.mcp` + гейты/отмена
+ TOFU-пиннинг каталога + статусы/полные описания в настройках + i18n + spec §9.6;
живой e2e через оркестратор — GO); прежнее направление —
**рендер Mermaid-диаграмм в ленте** (`feat/mermaid-render`, план
`docs/research/mermaid-ascii-rendering.md`) — **завершено**: зонд (NO-GO на
`mermaid-text` 0.56.0 — паника/порча подписей на кириллице) → **наш апстрим-фикс**
многобайтовости (leboiko/markdown-reader#29 + PR #30 → релиз 0.56.1, «the shipped
...
(`feat/linux-packages`), этап 3 «Windows-инсталлятор» (`feat/windows-installer`);
ещё прежние — **релизная инженерия** (версии/CHANGELOG/CI/миграции схем данных) —
завершено; **i18n CLI** (весь текст CLI/`main.rs`/фич/хвоста движка в бандлах, свой
...
на лету по настройкам**, **рендер Mermaid-диаграмм в ленте** (flowchart/sequence
текстовой графикой, жёсткий фолбэк на исходник), **«модель себя»/собеседника** и
**связность заметок** (нарратив как заметки). Полный журнал пост-M9 — ниже.
...
### Пост-M9: релизная инженерия — этап 6 (cargo-deny) + словари в релизе (сделано)
- **Опциональный этап 6** направления «релизная инженерия» (Ф6) + правка релизного
  пайплайна; ветка `feat/cargo-deny-release-dicts`.
- **Аудит зависимостей `cargo-deny`**: `deny.toml` (advisories/licenses/bans/sources)
  + `.github/workflows/audit.yml` (**еженедельно `cron` + `workflow_dispatch`, НЕ на
  `pull_request`** → merge не блокирует, advisory-режим). `EmbarkStudios/cargo-deny-action@v2`.
  Прогнан локально (`cargo-deny 0.20.2`): licenses/bans/sources — ok (allow-список
  лицензий выверен по графу); из advisories **`anyhow` (RUSTSEC-2026-0190, unsound
  `Error::downcast_mut`) — прямая зависимость, починен обновлением `1.0.102 → 1.0.103`**
  (floor в Cargo.toml поднят); 4 неустранимых транзитивных через `syntect`
  (yaml-rust/bincode unmaintained, quick-xml ×2 DoS — RUSTSEC-2024-0320/2025-0141/
  2026-0194/2026-0195) занесены в `ignore` с обоснованием (syntect парсит **свои
  вшитые** синтаксисы/темы, не ввод пользователя → риск не реализуется; снять при
  обновлении syntect). Принцип: чиним что контролируем, документированно игнорируем
  неустранимое транзитивное. `cargo deny check` зелёный.
- **Словари спелл-чека в релизе**: `release.yml` кладёт `dictionaries/*.aff`+`*.dic`
  (`en_US`/`en_GB`/`ru_RU`, ~5 МБ) в архив как **`data/dictionaries/`** — портативную
  раскладку, которую приложение читает рядом с бинарником → **спелл-чек из коробки** и
  в релизе (а не только в dev через `build.rs`). Действует со **следующего** тега
  (опубликованный `v0.9.0` словарей не содержит — при желании перевыпустить).
- **Уточнены устаревшие комментарии**: словари **давно в репозитории** (6 файлов в git,
  записи в `.gitignore` нет) — правки в `build.rs`, CLAUDE.md (пункт ниже),
  AGENTS.md §6 (состав архива). Прежние «не входят в репозиторий (.gitignore)» —
  устаревшая справка.
- Чистая инфраструктура/доки + patch-bump зависимости — **живой прогон движка не
  требуется**. **1071 юнит-тестов зелёные** (anyhow 1.0.103 API-совместим), 48
  `#[ignore]`, clippy `-D warnings`/fmt/`cargo deny check` чисты. YAML `audit.yml`
...
### Пост-M9: рендер Mermaid-диаграмм в ленте (сделано)
- **` ```mermaid `-блоки ленты рендерятся текстовой графикой** (крейт
  `mermaid-text` ≥ 0.56.1; net-new зависимостей две — `mermaid-text` + `ascii-dag`)
  вместо печати исходника. Путь к фиче — трёхходовка: **зонд** (2026-07-14, вердикт
  NO-GO на 0.56.0 — паника на кириллических sequence + молчаливая порча подписей
  flowchart, байтовые offset'ы как символьные;
  [docs/research/mermaid-ascii-rendering.md](docs/research/mermaid-ascii-rendering.md)) →
  **апстрим-фикс наш** (баг-репорт
  [leboiko/markdown-reader#29](https://github.com/leboiko/markdown-reader/issues/29) +
  готовый [PR #30](https://github.com/leboiko/markdown-reader/pull/30) с тремя фиксами и
  регресс-тестами; мейнтейнер по нашей «broader note» провёл полный аудит, закрыл ещё
  3 места того же класса и выпустил 0.56.1) → **перепроверка зондом** (0 паник на
  корпусе 31 кейса, порча ушла) → реализация по провалидированному плану §4.
- **Ключевое правило — жёсткий фолбэк вместо клипа** (у таблиц клип с «…» допустим,
  обрубленная диаграмма нечитаема): рендерится только whitelist
  (**flowchart/sequence** по `detect`; pie/gantt/class/state в тексте убоги) и только
  целиком влезающее в ширину панели (**наш пост-чек**: `max_width` крейта — мягкая
  подсказка, sequence её игнорирует; инвариант ленты «строка ≤ ширины» жёсткий, иначе
  повторный перенос порвал бы рамки). Любой сбой (мусор/обрубок стрима/тип вне
  whitelist/переполнение) → исходник код-блоком **байт-в-байт как при выключенном
  тумблере** (golden-тест `mermaid_fallback_matches_disabled_render` сравнивает
  `Line`-ы целиком, со стилями). Худший случай = прежнее поведение.
- **Реализация** (по образцу `TableBuilder`): `Writer.mermaid: Option<(info, src)>`
  — `start_codeblock` при включённом флаге и `lang=="mermaid"` буферизует блок (забор
  не печатается), `text()` копит исходник как есть (устойчиво к дроблению на события),
  `end_codeblock` решает: `mermaid::render_mermaid_block` (новый подмодуль
  `shared/markdown/mermaid.rs`) или `emit_fenced_source` (фолбэк, повторяющий прежний
  неподсвеченный вид: реверс + DIM-заборы с полной инфо-строкой ` ```mermaid title=x `).
  Компат-палитра (`palette.compat`) → `render_ascii_with_width` (ASCII-глифы, conhost).
  Стрим: недописанный блок не парсится → исходник; по дописывании кэш ленты
  пересчитывает сообщение и подменяет диаграммой. **Панику апстрима сознательно не
  ловим** (`catch_unwind`): panic-hook приложения восстанавливает терминал на любую
  панику, в т.ч. пойманную — «поймать и продолжить» оставило бы TUI сломанным;
  полагаемся на аудит 0.56.1 + узкий whitelist (зафиксировано в доке модуля).
- **Проводка**: `RenderOpts.render_mermaid` (Default=false — тесты/tool-карточки без
  изменений поведения); лента протаскивает **весь `RenderOpts`** вместо голого
  `table_row_separators: bool` через `build_message_block`/`push_body`/
  `push_assistant_body`/`push_markdown_fragment`/`push_tool`/`push_block` (будущий
  флаг больше не тронет сигнатуры; `soft_break_as_newline` докидывает `push_body` для
  пользователя как раньше). `MessageFeed.render_mermaid` + `set_render_mermaid` +
  поле в `CacheKey` (смена тумблера сбрасывает кэш); `ChatScreen::set_settings`
  прокидывает из снимка. Конфиг `interface.render_mermaid` (`#[serde(default)]`
  контейнера → старые `settings.json` без миграции), **по умолчанию включён** —
  фолбэк делает включение безопасным. Тумблер «Диаграммы Mermaid» в группе
  «Оформление» секции «Интерфейс» (`FieldId::IMermaid`, описание в обоих бандлах
  локалей — i18n-гейты покрыли автоматически).
- **Тесты**: модуль mermaid (sequence/кириллический flowchart без порчи; фолбэки —
  узкая ширина/вне whitelist/мусор/обрубок; ASCII в компат-палитре; бюджет ширины на
  60/90/120); writer (диаграмма вместо исходника + golden-фолбэк 4 сценария +
  выключенный флаг + кириллица); лента (тумблер + инвалидация кэша); настройки
  (тумблер сохраняет конфиг + описание); конфиг (default-on). **1090 юнит-тестов
  зелёные** (+13), 48 `#[ignore]`, clippy `-D warnings`/fmt чисты; лицензии новых
  крейтов (MIT; MIT OR Apache-2.0) в allowlist `deny.toml`. **Живой прогон движка не
  требуется** (чистый рендер-модуль без движка/памяти — прецедент
...
- **Первый этап направления «система плагинов»** (исследование
  [docs/research/plugin-system.md](docs/research/plugin-system.md), развилки Р1–Р8
  приняты пользователем 2026-07-17 по рекомендациям; ветка `feat/generic-import`,
  стек на `docs/plugins-research` — общие правки roadmap, прецедент линейного стека
  инсталляторов). Импорт из сторонних приложений переведён с зашитого в монолит
  импортёра LameLLaMA на **нейтральный документированный формат обмена**: внешний
  (возможно, приватный) конвертер эмитит один JSON-файл — приложение импортирует
  его командой `mindfork import <file>`. Паттерн «конвертер → документированный
  файл → импорт» — прецеденты beancount/beangulp, KeePass, Netscape bookmarks
  (исследование §5); знание о непубличной LameLLaMA покидает монолит.
- **Формат `mindfork-import` v1** ([docs/import-format.md](docs/import-format.md) —
  внешний контракт): `format`/`version` + `profiles[]` (key/name/language/
  system_message/greeting/character_names/sampling) + `chats[]` (key/profile_key/
  title/даты/messages c ролями user|assistant|system) + опц. `settings`
  (sampling/interface). **Осознанно НЕ доменные сущности as-is** (Р3): выделенный
  маленький формат не фиксирует внутреннюю схему (`deleted`, `reflected_upto`, …)
  внешним контрактом. Свойства: **идемпотентность** — id = UUIDv5 от стабильного
  `key` (раздельные namespace профилей/чатов — ASCII `mindfork import` + тег;
  опц. явный `id` для непрерывности с ранее импортированным); **строгость к
  структуре** (неверный `format`, версия новее поддерживаемой → downgrade-guard,
  дубликат/пустой `key`, ссылка на профиль не из файла, неизвестная роль → понятная
  локализованная ошибка с координатами) при **терпимости к расширению** (неизвестные
  поля игнорируются — эволюция без bump'а, зеркало `#[serde(default)]`); BOM
  отбрасывается. Семплинг парсится прямо в `SamplingConfig` (неизвестные ручки
  отброшены); настройки интерфейса применяются **частично** (только заданные поля —
  не затирают настройки пользователя; уточнение против прежнего wholesale).
  Системное сообщение чата: явное поле приоритетно, иначе первое system-сообщение
  истории (system в историю не пишется — в mindfork оно отдельно от сообщений).
- **`features/import.rs`** (заменил `features/migration.rs`, тот удалён):
  чистые `parse_import(json, loc)`/`import_file(path, loc)` → `ImportResult
  {profiles, chats, sampling, interface}`; `main.rs::run_import` как прежде
  (data_migration до открытия хранилища, upsert'ы, частичное применение настроек).
  **CLI**: подкоманда `import <file>` вместо `import-lamellama <dir>` (Р4 —
  удалить сразу); запуск старой команды даёт **подсказку про замену**
  (`cli.import.lamellama_removed`), а не «неизвестная команда». i18n: 11 новых
  ключей `import.*` + правки `cli.*` (ru+en; гейты parity/no-dead/кириллица
  накрыли автоматически; нюанс — имя файла в тесте не `import.json`: сканер
  гейта ловит дотированные литералы с бандл-префиксом `import.`).
- **Тесты**: golden-фикстура формата (зеркало примера спецификации, с намеренными
  неизвестными полями), идемпотентность/раздельные namespace, явный `id`,
  приоритет явного system_message, все ошибки валидации (+ локализация en без
  кириллицы), BOM/отсутствующий файл; CLI (import/подсказка import-lamellama/
  справка/выравнивание колонок). **1093 юнит-теста зелёные** (+3: −8 migration,
  +10 import, +1 cli), 48 `#[ignore]`, clippy `-D warnings`/fmt чисты.
- **Живой прогон движка не требуется** (файловая операция без движка/памяти);
  вместо него — **ручной смоук реальным бинарником** в изолированном каталоге
  (portable `data/` рядом с exe): импорт 1 профиль + 2 чата → все поля на месте
  (язык/греетинг/семплинг профиля, system из истории, thoughts, частичные
  настройки: `temp=0.7`, `theme=dark`, спелл-чек не тронут); **повторный импорт
  идемпотентен** (те же UUID, перезапись + `.bak`, без дубликатов);
  `import-lamellama` — локализованная подсказка; `--help` — ровные колонки.
  Импорт реальных данных LameLLaMA (226 чатов) — после написания приватного
  конвертера (вне этого репозитория; готовые wire-типы прежнего `migration.rs`
  переносимы в него как есть).
- **Дальше**: зонд `spike/mcp-client` (этап 2 направления, go/no-go на живой
  Gemma 4) → `feat/mcp-host` (этап 3). См. план в исследовании §7.

### Пост-M9: плагины — этап 2: зонд MCP-клиента (GO) (сделано)
- **Зонд направления «плагины»** (этап 2 плана, ветка `spike/mcp-client` — стек на
  `feat/generic-import`): проверка go/no-go «живая локальная модель корректно
  вызывает инструменты реального MCP-сервера через нашу механику agentic-loop».
  Вердикт — **GO** (результаты — в исследовании
  [docs/research/plugin-system.md §9](docs/research/plugin-system.md)), этап 3
  (`feat/mcp-host`) разблокирован.
- **Мини-клиент `shared/mcp.rs`** (tools-only, stdio, целевая ревизия 2025-11-25;
  в зонде модуль под `#[cfg(test)]` — в бинарь не входит, в бинарь войдёт на
  этапе 3): транспорт отделён от процесса — `McpConnection::over` поверх любых
  `AsyncRead`/`AsyncWrite` (юнит-тесты протокола на `tokio::io::duplex` без
  процессов), `McpClient::spawn` добавляет подпроцесс (`CREATE_NO_WINDOW`,
  непрерывный дренаж stderr в лог — иначе заполненный pipe блокирует сервер,
  shutdown-лестница «закрыть stdin → подождать → kill», `kill_on_drop`).
  Протокол: `initialize` (шлём 2025-11-25, встречную версию принимаем любую —
  tools-подмножество wire-стабильно с 2024-11-05) → `notifications/initialized`;
  `tools/list` с пагинацией `nextCursor`; `tools/call` (текстовые блоки склеены,
  не-текстовые → плейсхолдер, `isError` наружу); ответ на `ping` пустым
  результатом и `-32601` на прочие запросы сервера (молчание подвесило бы
  корректный сервер); `notifications/cancelled` при таймауте запроса;
  **мусорные строки в stdout пропускаются с warn** (питфолл №1 полевых хостов),
  неизвестные нотификации игнорируются.
- **Юнит-тесты протокола** (5, на скриптованном фейк-сервере поверх duplex):
  handshake + пагинация + вызов; баннер-мусор до JSON не рвёт соединение;
  ping→пустой результат и sampling-запрос→`-32601` (проверка ответов клиентской
  стороны с точки зрения сервера); таймаут шлёт `notifications/cancelled` с
  reason=timeout; JSON-RPC-ошибка всплывает текстом.
- **Живой смоук — GO** (`gemma_reads_file_via_mcp_filesystem_server`,
  `#[ignore]`; Gemma 4 31B q4, external `llama-server --jinja` на соседней машине
  + **реальный сторонний** `npx @modelcontextprotocol/server-filesystem`):
  сервер `secure-filesystem-server 0.2.0` подтвердил протокол **2025-11-25**;
  **14 инструментов, схемы ≈ 7 КиБ** — 16k-контекст цел; модель вызвала
  `read_text_file` с корректными JSON-аргументами **первым же раундом**,
  результат ушёл вторым раундом, финальный ответ использует содержимое файла;
  2 раунда, ~10 с, штатный shutdown. Ручной мини-agentic-loop смоука повторяет
  механику оркестратора (стрим → `ToolCallAccumulator` → исполнение →
  `assistant_tool_calls`+`tool`-сообщения → следующий раунд). Питфолл «npx —
  `.cmd`-шим» подтверждён: на Windows спавн через `cmd /c npx …`.
- **Уточнения к этапу 3 по итогам зонда**: встречную версию протокола принимать
  любую (сервер 2025-11-25 отвечает нашей же версией; строгий отказ не нужен);
  конфиг серверов должен принимать и `cmd /c npx …`-команды, и прямые exe-пути.
  **1098 юнит-тестов зелёные** (+5), **49 `#[ignore]`** (+1), clippy
  `-D warnings`/fmt чисты.

### Пост-M9: плагины — этап 3a: ядро MCP-хоста (сделано)
- **Этап 3 направления «плагины»** (docs/research/plugin-system.md §4.4–§4.7, §7;
  часть 3a — ядро; ветка `feat/mcp-host`, стек на `spike/mcp-client`): MCP-серверы
  поднимаются приложением по конфигу, их инструменты — полноправные `Tool`-ы в
  реестре, вызываются штатным agentic-loop. UI-полировка (статусы/описания в
  настройках, TOFU-пиннинг каталога), i18n-хром и доки (spec/architecture/README/
  install/CHANGELOG/ADR) — этап 3b.
- **`shared/mcp.rs` вышел из `#[cfg(test)]` в бинарь** и дорос до хоста:
  монитор-задача владеет `Child` (токены `kill`/`exited`, паттерн `managed.rs`;
  `Drop` клиента взводит kill, grace-период на самостоятельный выход после
  закрытия stdin); **Job Object kill-on-close** (Windows, приём `sandbox.rs`) —
  дерево процессов (`cmd /c npx` → `node`) не переживает выход/крах приложения;
  спавн принимает **env-карту** (уже разрешённые пары); **запрет `.bat`/`.cmd`
  как команды сервера** (`forbidden_batch_command`, BatBadBut CVE-2024-24576 —
  `cmd /c npx …` и прямые exe разрешены); `request_cancellable` — отмена токеном
  шлёт серверу `notifications/cancelled` (reason=cancelled, зеркало таймаута);
  `list_tools`/`call_tool` переехали на `McpConnection` (тестируемы на duplex
  без процессов); `shutdown(self)` упрощён до «drop + ожидание exited».
- **Конфиг `config.mcp`** (`McpSettings`, `#[serde(default)]` → без миграции):
  мастер-выключатель `enabled=false` (как Python) + `servers[]`
  (`id`-slug / `command` / `args` / `env`-карта **имён** переменных-источников —
  секреты не в `settings.json`, Р8 / `enabled` / `tool_timeout_secs`=60 /
  `max_result_chars`=20k).
- **Обёртка `McpTool: Tool`** (`features/tools/mcp.rs`): id `mcp__<server>__<tool>`
  (санитайзер `[A-Za-z0-9_-]`, ≤64 — усечение + hex-хвост от полного имени против
  коллизий); описание/схема — **снимок сервера, не локализуются** (граница i18n,
  как probe-ошибки движка); `invoke` → `tools/call` с per-call таймаутом и
  `ctx.cancel`; результат **клипуется** (`max_result_chars`, локализованная
  пометка `tool.mcp.result_truncated`); `isError` с пустым текстом → понятная
  пометка. Группа `ToolGroup::Plugins` (ALL=9) + гейт `ToolGate::Mcp`;
  `enabled_by_default=false` (**двойной opt-in**, Р7 — `reconcile_tools` не
  включает автоматически); лейблы — intern в `&'static str` (`ToolInfo.label`;
  прецедент intern кодов языков i18n).
- **`effective_tool_ids` += `mcp_enabled`**: инструменты `mcp__*` гейтятся
  мастер-выключателем **по префиксу** — они динамические, в статическом `CATALOG`
  их нет (CATALOG-lookup невозможен). 6-й параметр, все сайты обновлены.
- **Отмена в `ToolContext`** (маленький рефактор, полезный и родным инструментам):
  `TurnInfo`/`ToolContext` несут `cancel: CancellationToken` (клон токена задачи
  генерации / фоновой петли); agentic-loop оборачивает `registry.invoke` в
  **`select!` с токеном** — Esc не блокируется долгим инструментом (MCP/сеть);
  отменённый вызов даёт результат `loop.tool_cancelled`, после раунда петля
  завершает ход `Cancelled` (накопленное сохранено). MCP-вызов при этом сам
  уведомляет сервер (`notifications/cancelled`); прочим инструментам select —
  страховка.
- **`McpManager`** (`app/orchestrator/mcp.rs`, зеркало `EngineManager`): спавн
  включённых серверов **фоновыми задачами** (обработчики команд синхронны) →
  события `Ready{conn,tools}`/`Failed{reason}`/`Exited` во внутренний канал петли
  `run`; **epoch-гард** — поздние события погашенных серверов (гонка «умер до
  cancel») отбрасываются; валидация конфига до спавна (slug-id, непустая команда,
  запрет batch) → `Disconnected(reason)`; отсутствующая переменная-источник env —
  warn+пропуск. **Рестарт-бюджет** (паттерн VS Code LSP): крах после готовности →
  перезапуск, но ≤3 за 5 мин, сверх — `Disconnected` до правки настроек;
  стартовый провал (`Failed`) не перезапускается (вероятна ошибка конфига).
  `rebuild_registry()` — **единственный путь** пересборки реестра (стандартный
  набор + живые MCP-обёртки; прежние сайты `build_registry` переведены);
  правки `config.mcp` идут через дебаунс `RestartQueue.mark_mcp` (4-й флаг);
  `Quit` гасит серверы (drop слотов → cancel → shutdown-лестница).
- **Динамический каталог в UI**: `AppEvent::Settings` += `mcp_tools:
  Vec<ToolInfo>` (снимок из менеджера; `ChatScreen.settings_snapshot` стал
  4-кортежем `SettingsSnapshot`); `SettingsScreen::tool_catalog()` — instance-метод
  (статический каталог + MCP-хвост, индексы `PTool` статической части стабильны;
  `default_fields` копирует MCP-снимок в tmp-экран). Тумблеры MCP-инструментов —
  в профиле группой «Плагины (MCP)» с **честным гейтом** («выкл. глобально:
  MCP-серверы»); мастер-тумблер «MCP-серверы» — в секции «Инструменты»
  (`FieldId::TMcpEnabled`). Статусы серверов на экране — этап 3b.
- **Тесты**: клиент (отмена → `notifications/cancelled`; `call_tool` склейка/
  плейсхолдеры не-текста; запрет batch-команд, в т.ч. в `spawn`); config
  (частичная запись сервера → дефолты, round-trip); McpTool (санитайзер id —
  короткие/длинные с hex-хвостом; клип по символам + локализация; invoke через
  duplex-фейк; отмена через `ctx.cancel`); менеджер (slug-валидация; рестарт-бюджет
  на paused time; Ready строит каталог + чужой epoch отброшен; Exited чистит
  инструменты и уважает бюджет; выключенный/невалидный сервер); оркестратор
  (Ready → обёртка в реестре + `mcp_tools` в снимке Settings; Exited убирает из
  реестра); настройки (MCP-тумблеры в профиле с гейтом; мастер-тумблер).
  **1120 юнит-тестов зелёные** (+22), **50 `#[ignore]`** (+1), clippy
  `-D warnings`/fmt чисты.
- **Живой прогон — GO** (Gemma 4 31B q4, external `llama-server` + реальный
  `npx @modelcontextprotocol/server-filesystem`): `mcp_filesystem_e2e_live` —
  **полный путь через оркестратор** (конфиг с сервером → `Ready` → тумблеры
  профиля → ход): 14 инструментов в каталоге, модель вызвала
  `mcp__fs__read_text_file` и назвала секретное число из файла; клиентский смоук
  зонда (`gemma_reads_file_via_mcp_filesystem_server`) зелёный на новом API
  (протокол 2025-11-25, 2 раунда, ~5–8 с). Попутный урок теста: `wait_for`
  дренирует события — `ProfileList` (bootstrap) нужно забирать **до** ожидания
  более позднего Settings с MCP-каталогом, иначе тест зависает.
- **Осознанные границы 3a** (→ 3b/заделы): статусы серверов и просмотр полных
  описаний инструментов в UI, TOFU-пиннинг каталога (rug-pull-детектор),
  локализация статус-причин (сейчас русские, технический слой),
  `notifications/tools/list_changed` (пересписок каталога), не-текстовые блоки
  результата (плейсхолдер), потолок инструментов на сервер.

### Пост-M9: плагины — этап 3b: MCP-хост (TOFU, UI, i18n, доки, ADR) (сделано)
- **Завершение направления «плагины»** (та же ветка `feat/mcp-host`, поверх 3a):
  безопасность и наблюдаемость MCP-хоста + полный комплект документации.
  Направление (этапы 1–3) закрыто **ADR 0007** «Плагины: MCP-хост + нейтральный
  формат обмена импорта»; поведение — spec **§9.6** (новый раздел).
- **TOFU-пиннинг каталога** (rug-pull-детектор / tool poisoning, Р7):
  `catalog_hash` (`features/tools/mcp.rs`) — sha256 по отсортированным
  (имя+описание+JSON-схема) инструментов; детерминизм — сортировка по имени +
  упорядоченные ключи `serde_json` (BTreeMap). Пин — `McpServerConfig.
  pinned_catalog` (`#[serde(skip_serializing_if)]`; пишет **оркестратор**, не UI;
  ручное удаление поля = сброс доверия). Поток: первый подъём → авто-пин
  (`McpEventOutcome.pin` → `persist_mcp_pin` — прямая запись конфига, НЕ через
  `handle_update_config`, иначе diff `config.mcp` зациклил бы рестарты); совпадение
  → регистрация; **расхождение** → каталог придержан в слоте
  (`PendingCatalog{conn,tools,hash}`), инструменты модели НЕ отдаются, статус
  «каталог изменился — переподтвердите». Подтверждение: Enter на строке сервера →
  `SettingsIntent::ConfirmMcpCatalog` → `AppCommand::ConfirmMcpCatalog` →
  `McpManager::confirm` (регистрация + новый пин персистится). Защита от
  UI-гонки: `handle_update_config` **наследует пины** по id сервера из прежнего
  конфига, если снимок из UI их не несёт (устаревшая копия не сбрасывает доверие
  и не провоцирует ложный рестарт).
- **Снимок MCP-хоста в UI**: `McpSnapshot { tools: Vec<ToolInfo>, servers:
  Vec<McpServerSnapshot{id,status,tool_count,pending_catalog}> }`
  (`features/tools/mcp.rs` — FSD-чистый для screens) заменил голый `mcp_tools` в
  `AppEvent::Settings`; `handle_mcp_event` теперь эмитит настройки **на любое**
  событие (статусы live), реестр пересобирает только при смене каталога.
  `ToolInfo` получил `description: Option<String>` — **полное** описание
  MCP-инструмента (текст сервера; у встроенных `None` — их описания в бандлах).
- **UI настроек**: в группе «Плагины (MCP)» секции «Инструменты» — строки
  статусов серверов (`готов · инструментов: N` / `подключение…` / причина
  отказа; read-only `FieldId::TMcpServer(idx)`); сервер с изменившимся каталогом
  — warn + подсказка «Enter: подтвердить». Тумблеры MCP-инструментов в профиле
  показывают **полное описание** в нижней панели при фокусе (антидот
  tool-poisoning — описания идут в системный промпт). Для этого
  `FieldRow.description` переведён с `Option<&'static str>` на `Option<String>`
  (`describe(impl Into<String>)`; ~3 потребителя поправлены) — динамический текст
  сервера не интернируется.
- **i18n статус-причин** (ось B): `McpManager::apply`/`handle_event`/задача
  спавна получили `loc` — валидация конфига, рестарт-бюджет, «каталог изменился»
  и контексты `Failed` — из бандлов (`ui.err.mcp.*`, ru+en); тексты строк
  серверов — `ui.settings.mcp.*`. **Граница**: wire-ошибки клиента
  (`shared/mcp.rs`) — технический слой (как обёртки HTTP-клиентов, i18n-cli §7),
  описания/схемы инструментов — текст сервера (не локализуются).
- **Доки**: spec §9.6 (полное описание подсистемы), architecture §3 (карта:
  `shared/mcp.rs`, `orchestrator/mcp.rs`, `features/tools/mcp.rs`) + §8 (группа
  «Плагины (MCP)» в таблице, bullet реализации), README (фича-блок),
  install.md §4.2 (конфигурирование серверов с jsonc-примером, `cmd /c npx`,
  TOFU), CHANGELOG (Добавлено + Безопасность), roadmap (заделы MCP: HTTP-транспорт,
  resources/prompts, list_changed, deferred-схемы, UI-редактор, …), ADR 0007
  (+ссылки из CLAUDE.md/architecture), статус в исследовании plugin-system.md.
- **Тесты**: менеджер (первый подъём авто-пиннит + возвращает пин для персиста;
  изменившийся каталог придержан → confirm регистрирует и отдаёт хэш; совпавший
  пин — без повторного персиста; описания в `ToolInfo`); оркестратор (пин
  персистится в `settings.json`; изменившийся каталог не попадает в реестр до
  `ConfirmMcpCatalog`; наследование пинов при устаревшем UI-снимке); настройки
  (строки серверов + Enter-подтверждение только при pending; полное описание в
  строке тумблера). **1125 юнит-тестов зелёные** (+5 к 3a), **50 `#[ignore]`**,
  clippy `-D warnings`/fmt чисты.
- **Живой прогон — GO** (Gemma 4 31B q4 + реальный `npx server-filesystem`):
  e2e `mcp_filesystem_e2e_live` перегнан после 3b — TOFU-авто-пин на happy path
  прозрачен (14 инструментов, `mcp__fs__read_text_file`, 7319 в ответе, ~7 с);
  негативный путь TOFU (изменившийся каталог) покрыт юнит/интеграционными тестами
  (живая симуляция подмены сервера не нужна — механика чисто клиентская).

### Пост-M9: Mermaid — исходник до закрывающего забора (фикс мерцания при стриме) (сделано)
- **Симптом**: при стриминговом ответе ```mermaid-блок «мерцал» — по мере прихода
  чанков лента то рендерила огрызок диаграммой, то откатывалась к исходнику.
  **Причина**: CommonMark (pulldown-cmark) дотягивает незакрытый забор до конца
  документа, поэтому по событиям парсера недописанный стримом блок неотличим от
  полного, а синтаксически валидный огрызок (`flowchart LR\n A --> B` без хвоста)
  успешно рендерился частичной диаграммой. Заявленное в spec §11.4 «недописанный
  блок не парсится → исходник» опиралось на то, что огрызок не распарсится — а он
  часто парсится. Ветка `fix/mermaid-stream-fallback`.
- **Фикс** (`shared/markdown/`): закрытость забора проверяется **по исходнику**.
  `render_with` перешёл на `Parser::into_offset_iter()`; `Writer::run(src, iter)`
  перед каждым `Start(CodeBlock)` (только при включённом рендере mermaid) зовёт
  новую `fenced_block_is_closed(src, range)` — у `Start(Tag)` диапазон покрывает
  элемент целиком: блок, за которым в документе есть текст, закрыт по построению;
  у блока до EOF последняя строка диапазона должна быть закрывающим забором (та же
  литера `` ` ``/`~`, не короче открывающего; префикс цитаты/отступ тримятся).
  Результат — поле `Writer.codeblock_closed`; mermaid-ветка `start_codeblock`
  буферизует блок только при закрытом заборе, иначе — обычный путь исходника
  (байт-в-байт как при выключенном тумблере, прежний golden-инвариант). Проверка
  нарочно простая: ложное «закрыт» на экзотике лишь дало бы попытку рендера со
  штатным фолбэком.
- **Плюмбинга через экран/события не нужно**: кэш ленты уже пересчитывает
  стримящееся сообщение по фингерпринту — когда закрывающий забор доедет, блок
  пересчитается и исходник подменится диаграммой; ровно **один переход
  «исходник → диаграмма» на блок** (закрытость — свойство блока, не конца
  сообщения: закрытая диаграмма рендерится, даже пока хвост сообщения стримится).
  Бонус: блок, у которого модель навсегда забыла закрывающий забор, остаётся
  исходником (согласуется с философией жёсткого фолбэка).
- **Тесты**: writer (`mermaid_unclosed_fence_streams_as_source` — golden: 6 форм
  обрыва, включая валидный огрызок, тильда-забор и «закрывающий короче
  открывающего», байт-в-байт равны выключенному рендеру;
  `mermaid_closed_fence_renders_even_while_tail_streams`); лента
  (`streamed_mermaid_stays_source_until_fence_closes` — симуляция стрима двумя
  чанками через кэш). **1128 юнит-тестов зелёные** (+3), 50 `#[ignore]`, clippy
  `-D warnings`/fmt чисты. Живой прогон не требуется (чистый рендер-модуль без
...
- **Тесты**: интеграционные (`orchestrator/tests/self_consolidation.rs` — спавн при
  ...
  ручной шаг. **Живой смоук зелёный** (Gemma 4 31B q4 + bge-m3): при
  `auto_consolidate_every=1` два похожих наблюдения сведены, раздутый summary сжат.
...
- **Второй этап направления** (дизайн-план
  ...
  (свободный текст) — поэтому абзацы эмбеддятся **на лету** одним запросом (прямое
  зеркало ворот черт `self_model::near_duplicate_traits`).
- **Async-надстройка при синхронном хендлере** (ключевой нюанс): `build_self_
  consolidation_overview` остаётся **чистым синхронным** чтением БД (3 прежних раздела,
  тесты целы). Семантика A2 — отдельный **async** хелпер
  `notes::summary_observation_overlaps(storage, embedder, profile, loc)` (мягкая
  деградация: нет эмбеддера / нестыковка числа векторов → `None`). Проводка в три места:
  (1) интерактивный `reflect` (`Reflect::invoke`, уже async) — дописывает секцию;
  (2)+(3) фоновые рефлексия/`self_consolidation` — их хендлеры **синхронны** и
  спавнят задачу, поэтому секция считается **внутри спавненной задачи**: у `SilentLoop`
  новое опциональное поле `summary_semantics: Option<SummarySemantics{embedder,storage,
  profile_id,loc}>`, и `spawn_silent_loop` перед петлёй `await`-ит хелпер и дописывает
  результат к первому user-сообщению запроса. Консолидация заметок (`consolidation.rs`)
  передаёт `None` (её обзор — о заметках собеседника, не о `summary`).
- **Порог откалиброван на живом bge-m3** (не угадан; смоук
  `summary_obs_calibration_e2e_live` печатает косинусы размеченных пар): перефразы
  «абзац ↔ наблюдение» дали **0.69–0.80**, несвязанные пары — **0.48–0.51**; чистый
  разрыв 0.51→0.69 → `SUMMARY_OBS_SIMILARITY = 0.62` (в разрыве, с запасом в обе
  стороны). Абзацы длиннее коротких черт, поэтому перефразы чуть ниже, чем у ворот
  черт (0.73–0.83, порог 0.72). Одно совпадение на абзац (не шумим); фрагменты < 40
  символов отбрасываются.
- **Без конфига/миграции** — порог пока код-константа (доп. поле настроек не заводим,
  как в дизайн-доке); i18n-ключ раздела `notes.self_overview.summary_obs` (ru+en).
  DB-only, инвариант «единственный владелец `Chat`» цел; FSD (хелпер в `features`,
  `tool_loop` его зовёт).
- **Тесты**: юнит на `MockEmbedder` (порого-устойчивы: совпадение ≈1.0, несвязанное
  ≈0.0) — раздел поднимает совпавший абзац и не поднимает несвязанный; мягкая
  деградация (нет наблюдений / пустой summary / нестыковка векторов → `None`). Живые
  `#[ignore]`: `summary_obs_calibration_e2e_live` (печать косинусов для калибровки) и
  `summary_obs_overlap_e2e_live` (секция поднимается на реальном bge-m3 — измерено
  0.77). **1138 юнит-тестов зелёные** (+2), 53 `#[ignore]`, clippy `-D warnings`/fmt/
  i18n-гейты чисты. **Живой прогон зелёный** (bge-m3): калибровка + подъём секции
  подтверждены; порог 0.62 ловит все перефразы, отсекает несвязанные.
- **A3** (старение `current_interests`) — следующий/последний этап направления.
...
- **Заключительный этап направления** (дизайн-план
  ...
  «протокол ведения» `selfmodel.policy_core` — «интересы «текущие»: убирай через
  `remove_interests` те, что собеседник давно не подтверждает». Инструмент
  `update_user_model` уже поддерживает `remove_interests` — код/схема не тронуты,
  правка только текста бандла (ru+en).
- **Единый источник → три потребителя**: `policy_core` композицией питает
  турную инъекцию (`maintenance_protocol`), авто-рефлексию и авто-консолидацию модели
  себя — нудж propagates во все три без дублей. **Тяжёлый путь** (`current_interests:
  Vec<Interest{text, updated_at}>` — настоящее старение по времени) сознательно **не**
  делали: ломает плоский `Vec<String>` и ~десяток мест; оставлен заделом, если
  лёгкого на живой модели окажется мало.
- **Тесты**: `policy_core_nudges_interest_aging` (нудж несёт `remove_interests` +
  идею старения). **1139 юнит-тестов зелёные** (+1), 53 `#[ignore]`, clippy
  `-D warnings`/fmt/i18n-гейты чисты. Эффект поведенческий/мягкий (композированные
  промпты уже покрыты живыми смоуками рефлексии/консолидации) — отдельный живой прогон
  не требуется. **Направление «консолидация модели себя» (A1–A3-лёгкий) — завершено**
  (A3-тяжёлый — задел).
...
- **Первый этап направления «RAG: источники и извлечение»** (дизайн-план
  ...
  отбрасывая nav/header/footer/aside/скрипты; `pub(crate)`, покрыт тестами).
- **FSD-чистая раскладка** (осознанное отклонение от буквы дизайн-дока, где
  `extract_text`-диспетчер предлагался в `rag_ingest`): `features/rag_ingest.rs`
  остаётся **чистым файловым модулем** (`SUPPORTED_EXTENSIONS` += `html`/`htm` +
  хелпер `is_html`), а **извлечение диспетчеризуется в слое `app`**
  (`orchestrator/rag.rs::read_source_text`) — иначе `features → features/tools` было
  бы боковым импортом, запрещённым FSD; `app` же вызывать `features/tools/web` вправе.
  `read_source_text` (raw через `read_text` → для HTML `extract_readable(&raw,
  usize::MAX)`, RAG чанкует целиком → без усечения) заменил `read_text` в двух точках
  чтения (`index_file` и legacy-чтение файла при `/rag rebuild`).
- **`index_source` не тронут**: HTML-источник — не markdown → идёт в `chunk_text`
  (у извлечённого текста нет структуры заголовков). Единственная пользовательская
  строка про форматы (`ui.err.rag_no_files`) обновлена на `.txt/.md/.html`.
- **Тесты**: `is_supported`/`is_html` на html/htm/HTML (регистронезависимо);
  `read_source_text` извлекает абзац статьи и отбрасывает nav/header/script,
  не-HTML возвращает дословно (tempdir). **1141 юнит-тест зелёный** (+2), clippy
  `-D warnings`/fmt/i18n-гейты чисты. Живой прогон не требуется (чистое извлечение
  без движка). Задел направления: **B1b** (pdf/docx — с новыми крейтами и проверкой
  лицензий), **B2** (ранкинг/дедуп + прогресс по чанкам).
...
- **Второй этап направления «RAG: источники и извлечение»** (дизайн-план
  ...
  двигается по мере готовности чанков.
- **Механика**: `RagProgress::Indexing` получил поля `chunks_done`/`chunks_total`
  (0/0 — файл только начат, чанкинг впереди → баннер как раньше). `index_source`
  принял колбэк `progress: impl FnMut(usize, usize)` и эмбеддит/пишет циклом по
  под-батчам `chunks.chunks(EMBED_BATCH_CHUNKS=16)`, вызывая `progress(done, total)`
  после каждого; `index_file` пробрасывает колбэк. Обе фоновые задачи
  (`spawn_rag_ingest`, `spawn_rag_rebuild`) эмитят стартовый `Indexing{…,0,0}`, затем
  из колбэка — обновлённые счётчики чанков. Баннер (`screens/chat/rag.rs`) при
  `chunks_total>0` дописывает суффикс `ui.rag.chunks` (« · чанки N/M»).
- **Смена батчинга** (улучшение): файл с ≤16 чанками — по-прежнему один запрос
  (идентичный результат); крупный файл теперь шлёт **ограниченные** по размеру
  запросы вместо одного гигантского (bounded память/запрос + живой прогресс). Порядок
  чанков и хранимые документы не меняются. Запросы **меньше** прежнего единого батча,
  поэтому на реальном сервере строго безопаснее.
- **Тесты**: `index_source_reports_chunk_progress_in_subbatches` (файл > 16 чанков:
  первый тик `(0,N)`, последний `(N,N)`, монотонность, все N документов записаны и
  находятся `rag_search`), `index_source_empty_content_single_zero_tick`, банннер-тест
  (суффикс чанков при `chunks_total>0`, без него при 0). **1143 юнит-теста зелёные**
  (+2), clippy `-D warnings`/fmt/i18n-гейты чисты. Живой смоук `rag_en_e2e_live` на
  реальном bge-m3 зелёный (embed-путь цел; под-батчинг регрессий не внёс).
  `index_source` вышел на 8 аргументов — точечный `#[allow(clippy::too_many_arguments)]`
  с пояснением (когезивные аргументы, выделять пучок ради одного параметра — лишний
...
- **Текстовый дедуп, не эмбеддинговый** (развилка, решение пользователя): дизайн-док
  предлагал эмбеддинговый дедуп+реранкинг, но `rag_search` — **горячий путь** (каждый
  поиск), а vec0 **уже ранжирует** по релевантности (distance). Переэмбеддинг пассажей
  на каждый поиск добавил бы задержку, а реранкинг во многом дублировал бы сортировку
  vec0. Поэтому — детерминированный текстовый дедуп: **без эмбеддера, без порога/
  калибровки, без задержки**.
- **Механика** (`features/tools/rag.rs::dedup_passages`, чистая функция): после
  `stitch_hits` (склейка соседних чанков внутри источника) пассажи проходят дедуп —
  в порядке ранжирования оставляется пассаж, только если его нормализованный текст
  (нижний регистр Unicode + схлопывание пробелов + трим) **не содержится целиком** в
  тексте уже оставленного (проверка `k.contains(&norm)` покрывает и равенство, и
  вхождение-подмножество). Порядок vec0 сохраняется (не пересортировываем).
  Источнико-агностично (главный случай — кросс-источниковый дубль, но внутриисточниковый
  повтор тоже шум). **Лоу-ранкед супермножество** более релевантного пассажа — оба
  остаются (не теряем информацию: снимаем только пассаж, входящий в **более
  релевантный** уже оставленный). Вставлено как `dedup_passages(stitch_hits(hits))` в
  `RagSearch::invoke` перед форматированием; формат/пустая выдача не тронуты.
- **Тесты**: идентичный дубль из другого источника → один (более релевантный);
  подмножество более релевантного → снято; различные пассажи → все, порядок цел;
  дедуп нечувствителен к регистру/пробелам; лоу-ранкед супермножество → оба остаются;
  пустой вход → пустой выход. **1157 юнит-тестов зелёные** (+6), clippy `-D warnings`/
  fmt чисты. Без новых зависимостей/конфига/i18n. Живой прогон не требуется (чистая
  текстовая логика; embed-путь не тронут). **Направление «RAG: источники и извлечение»
  (B1a html + B1b pdf/docx + B2a прогресс по чанкам + B2b дедуп) — завершено.**

### Релиз 0.9.1 (подготовлен)
- **Релизный PR** по чек-листу [AGENTS.md §6](AGENTS.md) (ветка
  `chore/release-0.9.1`): bump `Cargo.toml` `0.9.0 → 0.9.1` (+ `Cargo.lock`),
  `CHANGELOG.md` — `[Unreleased]` → `[0.9.1] — 2026-07-18`, заведена пустая
  `[Unreleased]`, обновлены сравнительные ссылки. Тег `v0.9.1` ставит пользователь
  после мержа (агент теги/`main` не пушит).
- **Пакеты и инсталлятор в релизе уже были подключены** (направление «инсталляторы»,
  этапы 2–3): `release.yml` содержит job'ы `linux-packages` (nfpm → deb/rpm/
  pkg.tar.zst) и `windows-installer` (Inno Setup → `setup.exe`), оба в `needs`
  публикующего job'а, артефакты копируются в `dist/` и попадают в `sha256sums.txt` и
  GitHub Release. Кода пайплайна править не пришлось — сверены только концы:
  `.iss` пишет в `dist/*.exe` (совпадает с путём выгрузки) и принимает
  `/DAppVersion`, `build-packages.sh` + `packaging/nfpm.yaml` на месте. **v0.9.1 —
  первый тег, фактически несущий пакеты/инсталлятор**: они появились в пайплайне уже
  после `v0.9.0` (в опубликованном 0.9.0 их нет; там же нет и словарей спелл-чека).
- **Гейты**: `cargo fmt --check` / `cargo clippy --all-targets -- -D warnings` /
  `cargo test` зелёные — **1157 юнит-тестов, 53 `#[ignore]`**. Живой прогон не
  требуется (версия + доки, кода приложения не касались). Попутно: в `[0.9.1]`
  слиты две дублирующие рубрики «Исправлено» и порядок рубрик приведён к
  объявленному в шапке CHANGELOG; счётчик тестов и дата в шапке «## Статус»
  актуализированы (были 1128/50 от 2026-07-17).

### Пост-M9: айдентика — логотип и вордмарк (этапы 1–4) (сделано)
- **Новое направление «айдентика»** (дизайн-план [docs/branding.md](docs/branding.md)):
  довести каталог `artwork/` до полноценной айдентики — переносимые ассеты,
  бренд-гайд, иконка в пакетировании, логотип в TUI, вордмарк в документации.
  Развилки Р1–Р7 подтверждены пользователем 2026-07-18: **Р2** — логотип в шапке
  оверлея помощи (`F1`), **Р3** — палитру интерфейса **не** трогать (`accent` несёт
  смысл «активность»; логотип рисуется фирменными цветами), **Р4** — `.desktop`
  с `Terminal=true`; Р5–Р7 по рекомендации (CLI-баннер не делаем, `artwork/` —
  единственный источник, AppStream — задел).
- **Этап 1 `feat/brand-assets`** — ассеты в git + починка вордмарков. **Ключевая
  находка**: все шесть `mindfork-wordmark*.svg` несли `<text class="wm">` **без
  `<style>`, `font-family` и `font-size`** (классы нигде не определялись — блок
  стилей потерялся при экспорте), т.е. рендерились дефолтным шрифтом с засечками.
  Дописать `font-family` не помогло бы: у зрителя (GitHub, чужой браузер,
  просмотрщик в Linux) нужного моноширинного шрифта нет. **Текст переведён в кривые**
  (`<path>`) — SVG самодостаточны. Шрифт восстановлен по эталону
  `wordmark-example.png` **подгонкой метрик** (границы букв → метрики шрифта, МНК):
  **JetBrains Mono ExtraBold** (SIL OFL 1.1), кегль 277.7 px, трекинг −0.035 em.
  Реконструкция совпадает с эталоном **попиксельно** — ширина слова 1249 px и зазор
  «иконка → текст» 213 px в обоих, ширины букв расходятся ≤3 px из ~140
  (антиалиасинг); подтверждающая деталь — выносной вылет `o`/`d` (−10 единиц шрифта)
  предсказывает их нижнюю кромку ровно на измеренные 348 px. Шрифт нашёлся локально
  (бандл PyCharm) — качать не пришлось; `.ttf` в репозиторий не кладём (нужен только
  для перегенерации). Добавлены `artwork/build-wordmarks.py` (генератор; палитра,
  трекинг и пропорции лockup'а — константами) и `artwork/README.md` (бренд-гайд:
  палитра, геометрия глифа, метрики, правила применения). `viewBox` подтянуты под
  фактическое содержимое (`219.9×48` вместо `340×80` и т.д.) — прежние значения
  предполагали кегль вдвое больше эталонного и никогда корректно не рендерились;
  вертикальный лockup перерисован (ширина слова = 2× иконки — эталона для него нет, а
  при пропорциях горизонтального композиция была нижне-тяжёлой). Вордмарк поставлен
  в шапку README (`<picture>` + `prefers-color-scheme`, фолбэк — светлый вариант;
  вложен в `<h1>` с `alt`, чтобы заголовок остался доступным).
- **Этап 2 `feat/windows-icon`** — иконка `.exe` крейтом **`winresource`** (форк
  заброшенного `winres`) из готового `mindfork.ico`. **Двойной гейт в `build.rs`
  обязателен**: крейт объявлен в `[target.'cfg(windows)'.build-dependencies]`, а у
  **build**-зависимостей `cfg` вычисляется по **хосту** — на Linux-хосте крейта нет
  и обращение к нему не скомпилируется; поэтому `#[cfg(windows)]` по хосту (есть ли
  крейт) **плюс** `CARGO_CFG_TARGET_OS` по таргету (нужна ли иконка). Следствие:
  кросс-сборка Linux → Windows иконку не вошьёт — там печатается `cargo:warning`
  (штатный путь не страдает: релизный workflow собирает Windows на windows-раннере).
  Сбой вшивания не валит сборку (`cargo:warning`) — на машине без Windows SDK
  приложение должно собираться. В `.iss` добавлены `SetupIconFile` и
  `WizardSmallImageFile` (`artwork/mindfork-wizard-small.png`, 138×140 — размер
  modern-стиля, белый фон под белую шапку Inno); `UninstallDisplayIcon` и ярлыки
  `[Icons]` подхватили иконку сами.
- **Проверено вживую**: `rc.exe` из Windows SDK нашёлся, сборка без предупреждений;
  иконка **извлечена** из собранного `.exe` и из `setup.exe` — обе идентичны и несут
  ровно брендовые `#09090b`/`#5c6370`/`#c25a27`; `.iss` скомпилирован; мастер снят
  скриншотом (иконка в заголовке окна + логотип в шапке страницы; установка **не**
  выполнялась). **Опасение «Inno 6 принимает только BMP» не подтвердилось** —
  тест-компиляция обоих форматов прошла, выбран PNG (в 35 раз легче: 1.6 КБ против
  58 КБ). `cargo deny check` — `advisories/bans/licenses/sources ok` (winresource
  тянет 7 build-only крейтов: toml-стек + winnow). **1157 юнит-тестов зелёные**
  (число не изменилось — код крейта не затронут), 53 `#[ignore]`, clippy/fmt чисты.
- **Этап 3 `feat/linux-desktop-entry`** — запись в меню приложений и иконки темы:
  `packaging/linux/mindfork-rs.desktop` (**`Terminal=true` обязателен** — приложение
  TUI, stdout занят интерфейсом; без него запуск из меню закрыл бы окно мгновенно;
  локализованные `GenericName`/`Comment`/`Keywords` для ru) + `nfpm.yaml`: `.desktop`
  в `/usr/share/applications`, шесть растровых иконок в `hicolor/<N>x<N>/apps` и svg
  в `hicolor/scalable/apps` — все под именем `mindfork-rs` (обязано совпадать с
  ключом `Icon=`). Смоук `packaging.yml` расширен проверками наличия/содержимого
  `.desktop` и всех иконок; `desktop-file-utils` ставится в Ubuntu/Fedora, чтобы
  штатный `desktop-file-validate` **реально отрабатывал**, а не пропускался по
  `command -v`. `Chat` в категориях **намеренно нет** — по спецификации требует
  основной `Network`, иначе валидатор предупреждает. Скриптлеты обновления кэшей не
  добавляем: Debian (триггер `update-icon-caches`) и Fedora (file trigger на
  `hicolor`) делают это сами. Проверено локально настолько, насколько возможно на
  Windows (YAML валиден, все `src` из `contents` существуют, `.desktop` — UTF-8 без
  BOM, только LF, есть основная категория); реальные `nfpm`/установка в контейнерах
  на Windows невоспроизводимы — их валидирует `packaging.yml` на PR.
- **Этап 4 `feat/tui-logo`** — логотип ячейками терминала. Новый `widgets/logo.rs`:
  пара вертикальных пикселей кодируется одной ячейкой (обе половины одного цвета —
  `█`; разные — `▀` с `fg`/`bg`; одна — `▀`/`▄` **без фона**, чтобы не тащить
  прямоугольник подложки). Размер — **по границам чернил** (x 3…13, y 2…14 → 10×12
  пикселей = **10 колонок × 6 строк**; высота чётная, половинные блоки ложатся
  ровно), а не по полной сетке 16×16 с пустыми полями. Цвета — **фирменные RGB, не
  из палитры** (Р3): логотип не перекрашивается темой. Глифы `█`/`▀`/`▄` входят в
  WGL4 → режим совместимости с conhost отдельной замены не требует (как `█`
  скроллбара и `▌` рейлов). Размещение — первым блоком содержимого оверлея помощи
  (`F1`, там уже версия в титуле → де-факто «About», новых клавиш не потребовалось),
  но **только при запасе высоты**: список клавиш длинный (33 записи, попап ~35
  строк), поэтому при нехватке логотип не рисуется вовсе — клавиши не сдвигаются и
  лишней прокрутки не появляется (та же деградация, что у скроллбара и Mermaid).
- **Гейт «код ≡ ассет»**: тест парсит `<rect>` из
  `artwork/mindfork-icon-transparent.svg` и сверяет с таблицей `GLYPH` в коде —
  ассет и код не разъедутся молча (приём как у i18n key-parity и `LABEL_CAP`).
  **Мутационно проверен**: правка одной константы роняет тест с понятным сообщением.
  **1164 юнит-теста зелёные** (+7: 5 на виджет — включая «только WGL4-глифы» и
  «ствол есть в каждой строке» — и 2 на показ/скрытие в справке), 53 `#[ignore]`,
  clippy `-D warnings`/fmt чисты. Живой прогон не требуется (чистый UI, `TestBackend`).
- **Направление «айдентика» (этапы 1–4) завершено.** Задел: большой
  `WizardImageFile` для страницы «Завершение» инсталлятора, AppStream metainfo (Р7),
  ASCII-баннер в CLI (Р5 — решено не делать).

### Пост-M9: айдентика — вордмарк в TUI, лockup вместо голого глифа (сделано)
- **Доводка этапа 4 по замечаниям пользователя** (ветка `feat/tui-lockup`): в шапке
  оверлея помощи (`F1`) был **центрированный глиф без слова и без отбивки сверху** —
  читался отдельной картинкой, приклеенной к рамке. Теперь там **горизонтальный
  лockup** (глиф + слово `mindfork`), прижатый влево, с отбивками сверху и снизу.
  Ассеты/пакетирование не тронуты; правки только в `widgets/logo.rs` и
  `screens/chat/popups.rs`.
- **Вордмарк — собственный пиксельный шрифт, а не SVG.** Переиспользовать
  `artwork/mindfork-wordmark*.svg` нельзя: там текст **переведён в кривые** (этап 1,
  §2 branding.md) — растеризовать их в терминале нечем. Поэтому в `logo.rs` заведена
  матрица `#`/`.` на букву (`WORDMARK`, 8 глифов слова), рисуемая теми же
  половинными блоками, что и глиф. Начертание повторяет бренд (JetBrains Mono
  ExtraBold): строчные, выносные у `d`/`f`/`k`, **штрихи в 2 пикселя** — той же
  толщины, что бары глифа (в его сетке они тоже по 2 единицы). Итог — 52 колонки × 4
  строки; лockup целиком — **65×6**.
- **Пропорции взяты из метрик бренда, не на глаз** (`artwork/README.md`): высота
  слова = `0.5227 × S`, где `S` — **размер иконки** (16) → ≈ 8 пикселей = 4 строки
  против 6; базовая линия (`0.7418 × S`) → низ слова на **строку выше** низа глифа
  (`WORDMARK_TOP_ROW = 1`); зазор `0.3608 × S` меряется от **бокса** иконки, а мы
  рисуем только чернила (x 3…13), поэтому в терминале он равен `0.3608 × 16 − 3`
  ≈ **3 колонки**. Разрядка между буквами — 1 колонка.
- **Правка по отзыву на первую версию** (слишком много воздуха справа; `i` вдвое
  жирнее прочих букв) — **один корень**: доля `0.5227` бралась от высоты **чернил**
  глифа (12), а не от размера иконки (16), поэтому слово вышло 6 пикселей вместо 8.
  В 6 пикселей 2-пиксельные штрихи не влезают (x-высота 4 не оставляет просвета
  внутри `o`/`d`), и шрифт получился 1-пиксельным — светлым при чанковом глифе;
  единственной буквой с правильным 2-пиксельным стволом оказалась `i`, оттого и
  «двойная ширина». Исправление высоты до 8 разблокировало правильный вес: слово
  стало ExtraBold, как бренд, и заодно выросло с 48 до 65 колонок — воздух справа
  ужался с ~26 до ~7 колонок (при попапе ~76). Обе претензии закрылись одной правкой.
- **Вторая правка по отзыву — `f`**: у неё крючок (выносная высота) и перекладина
  (x-высота) идут подряд, а разделяющего ряда в бюджете нет — 2-пиксельный крючок
  слипался с перекладиной в сплошной блок. Крючок сделан **1-пиксельным**: попадает в
  верхнюю половину ячейки (`▀`), нижняя пуста → просвет виден, а перекладина остаётся
  на x-высоте, в одну строку с верхними барами `n`/`o`/`r` (альтернатива — опустить
  перекладину к середине x-высоты — сохранила бы вес крючка, но сломала общую линию).
  `f` расширена 5 → 6 колонок, чтобы крючок был двухколоночным `▀▀` и читался.
- **Цвет `mind` — из палитры, и это не нарушение Р3.** `fork` остаётся фирменным
  акцентом (`#c25a27`), а `mind` в бренде — «текст на тёмном»/«текст на светлом»
  (ровно для этого заведены раздельные `-dark`/`-light` варианты), т.е. цвет,
  производный от фона. Берём `palette.text` → один лockup корректен в обеих темах,
  знаковые цвета при этом фирменные. Глифы `█`/`▀`/`▄` — WGL4, компат-режим conhost
  отдельной замены по-прежнему не требует.
- **Выравнивание влево** по тому же полю, что список клавиш (два пробела), плюс
  пустая строка сверху (отбивка от рамки) и снизу. Знак читается как шапка блока и
  делит вертикаль с клавишами. Блок вырос с `LOGO_ROWS + 1` = 7 до `LOCKUP_ROWS + 2`
  = 8 строк.
- **Гейт показа расширен шириной**: прежде проверялась только высота. Ширину попапа
  под лockup **специально не растягиваем** (попап меряется списком клавиш) — если
  `width < LOCKUP_COLS + отступ + поля`, знак не рисуется вовсе. Та же жёсткая
  деградация, что у скроллбара и Mermaid. Порядок вычислений в `render_help`
  переставлен: сперва ширина, затем решение о знаке (оно от неё зависит), затем
  высота попапа.
- **Инвариант «слово влезает в высоту глифа» — `const`-ассерт**, а не тест: правка
  шрифта или `WORDMARK_TOP_ROW`, из-за которой `lockup_lines` молча обрезала бы
  нижние строки слова, **роняет сборку**. Гейт «код ≡ SVG» для глифа (этап 4) цел.
- **Тесты**: шрифт целостен (слово — ровно `mindfork`, матрицы прямоугольные, нет
  пустых букв, посторонних символов нет); размер слова и раздельная окраска
  `mind`/`fork`; лockup не трогает глиф (первые `LOGO_COLS` колонок совпадают
  байт-в-байт) и ставит слово **только** в свои 3 строки; `uses_only_wgl4_block_glyphs`
  распространён на лockup; тест справки переписан с «есть оранжевый» на позиционный —
  ствол стоит ровно на поле списка клавиш (т.е. знак прижат влево, а не центрирован),
  «fork» — справа от глифа. Ловушка теста: `Span::content.len()` — **байты**, а
  блочные глифы 3-байтовые (ширину считать `chars().count()`); и `╭` в буфере есть у
  панелей самого чата в колонке 0 — угол попапа берётся как самый правый.
  **1167 юнит-тестов зелёные** (+3), 53 `#[ignore]`, clippy `-D warnings`/fmt чисты.
- **Живой прогон не требуется** (чистый UI без движка/памяти; покрыт `TestBackend`).
  Композиция сверена дампом отрисованного попапа в обеих локалях (ru/en): слово
  читается, лockup вписывается в ширину попапа с запасом (65+6 против ~78).

### Пост-M9: попап эмодзи — «висячий» след выделения после закрытия (сделано)
- **Симптом** (ветка `fix/emoji-picker-residue`): после закрытия попапа эмодзи
  (`Ctrl+B`) на месте выделенной ячейки оставался цветной прямоугольник шириной
  ~1 колонка (фон `keycap_bg`), переживавший последующие перерисовки.
- **Причина найдена эмпирически** (проба на реальном `Buffer::diff` ratatui 0.30.1,
  не догадкой): широкий эмодзи занимает **две** ячейки — свою (символ) и
  **хвостовую**, которую ratatui сбрасывает в дефолт (`" "`, дефолтный стиль).
  Когда попап закрывается, кадр «попап → пусто» даёт обновления для ячеек `x`
  (символ) и `x+2` (пробел справа), но **не для `x+1`**: в обоих буферах она
  дефолтный пробел, diff считает её неизменной и терминалу не шлёт. Сам conhost/
  Command Prompt вторую половину широкого глифа при перезаписи первой не чистит →
  на экране остаётся её кусок. Виден он только там, где у ячейки был **фон**, —
  поэтому «светилась» ровно выделенная ячейка сетки, а не весь попап.
- **Фикс** — переиспользован уже существующий в проекте механизм полной
  перерисовки без мигания (буфер-сентинел `"\0"` + `swap_buffers`, заведённый для
  «съезжающих» VS16-эмодзи при прокрутке ленты): проверено пробой, что с сентинелом
  diff эмитит **все** ячейки, включая хвостовую. `ChatScreen` получил флаг
  `full_redraw` + `request_full_redraw()`; `handle_emoji_key` просит перерисовку на
  **любое** действие в попапе — не только на закрытие, но и на сдвиг выделения
  стрелками (подложка так же уезжает с прежней ячейки, оставляя тот же артефакт).
  Открытие попапа перерисовки не требует (глиф ниоткуда не уходит).
- **Обобщение флага без потери прежнего источника**: петля `app/runtime` перешла с
  `take_feed_scrolled()` на `take_full_redraw()`, который забирает **оба** источника
  безусловно (не через `||` — `take_feed_scrolled` обязан сбросить внутреннее
  состояние виджета ленты независимо от второго запроса); прежний VS16-путь
  дополнительно закреплён ассертом в существующем тесте.
- **Тесты**: поведение экрана (`emoji_picker_actions_request_full_redraw` — закрытие
  `Enter`/`Esc` и сдвиг выделения просят перерисовку, флаг забирается однократно,
  открытие — не просит; **мутационно проверен**: снятие `request_full_redraw()`
  роняет тест) + **канарейка на корневую причину**
  (`wide_glyph_trailing_cell_is_not_repainted_by_plain_diff` в `widgets/emoji_picker`):
  фиксирует, что обычный diff хвостовую ячейку пропускает, а сентинел — переписывает;
  падение теста будет означать, что апстрим починил это сам и обходной путь можно
  пересмотреть. **1169 юнит-тестов зелёные** (+2), 53 `#[ignore]`, clippy
  `-D warnings`/fmt чисты. Живой прогон движка **не требуется** (чистый UI-фикс без
  движка/памяти); визуальная проверка на conhost — за пользователем.
- **Смежный случай закрыт по запросу пользователя: попап подсказок орфографии
  (`Ctrl+G`).** Сперва **проверен пробой**, а не «на всякий случай»: гипотеза была,
  что дефекта нет, т.к. `List` кладёт подсветку через `buf.set_style(row_area, …)`
  **после** отрисовки содержимого — казалось бы, накрывая и хвостовую ячейку.
  Проба показала обратное: глиф `➕` = `mod=ITALIC | REVERSED`, а его **хвост** —
  `mod=NONE`, и при закрытии он не перерисовывается. То есть класс тот же, что у
  эмодзи (подсветка до хвостовой ячейки не доходит), и фикс нужен. Добавлен
  `request_full_redraw()` в `handle_suggest_key` — **точнее, чем у эмодзи**: только
  когда действие реально что-то меняет (`Esc`/`Enter`/`↑`/`↓`), а прочие клавиши
  (`_ => return`) перерисовку не просят. Тест
  `suggest_popup_actions_request_full_redraw` (сдвиг выделения / оба закрытия просят,
  no-op клавиша — нет; **мутационно проверен**). `➕` — не VS16 (U+2795), поэтому
  сдвига ряда, как у `❤️`, здесь не возникает.
- **Вторая итерация по отчёту пользователя (та же ветка): VS16-кластеры убраны из
  списка попапа.** Полная перерисовка вскрыла соседний дефект: после прохода
  выделением по сетке пропадал `🔥` и «ломалась» рамка в двух местах (ряды 3 и 4 —
  ровно те, где стояли `✌️` и `❤️`). **Механика** (подтверждена пробой на реальном
  `Buffer::diff` + чтением бэкенда, не догадкой): для VS16-кластера `ratatui`
  **намеренно** шлёт хвостовую ячейку глифа (их обход терминалов, не чистящих вторую
  половину), а `CrosstermBackend::draw` ведёт `last_pos` **по номеру ячейки, без
  учёта ширины символа** (`x == p.x + 1` → без `MoveTo`) — хвост печатается на
  колонку правее, и остаток ряда съезжает: у идущего следом широкого эмодзи
  затирается правая половина (conhost гасит глиф целиком → пропал `🔥`), а рамка
  выталкивается наружу. Без сентинела хвост не эмитился (символ не менялся:
  пробел → пробел), поэтому дефект и проявился только после первой итерации.
  **Фикс — у источника**: `✌️`→`🤞`, `❤️`→`💖` (supplementary-плоскость, честные
  2 колонки, хвостовых ячеек не порождают); прочий набор не тронут. Ширины сверены
  программно (`unicode-width` 0.2 считает VS16-кластер как 2 — модель сетки была
  согласована, дело именно в выводе в терминал).
- **Два гейта на инвариант** (оба **мутационно проверены** возвратом `❤️` в список):
  `emoji_list_is_width2_without_vs16` — прямой (ширина 2, нет U+FE0F, сообщение
  подсказывает замену) и `sentinel_repaint_never_writes_into_second_half_of_wide_glyph`
  — поведенческий и терминало-независимый: при полной перерисовке ни одно обновление
  diff'а не целится во вторую половину широкого глифа. **1174 юнит-теста зелёные**
  (+7 к `main`), 53 `#[ignore]`.
- **Третья итерация: тот же дефект найден и вылечен в ленте — на уровне сентинела.**
  Проверка отложенного риска (запрос пользователя): давний путь прокрутки ленты
  пользуется тем же приёмом и включается **именно при наличии VS16**. Проба на
  реальном кадре ленты (40 строк с `🗂️`): при **обычной** прокрутке — **0**
  обновлений во вторую половину широкого глифа, **через сентинел — 6**, ровно по
  числу VS16-ячеек в кадре. То есть сдвиг ряда в ленте вызывал **сам сентинел**.
- **Фикс у механизма, а не у контента** (в отличие от эмодзи-списка): сентинел вынесен
  в именованный `shared/ui.rs::prime_full_redraw` и переустроен — вместо символа
  `"\0"` ячейка получает **пробел + модификатор-маркер `HIDDEN`**. Пробел совпадает с
  содержимым хвостовой ячейки широкого глифа (её `ratatui` сбрасывает в дефолт),
  поэтому условие эмиссии хвоста (`prev.symbol() != next.symbol()`) больше не
  срабатывает — сдвига нет; а отличие по модификатору сохраняет полноту перерисовки.
  `HIDDEN` в UI не используется (палитра обходится `DIM`/`BOLD`/`ITALIC`/`UNDERLINED`/
  `REVERSED`) — закреплено тестом `sentinel_modifier_is_unused_by_ui`. Замер: **1194
  из 1200** ячеек перерисованы, непокрытыми остаются ровно 6 хвостовых половин (их
  закрывает сам глиф — писать туда и не нужно).
- **Гейт переехал к механизму**: `prime_full_redraw_repaints_all_but_wide_glyph_tails`
  (`shared/ui.rs`) на синтетическом кадре с VS16 проверяет оба свойства —
  (1) ни одно обновление не целится во вторую половину широкого глифа, (2) дыр в
  перерисовке нет (непокрытые ячейки — только хвостовые). **Мутационно проверен**:
  возврат сентинела `"\0"` роняет его с точным сообщением. Тесты попапа эмодзи
  переведены на реальный хелпер вместо своей копии сентинела. **1177 юнит-тестов
  зелёные** (+10 к `main`), 53 `#[ignore]`.
- **Четвёртая итерация: триггер полной перерисовки расширен с прокрутки на изменение
  содержимого ленты.** Детальный прогон пользователя на conhost (чат про механику
  эмодзи — VS16, ZWJ-семьи, суррогатные пары) показал: артефакты вылезают при
  **потоковом выводе** и при **добавлении заметки** (`F5` «Переписка скопирована…»,
  лента при этом прокручивается вниз), а **прокрутка их чинит**. Причина системная:
  полную перерисовку заказывала **только** прокрутка (`take_feed_scrolled`), а
  `scroll_to_bottom` флаг вообще не взводит — изменение содержимого триггера не имело.
- **Как определяется «изменение» (итог второй попытки)**: флаг ставят сами **мутаторы**
  ленты (`ChatScreen::mark_feed_changed` в `activate_chat`/`push_*`/`continue_`/
  `rewrite_assistant`/`Ctrl+T`). Первая попытка определяла изменение **по факту
  отрисовки** (промах кэша блоков ленты) — работало, но опаздывало на кадр: артефакт
  успевал **мелькнуть** («дефекты появляются на долю секунды и исчезают» в отчёте
  пользователя). Спрятать плохой кадр за синхронизированный вывод нельзя — на conhost,
  где проблема и живёт, режим 2026 игнорируется. Поэтому определяем **до** отрисовки:
  `take_full_redraw()` в петле видит флаг уже взведённым, плохой кадр не рисуется вовсе.
  Проводка по факту рендера (`MessageFeed.content_changed`) и кадр-«догон» в петле
  удалены.
- **Флаг риска кэшируется** (`ChatScreen.feed_has_risky`) и только накапливается:
  правки всегда затрагивают последний блок, поэтому `mark_feed_changed` проверяет его,
  а `activate_chat` (полная замена ленты) пересчитывает с нуля — иначе скан всей ленты
  на каждый чанк стрима был бы O(весь текст чата). Пере-оценка в большую сторону
  безопасна: лишняя перерисовка не видна, пропущенная оставляет артефакт.
- **Риск подхода закрыт гейтом** `every_feed_mutator_marks_content_change`: прогоняет
  **все** мутаторы ленты (+`Ctrl+T`) и требует запроса перерисовки — забытый вызов
  вернул бы мелькание. **Мутационно проверен** (снятие вызова в `push_chunk` роняет
  тест с именем мутатора в сообщении).
- **Детектор расширен** с VS16 на группу риска (`feed::is_risky_glyph`): VS16, **ZWJ**
  (`👨‍👩‍👧` — на скриншотах именно они дают худшую кашу), модификаторы тона кожи,
  пиктограммы supplementary (`😀`), BMP-эмодзи ширины 2 (`✅`/`⭐`). **CJK намеренно
  не входит** — иероглифы терминалы рисуют согласованно, гонять из-за них полную
  перерисовку на каждый чанк незачем. Цена: при стриме текста с эмодзи каждый кадр
  становится полной перерисовкой (на чистом тексте — как раньше).
- **Не закрыто: «фон инвертируется в половину символа»** (третий симптом отчёта).
  Механика иная и **лежит в апстриме**: `ratatui` сбрасывает хвостовую ячейку широкого
  глифа в **дефолтный стиль** (проверено пробой: у ячейки глифа `bg=Red`, у хвоста
  `bg=Reset`) и в diff её не шлёт, а conhost атрибут второй половины сам не выставляет
  — половина глифа остаётся с прежним фоном. Полная перерисовка это не лечит (ячейка
  просто не эмитится). Чинится либо в `ratatui` (наследовать стиль глифа хвостовой
  ячейкой), либо в `ratatui-crossterm` (вести `last_pos` с учётом ширины глифа — тогда
  эмиссия хвоста перестанет сдвигать ряд и станет безопасной). Кандидат в апстрим-PR,
  прецедент есть (mermaid-text #29/#30).
- **Следствие**: замена `❤️`/`✌️` → `💖`/`🤞` в попапе теперь **не строго необходима**
  (сдвиг лечится сентинелом), но оставлена как защита в глубину — VS16-кластеры в
  сетке фиксированной ширины остаются хрупким классом, и гейт списка это фиксирует.

### Пост-M9: обновление ratatui-core/crossterm 0.1.1 → 0.1.2 (сделано)
- **Обновление зависимости с ревизией обходных путей** предыдущего PR (#189, артефакты
  эмодзи на conhost); ветка `chore/ratatui-0.1.2`. Развилки подтверждены пользователем
  2026-07-20 (сузить перерисовку до закрытия; замену VS16-эмодзи оставить).
- **Объём релиза 0.1.2 — ровно одно функциональное изменение** (сверено по исходникам
  крейтов, не по нотам): `ratatui-core/src/buffer/diff.rs`; `backend.rs` — доки
  (упоминания Termina), а **`ratatui-crossterm` байт-в-байт идентичен 0.1.1**. Отсюда
  узкая поверхность регресса.
- **Что именно чинит 0.1.2** (ratatui#2585, восстановлен `invalidated` в diff): хвостовая
  ячейка широкого глифа шлётся **при условии** `previous_width > cell_width` **И**
  «стиль заметен на пустой ячейке» (`bg != Reset` либо `REVERSED`/`UNDERLINED`/
  `SLOW_BLINK`/`RAPID_BLINK`/`CROSSED_OUT`). То есть закрыт ровно **наш** наблюдавшийся
  дефект — след подложки выделения при **закрытии** попапа.
- **Что НЕ починено** (проверено пробами на `Buffer::diff`, а не предположением):
  1. **Нестилизованные хвосты** — в сетке попапа эмодзи 43 из 44 глифов уходят без
     эмиссии хвоста (`styled emitted=1 missed=0 | unstyled emitted=0 missed=43`).
  2. **Сдвиг выделения** — глиф остаётся широким, условие `previous_width > cell_width`
     не выполняется, хвост не шлётся (подтверждено и на попапе эмодзи, и на
     `➕ добавить в словарь`: `ITALIC|REVERSED` → `tail_close=true tail_move=false`).
  3. **Бэкенд `last_pos` без учёта ширины глифа** (ratatui#2651, сдвиг ряда) — источник
     не менялся вовсе.
- **Ключевая находка, изменившая план** (задача предполагала, что перерисовку в попапах
  можно снять целиком): для **сдвига выделения** полная перерисовка провабельно **ничего
  не даёт** — сентинел обязан пропускать хвост широкого глифа (иначе бэкенд напечатает
  половину без `MoveTo` и сдвинет ряд, п.3), так что там она была холостой с самого
  начала; подложку снимает перепечатка самого глифа, приходящая обычным diff'ом.
  Поэтому запрос перерисовки **сужен до закрытия** попапов (эмодзи и орфографии), а не
  снят и не оставлен на любое действие. Остаётся он ради п.1 (нестилизованные хвосты),
  который апстрим не покрывает.
- **Канарейка переписана под новую границу** (старая `wide_glyph_trailing_cell_is_not_
  repainted_by_plain_diff` упала — by design, она и была заложена на «апстрим починил
  сам»): `upstream_repaints_styled_tail_on_close_but_not_on_selection_move` пинит **обе**
  стороны — (A) апстрим шлёт стилизованный хвост при закрытии (упадёт при регрессе
  апстрима), (B) при сдвиге не шлёт (упадёт, когда апстрим закроет и этот случай → тогда
  обходной путь снимать), плюс что сентинел достаёт хвост при закрытии и **не** трогает
  его при сдвиге.
- **Оставлено без изменений** (обоснованно): `prime_full_redraw` и его устройство
  «пробел + `HIDDEN`» — инвариант «не писать во вторую половину широкого глифа» держится
  на неисправленном #2651 и стал **важнее**, не наоборот; `mark_feed_changed` +
  `is_risky_glyph` — текст ленты нестилизован (`bg = Reset`), фикс 0.1.2 его не касается;
  замена VS16-эмодзи (`💖`/`🤞`) и гейт `emoji_list_is_width2_without_vs16` — защита в
  глубину, пока #2651 открыт.
- Тесты попапов переписаны под суженный контракт и **мутационно проверены в обе стороны**
  (возврат перерисовки на сдвиг → падает ассерт сдвига; снятие с закрытия → падает ассерт
  закрытия). **1177 юнит-тестов зелёные** (число неизменно), 53 `#[ignore]`, clippy
  `-D warnings`/fmt чисты. Доки: architecture.md §4 (граница с апстримом + номера issue),
  CHANGELOG (формулировки `[Unreleased]` приведены к фактической механике — упоминание
  «перемещения по сетке» снято).
- **Живой прогон движка не требуется** (рендеринг, без движка/памяти). **Ручной регресс
  на conhost прогнан пользователем**: попапы (эмодзи/орфография), mermaid и стрим ленты
  — чисто, сужение перерисовки регрессий не дало. Найден отдельный дефект того же
  класса (ниже).

### Пост-M9: полная перерисовка при смене экрана и в поле ввода с VS16 (сделано)
- **Симптом** (ручной регресс на conhost после обновления ratatui): в ленте с `❤️`
  переход на список чатов / `F3` и **возврат** добавлял лишний пробел после эмодзи;
  пропадал по прокрутке. Та же ветка `chore/ratatui-0.1.2`.
- **Причина — VS16-ветка diff'а + неисправленный ratatui#2651** (воспроизведено пробой
  на `Buffer::diff`, все три наблюдения совпали с отчётом): для VS16-кластера ratatui
  шлёт хвостовую ячейку, **когда её символ изменился**. Внутри одного экрана правки
  ленты хвост не трогают (`same_screen=false` — потому баг и не был виден при обычной
  работе), а на **возврате с другого экрана** там стоял чужой символ (`switch=true`) →
  хвост уходит в терминал, бэкенд печатает его без `MoveTo` (позиция ведётся по номеру
  ячейки, без учёта ширины глифа) → остаток ряда едет вправо. Прокрутка «чинила»,
  потому что заказывает полную перерисовку, а сентинел хвост **не** шлёт
  (`sentinel=false`) — ровно то свойство, ради которого он устроен как «пробел +
  `HIDDEN`».
- **Фикс — полная перерисовка на СМЕНУ ЭКРАНА** (`app/runtime/mod.rs`), централизованно:
  `prime_full_redraw` поднят из арма `ActiveScreen::Chat` перед `match`, условие —
  `requested || switched`, где `switched` = смена `std::mem::discriminant(&active)`
  **по факту отрисовки** (переключение «туда и обратно» между кадрами визуально ничего
  не меняет). Правка в одном месте вместо 8+ сайтов `*active = ActiveScreen::…`;
  покрывает любое направление и любой экран (VS16 бывает и в заголовке чата в списке,
  и в нарративе `F3`). Флаг чата берётся только когда активен чат (иначе потерялся бы).
- **Тот же класс найден и закрыт в поле ввода** (пробой, пользователем не сообщался):
  правка **левее** `❤️`, за которым сразу непробельный символ, двигает глиф на место
  чужого → хвост эмитится → ряд едет. `mark_input_changed` теперь заказывает полную
  перерисовку, если в поле есть глиф группы риска; проверка потоковая — новый
  `InputBox::any_char(pred)` (предикат передаёт вызывающий, виджет не знает про верхние
  слои — FSD), без сборки `text()` на каждую правку. На обычном тексте — no-op.
- **Тесты**: `shared::ui::screen_switch_emits_vs16_tail_without_full_redraw` пинит
  механику (смена экрана хвост шлёт; правка внутри экрана — нет; сентинел — нет) и
  объясняет, почему смена экрана обязана идти полной перерисовкой;
  `input_with_risky_glyph_requests_full_redraw` — поведение поля (мутационно проверен).
  **1179 юнит-тестов зелёные** (+2), 53 `#[ignore]`, clippy `-D warnings`/fmt чисты.
- **Осталось за апстримом**: половина фона у широких глифов (хвостовая ячейка несёт
  дефолтный стиль, conhost её атрибут не выставляет) — локально не чинится, ячейка
  помечена `skip` и в diff не попадает. Тот же корень, что ratatui#2651; чинится либо
  наследованием стиля глифа хвостовой ячейкой, либо ведением `last_pos` с учётом ширины
  (тогда эмиссия хвоста перестанет сдвигать ряд и станет безопасной).

### Пост-M9: попап орфографии — выделение как в остальных списках (сделано)
- **Симптом** (ветка `fix/suggest-popup-selection-style`): попап подсказок
  орфографии (`Ctrl+G`) выделял пункт **инверсией всей строки**
  (`highlight_style(Style::new().reversed())`) и без рейла, тогда как все прочие
  списки приложения давно устоялись на другом стиле: **мягкая подложка**
  `bg(palette.keycap_bg)` + **зелёный рейл `▌ `** (`palette.success`) у выбранной
  строки — список чатов (`widgets/chat_list.rs`, `item_line`), меню секций и поля
  экрана настроек (`screens/settings/render.rs` + `render_field_line`), экран
  «модели себя» (`screens/self_model.rs`). Попап выбивался из ряда.
- **Почему именно так, а не реверс** (обоснование уже было записано у меню секций
  настроек и перенесено в новый код): `reversed()` свапает `fg↔bg` **у каждого
  спана по отдельности** — рейл `▌` (левый полублок) от этого расползается на
  ~1.5 колонки, а разные спаны строки получают разный фон. Единая подложка
  `keycap_bg` + рейл поверх неё читаются чисто.
- **Правка** (`screens/chat/popups.rs::render_suggest`): элементы строятся через
  `.enumerate()`, у выбранного — префикс-рейл `▌ ` цветом `success`, у прочих —
  отступ той же ширины (текст не «прыгает» при перемещении выделения);
  `highlight_style` → `Style::new().bg(palette.keycap_bg)`. Индекс выделения
  (`popup.selected.min(len-1)`) считается один раз и переиспользуется
  `ListState`. `▌` — WGL4, компат-режим conhost отдельной замены не требует (как
  рейлы ролей ленты и `█` скроллбара).
- **Попутно уточнён комментарий** в `handle_suggest_key`: страховка полной
  перерисовкой при закрытии попапа объяснялась тем, что выделенная строка несёт
  `REVERSED`. Подложка `keycap_bg` для ratatui 0.1.2 — такой же «заметный на
  пустой ячейке стиль» (`bg != Reset`, см. architecture §4), поэтому механика не
  изменилась, но формулировка приведена к факту.
- **Тесты**: `suggest_popup_selection_matches_other_lists` (рендер в `TestBackend`:
  рейл на месте и зелёный, подложка — `keycap_bg`, ни одна ячейка выделенной
  строки не инвертирована). **Мутационно проверен** — возврат `.reversed()`
  роняет его. **1180 юнит-тестов зелёные** (+1), 53 `#[ignore]`, clippy
  `-D warnings`/fmt чисты. **Живой прогон не требуется** (чистый UI без движка и
  памяти, покрыт `TestBackend`).

### Пост-M9: API-ключи в настройках — этап 1 (ядро: машинно-привязанное хранение) (сделано)
- **Новое направление** (запрос пользователя): облачные ключи (OpenAI/Gemini/Claude)
  должны вводиться **в окне настроек**, а не через env-переменные («обычные
  пользователи слабо понимают, что такое переменные окружения»), и храниться в
  конфиге **безопасно** — зашифрованными **машинным ключом**, с сохранением
  переносимости конфига: сценарий A/B/A (ввёл на A → перенёс конфиг на B → там
  ввожу заново → вернулся на A → ключи читаются). Исследование
  [docs/research/api-key-storage.md](docs/research/api-key-storage.md); развилки
  Р1–Р6 **приняты пользователем по рекомендациям 2026-07-20**. Ветка
  `feat/api-key-store` (стек на `docs/api-keys-research`). Этап 1 — **ядро**
  (UI-поле ввода — этап 2), поэтому пользовательского эффекта пока нет.
- **Формат — список записей, по записи на машину** (`AppConfig.api_keys:
  Vec<ApiKeyEntry>`; `#[serde(default)]` + `skip_serializing_if` → additive, **без
  миграции и bump'а схемы**, политика ADR 0006 Ф12). Запись несёт `label` (имя ПК +
  дата, только для человека), `scheme`, `check` и `keys: provider → шифротекст`.
  **Опознание «нашей» записи — расшифровкой пробы `check`**, а не хранением
  machine-id: идентификатор машины не светится в переносимом конфиге, а
  Windows-пути он и не нужен. Чужие записи не трогаются (оживут на своих машинах);
  запись с **незнакомой схемой** читается, сохраняется и просто считается чужой —
  формат расширяем (задел: OS-кейчейн как ещё одна `scheme`).
- **Две схемы** (`shared/secrets.rs`, новый модуль): **`dpapi`** (Windows) —
  системный `CryptProtectData`/`CryptUnprotectData` (ключ *пользователя*, которым
  управляет ОС; расшифровка другим пользователем/на другой машине невозможна;
  `pOptionalEntropy` — константа приложения). **`machine-key-v1`** (Linux) —
  HKDF-SHA256 над `/etc/machine-id` (паттерн `sd_id128_get_machine_app_specific` —
  systemd прямо предписывает не брать machine-id сырым; фолбэк
  `/var/lib/dbus/machine-id`) + ChaCha20-Poly1305 (AEAD, случайный nonce префиксом),
  имя пользователя в `info` → привязка per-user, как у DPAPI. Нет machine-id →
  схема недоступна, остаётся env-путь. Шифротекст кодируется **hex** (свой кодек —
  прецедент `sandbox_setup::hex_lower`; base64-зависимость не нужна, разница в длине
  для конфига несущественна).
- **Резолюция «сохранённый → env»** (развилка Р3): `resolve_api_key(stored,
  api_key_env)` (`app/supervisor.rs`) — сохранённый ключ приоритетнее (введён явным
  действием, целевой пользователь env не видит), env остаётся фолбэком (CI, power
  users, системы без machine-id). Расшифровка живёт в `EngineManager`
  (`stored_key(api_keys, provider)`), а супервайзер принимает уже расшифрованный
  `stored_key: Option<&str>` — **слой запуска серверов не знает про формат хранения
  секретов**, и его тесты не требуют шифрования (отклонение от дизайн-дока,
  зафиксировано там же). Ключи **провайдеро-центричны**: один ключ OpenAI
  обслуживает чат + имперсонацию + эмбеддинги (снимает прежнее «указать env-имя
  трижды»); external-прокси остаётся env-only (Р4 — произвольный URL к провайдеру
  не привязать).
- **Команда `AppCommand::SetApiKey { provider, key }`** → `handle_set_api_key`
  (`orchestrator/settings.rs`): шифрует, кладёт в запись этой машины, персистит
  (`save_config`, откат при сбое), помечает на отложенный (пере)подъём **только те**
  слоты, чей активный провайдер сменил ключ (тот же дебаунс `RestartQueue`, что у
  правок движка). Пустой ключ = удаление; опустевшая запись убирается. Плейнтекст
  живёт только в аргументе и в HTTP-клиенте.
- **Round-trip защита**: `handle_update_config` восстанавливает `api_keys` из
  прежнего конфига — снимок из UI их не несёт вовсе, иначе правка любой настройки
  стёрла бы ключи (прецеденты `last_active_chat`, наследование TOFU-пинов MCP).
- **i18n**: `ui.err.server.no_api_key_env` → `ui.err.server.no_api_key` («введите
  ключ в настройках или укажите переменную окружения»); новый
  `ui.err.api_key_save_failed` (ru+en; i18n-гейты parity/no-dead накрыли автоматически).
- **Зависимости**: `chacha20poly1305` + `hkdf` (RustCrypto, чистый Rust, лицензии в
  allowlist `deny.toml`); `windows-sys` получил фичу `Win32_Security_Cryptography`
  (крейт уже был — Job Object песочницы). Новых C-зависимостей нет.
- **Модель угроз зафиксирована честно** (док §3, дока модуля): защищаем **файл** —
  перенос/копию/бэкап/синхронизацию конфига (вне своей машины это бесполезный
  шифротекст), а на Windows ещё и от других пользователей машины. От кода,
  исполняющегося под тем же пользователем, не защищает — он вызовет тот же DPAPI;
  это фундаментально для любой схемы «приложение расшифровывает само» (так же
  устроены Chrome, Git Credential Manager). Прежний env-путь не безопаснее.
- **Тесты**: `secrets` (7 — hex round-trip и отказ на мусоре; AEAD round-trip;
  **отказ чужому ключу/другому пользователю/при порче**; случайность nonce;
  детерминизм и доменная развязка `derive_key`; полный цикл записи на **живой
  платформенной схеме** — на этой машине это реальный DPAPI; чужая запись и запись
  незнакомой схемы игнорируются и **не затираются**); супервайзер (2 — приоритет
  сохранённого над env и работа **без** `api_key_env`; облачный режим `Ready` на
  одном сохранённом ключе); оркестратор (3 — ключ персистится **шифротекстом**
  (ассерт «плейнтекста нет в `settings.json`») и читается обратно; правка настроек
  не стирает ключи; пустой ключ удаляет запись). **1192 юнит-теста зелёные** (+12),
  53 `#[ignore]`, clippy `-D warnings`/fmt чисты.
- **Живой прогон движка не требуется** (протоколы клиентов не тронуты — тот же ключ
  уходит в тот же заголовок; движок/память не затрагивались). DPAPI-путь при этом
  проверен **реально** — юнит-тест полного цикла прогнан на живой Windows-машине.
  Linux-путь: чистое ядро схемы (`derive_key`/`encrypt_with_key`/`decrypt_with_key`)
  покрыто тестами и прогнано, платформенный источник ikm (`/etc/machine-id`)
  проверит CI-джоб `ubuntu` (Windows-хост даёт только свой таргет).
- **Дальше — этап 2 (`feat/api-key-ui`)**: маскированный режим `InputBox`, поле
  «API-ключ» со статусом «настроен (этот компьютер)» в облачных подсекциях, флаги
  «настроен» в `AppEvent::Settings`, `SettingsIntent::SetApiKey`, README/spec/
  install.md/CHANGELOG + **ADR** по итогам направления.

### Пост-M9: API-ключи в настройках — этап 2 (UI: поле ввода, маска, статус) (сделано)
- Завершение направления (ветка `feat/api-key-ui`, стек на `feat/api-key-store`).
  Ключи облаков вводятся **в окне настроек** — env-переменные больше не нужны.
  Итог направления зафиксирован **ADR 0008** (уточняет ADR 0004: «секреты не на
  диск» → «не на диск **открытым текстом**»).
- **Маскированный режим `InputBox`** (`set_mask`, по образцу `single_line`): символы
  рисуются как `•`, `selected_text()` возвращает `None` (секрет не утекает
  копированием — удаление выделения при этом работает, оно идёт через
  `remove_selection`), включение переводит поле в однострочный режим (иначе
  многострочный путь отрисовки показал бы содержимое). **Маска подменяет символы до
  расчётов ширины**: у `•` ширина 1, поэтому широкий глиф в секрете (эмодзи из
  буфера) не сдвигает курсор относительно видимого текста.
- **Поле «API-ключ»** в облачных подсекциях Ассистент/Имперсонация/Эмбеддинги
  (`FieldId::XApiKey`/`IxApiKey`/`EApiKey`, строка `api_key_row`): значение —
  **статус** («настроен (этот компьютер)» / «не задан» / «недоступно на этой
  системе», если нет machine-id), не секрет. `Enter` открывает **пустой**
  маскированный редактор (сохранённый ключ показать нельзя — его нет даже у
  экрана), `Del` удаляет. Коммит уходит **не** в рабочую копию конфига, а
  намерением `SettingsIntent::SetApiKey` → `AppCommand::SetApiKey` (ветка в
  `apply_text` до таблицы `field_spec`; `reset_field` — своя ветка, т.к. сравнение
  с дефолтом не годится: значением служит статус). Провайдер поля берётся из режима
  **своего** движка (`api_key_field_provider`) — ключ общий для всех трёх слотов.
- **Секреты не уезжают в UI вовсе** (усиление против первоначального дизайна):
  `emit_settings` **очищает** `config.api_keys` в снимке и добавляет флаги
  `api_keys_present: Vec<CloudProvider>`; экран хранит только их
  (`set_api_keys_present`). Так UI не таскает даже шифротексты, а round-trip защита
  этапа 1 (`handle_update_config` восстанавливает ключи из прежнего конфига)
  становится не «страховкой», а обязательным звеном.
- **i18n**: поле/статусы/описания (`ui.settings.field.api_key`,
  `ui.settings.value.key_set|key_unset|key_unsupported`,
  `ui.settings.desc.api_key|api_key_unsupported`) в ru+en; гейты parity/no-dead
  накрыли автоматически.
- **Тесты**: `input_box` (2 — маска: на экране только `•`, секрета в буфере нет,
  `selected_text` пуст, удаление работает; ширина/курсор при широком глифе);
  `settings` (4 — поле-статус только в облачных режимах и реагирует на флаг
  присутствия; редактор открывается **пустым и маскированным**, коммит даёт
  `SetApiKey`, а секрет **не оседает в конфиге экрана**; `Del` удаляет только когда
  ключ есть; поле каждого слота адресует провайдера своего движка); оркестратор
  (1 — снимок несёт флаги, но `config.api_keys` пуст). Тесты этапа 1 переведены с
  `config.api_keys` на `api_keys_present`. **1199 юнит-тестов зелёные** (+7), 53
  `#[ignore]`, clippy `-D warnings`/fmt чисты.
- **Живой прогон движка не требуется** (чистый UI + путь ключа: протоколы клиентов
  не тронуты). DPAPI-цикл проверен реально ещё на этапе 1; Linux-схему проверяет CI.
  Полный ручной сценарий (ввод ключа → облачный чат → перенос конфига) остаётся за
  пользователем — он требует настоящего терминала и живого ключа.
- **Задел** (ADR 0008): OS-кейчейн как дополнительная `scheme`; ключи для
  external-прокси и env-карт MCP-серверов тем же механизмом; UI-управление записями
  чужих машин («забыть компьютер»).

### Пост-M9: озвучивание сообщений чата (TTS) — OpenAI/Gemini/external (сделано)
- **Направление восстановлено из отревертнутого этапа 1** (`git show 19ead9f` —
  экстрактор `pulldown-cmark`, команда `/tts`, плеер `rodio`, точки остановки,
  вкладка настроек, клиенты) после **пере-исследования живым спайком**
  ([docs/research/tts.md §13](docs/research/tts.md)): реализованные ранее движки не
  давали приемлемого качества на реальном тексте, направление откатили (PR #197–199),
  затем проверили кандидатов на живом железе. **РЕШЕНИЕ (2026-07-23): основной движок
  — OpenAI TTS** (`gpt-4o-mini-tts`, голос по умолчанию **`onyx`** — проверен спайком:
  ударения ~1 ошибка на абзац, лучше всех локальных; англицизмы нативно; дрейфа нет).
  Ключевой продуктовый довод: **не новый vendor** — ключ OpenAI уже хранится
  (ADR 0008), новой регистрации не нужно (ElevenLabs/Azure её потребовали бы). **Объём
  этапа — OpenAI + Gemini + external** (три режима `TtsMode`); **локального сайдкара
  НЕТ**. Развилки Р1–Р9 (§10) валидны по поведению; пересмотрен лишь выбор движка.
- **Локальный managed-сайдкар — задел, не «этап 2»**: спайк показал, что локальные
  движки либо NO-GO по русскому (Qwen3-TTS, Supertonic 3 — неверные ударения, акцент/
  дрейф), либо требуют своего Rust-фронтенда (vosk-tts — GO с оговорками: 22 кГц, свой
  русский фронтенд для поставки без Python). Для offline/не-OpenAI аудитории —
  режим `external` (любой OpenAI-совместимый TTS-сервер: Kokoro-FastAPI/speaches/
  LocalAI). Managed-режим в селектор не добавлен (прецедент `Claude` до
  `AnthropicClient` — нерабочий пункт не шипим).
- **Конфиг `TtsSettings`** (`shared/config.rs`, поле `AppConfig.tts`), всё
  `#[serde(default)]` — без миграций: `mode: TtsMode {OpenAi(деф.)/Gemini/External}`;
  `openai`/`gemini: TtsCloudSettings` (model/voice/instructions/api_key_env/url);
  `external: TtsExternalSettings` (url/model_name/voice/api_key_env); `speed: f32=1.0`;
  `speak_roles=false`; `stop_on_chat_switch=true`; `stop_on_generation_start=false`.
  Дефолты: OpenAI `gpt-4o-mini-tts`/`onyx`, Gemini `gemini-2.5-flash-preview-tts`/`Kore`.
  Ключ облака **общий с чатом** (ADR 0008), фолбэк на env-переменную сохранён. **`speed`
  у `gpt-4o-mini-tts` де-факто игнорируется** (известный дефект) → скорость просят
  словами в `instructions`.
- **Клиенты** (`shared/tts/`): `openai.rs` — `POST {base}/audio/speech`, обслуживает
  **и** облако OpenAI (формат `pcm`, s16le 24 кГц, мимо декодера), **и** сторонний
  сервер (формат `wav`, самый переносимый); поле `instructions` только у облака;
  незаданные поля не шлём (`skip_serializing_if`). `gemini.rs` — нативный
  `generateContent` c `responseModalities:["AUDIO"]` (тот же транспорт, что у
  ...
  Известное у Gemini: 503-перегрузка и дрейф голоса между чанками (потому опция, не
  дефолт). Лимит входа: OpenAI 4096, Gemini/External 2000 символов.
- **Речевой экстрактор** (`shared/markdown/speak.rs`, `speakable_text(markdown, loc)`)
  — второй walker по событиям `pulldown-cmark` (ADR 0003), `Writer` не тронут: блоки
  кода, ` ```mermaid `, таблицы, display-math ($$…$$) **пропускаются с голосовой
  пометкой** на языке профиля (ось A); inline-код читается текстом, inline-math →
  `latex_to_unicode`, ссылки → текст (URL опущен). «Мысли» (CoT) и tool-блоки не
  озвучиваются (их нет в `Message.text`). Пометки/префиксы ролей — на **языке профиля**
  (ось A, речевой контент, а не UI-хром).
- **Плеер** (`shared/tts/playback.rs`, `rodio`): очередь `Player::append` даёт конвейер
  «синтез N+1 во время проигрывания N» бесплатно; headless/CI (`open_default_sink` →
  `Result`) — мягкая деградация «звук недоступен», не паника (паттерн
  `UnavailableEmbedder`, тест `open_degrades_gracefully_without_audio_device`).
- **Оркестрация** (`app/orchestrator/tts.rs`, зеркало `rag.rs`): снимок переписки
  берёт оркестратор (владелец `Chat`) → отбор по объёму (`build_utterances`, снимок на
  момент команды — работает и во время генерации, гейтов генерации нет) → чанкинг
  (`chunk_utterances`/`pack_sentences`: предложения `rag::split_sentences` пакуются в
  чанк до лимита провайдера, короткое сообщение → **один запрос** — нет «выросших пауз»
  из спайка; пакует по речевым блокам) → фоновая отменяемая задача (`tts_cancel:
  CancellationToken`, новая команда/`Quit` отменяют). **Точки остановки**: по настройке
  — смена чата (`stop_on_chat_switch`) и начало генерации (`stop_on_generation_start`);
  **безусловно** — удаление обмена (`Ctrl+E`), перегенерация (`Ctrl+R`), удаление чата
  (озвучиваемого текста больше нет). Контракт: `AppCommand::Tts(TtsScope)`/`TtsStop`,
  `AppEvent::TtsActive(bool)`.
- **UI**: команда `/tts`/`/tts N`/`/tts all`/`/tts stop` (парсер
  `features/tts_command.rs`, регистронезависимо, `TtsScope {Last/Recent(n)/All}`;
  ошибка несёт **сам аргумент**, текст подсказки строит UI локализованно), подсветка
  команды в поле ввода, оверлей помощи, тихий чип **«♪ озвучка»** (`ui.status.speaking`,
  нота U+266A в WGL4 → в компат-режиме не заменяется), вкладка **«Озвучивание»** в
  секции «Модель» настроек: группа «Движок» (mode-driven поля движок/голос/скорость/
  инструкции; ключ через общее поле-статус провайдера ADR 0008) + группа «Поведение»
  (тумблеры `speak_roles`/`stop_on_chat_switch`/`stop_on_generation_start`).
- **Зависимости**: `rodio` (+`cpal`/`symphonia`, MPL-2.0 уже в allowlist `deny.toml`)
  и `base64`. Новая **системная** зависимость — ALSA на Linux: сборка `libasound2-dev`
  (apt-шаги `ci.yml`/`release.yml`/`packaging.yml`), рантайм `libasound2t64 | libasound2`
  (deb; ловушка time_t64 — виртуальная зависимость иначе тянула OSS-шим без символов) /
  `alsa-lib` (rpm/arch, в Arch-смоуке ставится явно — `pacman -U` зависимости не тянет).
- **Тесты**: парсер команды, речевой экстрактор (golden на характерном ответе LLM +
  пер-локальный гейт), клиенты (wire-формы, разбор Gemini, частота из mimeType,
  блокировки), конвертация PCM→f32 + мягкая деградация плеера, отбор сообщений/префиксы/
  чанкинг, оркестрация (отказы при ненастроенном движке, точки остановки, идемпотентность
  `/tts stop`, гонка позднего `done`), настройки. **1258 юнит-тестов зелёные**,
  clippy `-D warnings`/fmt/`cargo deny` чисты (проверено при восстановлении). Живые
  `#[ignore]`-смоуки (синтез OpenAI/Gemini, сторонний сервер, воспроизведение тона, e2e
  через оркестратор) — из проверенного этапа 1 (проходили до реверта). **В этой сессии
  перепрогнан живой смоук `openai_synthesizes_russian_speech_live`** против реального
  OpenAI API — GO (300 КБ PCM). Качество OpenAI `onyx` подтверждено спайком (§13.9,
  прямые вызовы API). Интерактивный TUI-прогон `/tts` в этой сессии **не выполнялся**
  (нужен настоящий терминал) — команда/оркестрация покрыты юнит-тестами.
- **Пауза/продолжение `/tts pause`/`/tts resume`** (по просьбе пользователя после
  живого прогона — для длинного текста): `rodio::Player::pause`/`play`. Ключевой
  сдвиг — устройство теперь открывает **обработчик** (`handle_tts`, всё так же лениво
  «по команде»), оборачивает в `Arc<Playback>` (`Send+Sync`) и **разделяет с фоновой
  задачей**; оркестратор держит хэндл (`tts_playback: Option<Arc<Playback>>`) → pause/
  resume применяются **мгновенно**, не дожидаясь опроса. Петли задачи **не менялись**:
  на паузе очередь не дренится, поэтому конвейер сам придерживает синтез, а `done` не
  уходит (`is_drained=false`), пока не возобновят. `stop_tts`/`handle_tts_done` чистят
  хэндл (дроп закрывает устройство); `stop_tts` снимает паузу (иначе отменённая задача
  не дренилась бы). Открытие `Playback::open` переехало в обработчик: нет звука → нет
  задачи (раньше задача стартовала и падала). Контракт: `TtsCommand::Pause/Resume` →
  `ChatIntent`/`AppCommand::TtsPause/Resume` → `handle_tts_pause/resume` (no-op без
  активной озвучки). i18n: ключ `ui.help.tts_pause`, обновлён `ui.tts.bad_arg`. Тесты:
  парсер (pause/resume + регистр), no-op без устройства (звук в CI недоступен → путь
  `tts_playback=None`). **1255 юнит-тестов**, clippy/fmt/i18n-гейты чисты.
- **Разные голоса пользователя/ассистента при `/tts all`** (по просьбе пользователя
  после живого прогона): новое поле `voice.user_voice` в `TtsCloudSettings`/
  `TtsExternalSettings` (`#[serde(default)]` → без миграции). Когда задан отдельный
  «Голос пользователя» активного режима и он **отличается** от голоса ассистента,
  строится **второй движок** (`engines_from_config → (ассистент, Option<пользователь>)`,
  тип-алиас `TtsEnginePair`; переиспользует резолвинг модели/ключа/базы через
  `build_engine(voice_override)`). Пайплайн теперь **несёт роль**: `build_utterances`/
  `chunk_utterances` работают с `Vec<(MessageRole, String)>`, задача выбирает движок по
  роли чанка (`user_engine.unwrap_or(engine)` для реплик пользователя). Голос — свойство
  движка при создании (не запроса), поэтому два инстанса, а не параметр `synthesize` (по
  минимуму правок трейта/клиентов). `user_voice` **независим от `speak_roles`** (голоса
  различают акустически, префиксы — вербально); при `None` всё одним голосом (прежнее
  поведение). Конфиг-хелпер `TtsSettings::active_voices() -> (ассистент, пользователь)`
  читает поля активного режима, пустое = `None`. UI: поле «Голос пользователя»
  (`FieldId::TtsUserVoice`) на вкладке «Озвучивание» (обе ветки external/облако) +
  i18n `ui.settings.{field,desc}.tts_user_voice`. `engine_from_config` удалён (осиротел).
  Тесты: `active_voices` (режим/пустое), `engines_from_config` (второй движок только при
  заданном и отличном голосе), `chunk_inherits_role`, обновлены тесты пайплайна под
  кортежи. **1258 юнит-тестов**, clippy/fmt/i18n чисты. Живой прогон многоголосья — за
  пользователем (нужен терминал + аудио).
- **Задел** (roadmap): локальный сайдкар (vosk-tts + свой русский фронтенд, `mindfork
  tts setup` по образцу ADR 0005) + ADR по итогам; склейка чанков через границы речевых
  блоков (ещё более гладкая многоабзацная озвучка), SSE-стриминг внутри чанка, кэш
  аудио, авто-выбор голоса по языку сообщения, чтение таблиц построчно, озвучка
  выбранного сообщения из ленты, мульти-спикер (Gemini), ОС-TTS.

### Пост-M9: диалог справки/«О программе» с вкладками (F1) (сделано)
- **Оверлей помощи (`F1`/`?`) переработан из одного длинного скролл-списка в модальный
  диалог в стиле KDE/Qt** (ветка `feat/help-about-tabs`): логотип-лockup в шапке (тот же
  `widgets/logo::lockup_lines`, что и раньше), **таб-стрип** из четырёх вкладок и
  прокручиваемое содержимое активной вкладки со скроллбаром. Вкладки (в порядке показа):
  **«О программе»** (имя+описание, автор `Vladimir Shylov`, версия, ссылки — сайт
  `mindfork.io`, репозиторий, крейт `crates.io/crates/mindfork`), **«Горячие клавиши»**
  (прежний список `HELP_KEYS`), **«Лицензия»** (текст MIT), **«Компоненты»** (сторонние
  зависимости + их лицензии). Открывается на «Горячих клавишах» (`F1`/`?` — привычная
  клавиша справки; прежнее поведение сохранено, «О программе» — соседняя вкладка).
- **Новый модуль `shared/credits.rs`** (FSD `shared`, язык-нейтральные данные): константы
  `AUTHOR`/`SITE_URL`/`REPO_URL`/`CRATE_URL`, `LICENSE_TEXT` (`include_str!("../../LICENSE")`),
  `COMPONENTS: &[(&str,&str)]` (44 прямые зависимости рантайма + windows-sys, лицензии
  выверены по `cargo metadata`; dev/build-зависимости `tempfile`/`winresource` исключены).
  **Гейт-тест** `components_cover_direct_dependencies` сверяет `COMPONENTS` с
  `[dependencies]`+`[target.'cfg(windows)'.dependencies]` `Cargo.toml` (построчный парсер,
  как SVG-парсер в `widgets/logo.rs`), плюс тесты «отсортирован/лицензирован» и
  «LICENSE встроен». Данные лицензии/компонентов **не проходят через локали** — их берёт
  `screens/chat/popups.rs` напрямую (юр. текст/SPDX/URL язык-нейтральны).
- **Состояние** (`screens/chat/mod.rs`): `show_help: bool` + `help_scroll: usize` заменены
  на `help: Option<HelpState>` + типы `HelpTab {About,Hotkeys,License,Components}` (порядок
  = `ALL`) и `HelpState {tab, scroll}` (`next_tab`/`prev_tab` по кругу со сбросом
  прокрутки). Навигация (`screens/chat/input.rs`): `Tab`/`←→` — вкладки, `↑↓`/`PgUp`/`PgDn`/
  `Home` — прокрутка, `Esc`/повторный `F1`/`?` — закрыть, `Ctrl+Q`/`F10` — выход (пробивают
  диалог, как в попапе подтверждения). Прочие клавиши **игнорируются** (раньше «любая
  клавиша закрывала» — с вкладками это мешало бы навигации).
- **Рендер** (`screens/chat/popups.rs`, `render_help(frame, &mut HelpState, palette, loc)`):
  фикс-размер диалога (76×34 + рамка, кламп к экрану — окно не «прыгает» между вкладками),
  шапка = (опц. лockup + отбивка) + таб-стрип + разделительная линия `─`, ниже —
  прокручиваемое содержимое (`Paragraph.scroll`, `total`=число логических строк) +
  скроллбар на правой рамке **вдоль области содержимого** (не всей высоты — бегунок не
  заезжает на вкладки). Лockup — только при запасе места (`inner.height >= LOCKUP_ROWS+6`
  и ширина; жёсткая деградация, docs/branding.md §5). **Лицензия переносится по словам**:
  абзацы (разделены пустой строкой) собираются заново и переносятся `wrap::wrap_line` под
  ширину диалога (иначе длинные строки MIT клипались бы справа; сборка по `lines()` —
  CRLF-safe). Локальный таб-стрип (не переиспользуется `settings::helpers::tab_strip_line`
  — тот `pub(super)` к модулю настроек).
- **i18n**: новые ключи `ui.help.tab.{about,hotkeys,license,components}`, `ui.about.{desc,
  author,version,site,repo,crate}`, `ui.components.intro`, `ui.help.footer.tabs`
  (ru+en); `ui.help.title` → «О программе»/«About»; удалены осиротевшие
  `ui.help.footer.{scroll,any}`. i18n-гейты (parity/no-dead/кириллица) накрыли автоматически.
- **Тесты**: `credits` (гейт-сверка с Cargo.toml + сортировка/лицензии + встроенный
  LICENSE); экран чата (`f1_opens_help_and_esc_closes` — прочая клавиша не закрывает;
  `help_navigation_scrolls_and_switches_tabs` — прокрутка + Tab/← со сбросом; кламп+
  скроллбар на коротком терминале; `help_tabs_render_distinct_content` — все 4 ярлыка в
  таб-стрипе + характерное содержимое каждой вкладки; логотип показ/скрытие по высоте).
  **1262 юнит-теста зелёные** (+4), clippy `-D warnings`/fmt/i18n чисты. Живой прогон не
  требуется (чистый UI, `TestBackend`; композиция сверена дампом буфера всех вкладок).
- **Доводка по отзыву** (та же ветка): (1) **отдельная вкладка «Команды»** — команды
  поля ввода (`/rag …`/`/tts …`) вынесены из `HELP_KEYS` в новый `HELP_COMMANDS`
  (общий рендер `key_lines`), вкладка `HelpTab::Commands`, ключ `ui.help.tab.commands`;
  порядок вкладок теперь `About/Hotkeys/Commands/License/Components` (5). (2) **Версии
  компонентов** — `COMPONENTS: &[(&str,&str,&str)]` `(имя,версия,лицензия)`; новый
  гейт `component_versions_match_cargo_lock` (парсит `Cargo.lock`, версия должна быть
  среди залоченных — устойчиво к дублям `thiserror` 1/2, `windows-sys`). (3) **Отбивка
  над логотипом** (лишняя пустая строка в шапке). (4) **Память последней вкладки** —
  поле `ChatScreen.help_last_tab` (как `emoji_last`): закрытие сохраняет `help.tab`,
  `HelpState::open(tab)` восстанавливает при следующем открытии (`new()` удалён;
  `DEFAULT_HELP_TAB = Hotkeys`). (5) **Промежутки между пунктами «О программе»**
  (пустая строка перед каждым). (6) **Бренд-имя `mindfork`** в заголовке попапа
  (`credits::APP_NAME` вместо `CARGO_PKG_NAME=mindfork-rs`) — и в теле вкладки «О
  программе». (7) **Версия в заголовке окна терминала** (`app/runtime`: `SetTitle`
  теперь `mindfork v<версия>`, совпадает с попапом; Windows). Тесты: `help_remembers_
  last_tab`, вкладка «Команды» в `help_tabs_render_distinct_content` (+ проверка, что
  команд НЕТ на вкладке клавиш), версия компонента в дампе. **1264 юнит-теста зелёные**
  (+2), clippy `-D warnings`/fmt/i18n чисты.
- **Задел**: кликабельные ссылки (OSC 8 hyperlinks — терминало-зависимо), генерация
  списка компонентов (версии/лицензии) из `cargo metadata` в build.rs (сейчас
  curated-список + гейты по Cargo.toml/Cargo.lock).
...
- **Словари спелл-чека** (Hunspell-пары `*.aff`+`*.dic`: `en_US`/`en_GB`/`ru_RU`)
  лежат в `dictionaries/` корня проекта **и коммитятся в репозиторий**. `build.rs`
  копирует их в `target/<profile>/data/dictionaries/` при сборке (dev-спелл-чек из
  коробки); релизный workflow упаковывает их в архив как `data/dictionaries/`
  (спелл-чек из коробки и в релизе). Открытый `[R]`: качество ru_RU (по факту —
  работает).
