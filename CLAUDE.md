...
Сделан весь план **M0–M9** плюс обширный пост-M9 (в `main`). **1157 юнит-тестов
зелёные, 53 `#[ignore]`-смоука** (крупнейший счётчик — журнал ниже; текущее
направление — **система плагинов** — **завершено** (исследование
...
### Пост-M9: айдентика — этапы 1–2 (ассеты + иконка Windows) (сделано)
...
- **Дальше**: этап 3 `feat/linux-desktop-entry` (`.desktop` + hicolor-иконки в
  `nfpm.yaml`, проверки в смоуке `packaging.yml`), этап 4 `feat/tui-logo`
  (`widgets/logo.rs` — глиф half-block'ами 16×8 в шапке `F1` + гейт-тест «код ≡ SVG»).
