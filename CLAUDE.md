...
### Пост-M9: загрузка/удаление файлов в RAG командами `/rag add|delete` (сделано)
- **Команды в поле ввода**: `/rag add <путь>` индексирует файл или директорию в базу
  ...
  обход; `/rag delete <путь>` (синоним `remove`) удаляет из базы файл или директорию
  (со всем, что под ней). Пока поддержаны только `*.txt`/`*.md`. Разбор — чистая
