...
Сделан весь план **M0–M9** плюс обширный пост-M9 (в `main`). **1120 юнит-тестов
зелёные, 50 `#[ignore]`-смоуков** (крупнейший счётчик — журнал ниже; текущее
направление — **система плагинов** (исследование `docs/research/plugin-system.md`,
развилки Р1–Р8 приняты 2026-07-17): **этап 1 «generic-импорт» сделан**
(`feat/generic-import` — нейтральный формат `mindfork-import` +
`docs/import-format.md` + CLI `import`, `import-lamellama` удалён — знание о
непубличной LameLLaMA уезжает в приватный внешний конвертер); **этап 2 «зонд
MCP-клиента» — GO** (`spike/mcp-client`: мини-клиент `shared/mcp.rs` + живой смоук
— Gemma 4 31B вызвала инструмент реального `server-filesystem` первым раундом,
14 инструментов ≈ 7 КиБ схем, протокол 2025-11-25); **этап 3a «ядро MCP-хоста»
сделан** (`feat/mcp-host`: клиент в бинаре + `McpManager` в оркестраторе +
обёртка `McpTool` + `config.mcp` + гейты/отмена; живой e2e через оркестратор —
GO); следующий шаг — **3b** (UI статусов/описаний серверов, TOFU-пиннинг
каталога, доки, ADR направления); прежнее направление —
