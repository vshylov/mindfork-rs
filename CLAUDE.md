...
Сделан весь план **M0–M9** плюс обширный пост-M9 (в `main`). **1176 юнит-тестов
...
- **Гейт переехал к механизму**: `prime_full_redraw_repaints_all_but_wide_glyph_tails`
  ...
  переведены на реальный хелпер вместо своей копии сентинела. **1176 юнит-тестов
  зелёные** (+9 к `main`), 53 `#[ignore]`.
...
- **Как определяется «изменение»**: у ленты уже есть кэш блоков по fingerprint —
  промах кэша и есть «содержимое перерисовано заново». `MessageFeed.content_changed` +
  `take_content_changed()`; `ChatScreen::render` сворачивает флаг в `full_redraw`, если
  в ленте есть глифы риска. Так не пришлось трогать 9 мутаторов ленты, и триггер
  точный (правка/стрим), а не «кто-то что-то позвал».
- **Кадр-«догон»**: промах кэша виден только **в ходе** отрисовки, поэтому перерисовка
  исполняется следующим кадром. Петля будит себя сама (`has_pending_full_redraw()` →
  `dirty`), иначе последний чанк стрима оставлял бы артефакт до постороннего события.
  Догон ровно один: при неизменном содержимом кэш попадает и запрос не возобновляется
  (покрыто тестом — иначе была бы вечная перерисовка).
