...
- **Таблицы в ленте получили опциональные горизонтальные разделители между
  ...
  **по умолчанию включена**; выключение возвращает прежний компактный вид
  (разделитель только под заголовком). Тумблер «Разделители строк таблиц» —
...
- **Проводка через `markdown::RenderOpts`** (прецедент — `RenderOpts` у
  ...
  цел).
- **Лента**: поле `MessageFeed.table_row_separators` (дефолт `true` — зеркало
