...
- **[docs/decisions/](docs/decisions/)** — ADR: UI-крейты под ratatui 0.30 (0001),
  ...
  машинно-привязанным шифрованием (0008), озвучивание сообщений (0009).
...
## Статус (на 2026-07-21, версия 0.9.1)
Сделан весь план **M0–M9** плюс обширный пост-M9 (в `main`). **1264 юнит-теста
зелёные, 62 `#[ignore]`-смоука** (крупнейший счётчик — журнал ниже; текущее
направление — **озвучивание сообщений чата (TTS)** (команда `/tts`, речевой
экстрактор markdown, плеер `rodio`, провайдеры **managed** (локальный сайдкар
`piper` + `mindfork tts setup`)/OpenAI/Gemini/external, вкладка «Озвучивание» в
настройках; исследование [docs/research/tts.md](docs/research/tts.md), развилки
Р1–Р9 приняты 2026-07-21, итог — **ADR 0009**) — **завершено** (этапы 1 «ядро» +
2 «локальный движок»); прежнее —
**API-ключи в настройках** (ввод в окне настроек + машинно-привязанное
...
### Пост-M9: озвучивание сообщений чата (TTS) — этап 1 (сделано)
- **Новое направление** (запрос пользователя): читать сообщения чата вслух, с
  пропуском кода/диаграмм при озвучке и управлением командой в поле ввода.
  Исследование — [docs/research/tts.md](docs/research/tts.md) (три параллельных
  веб-обзора по первоисточникам: локальные движки, облачные API, воспроизведение в
  Rust); развилки **Р1–Р9 приняты пользователем 2026-07-21**. Этап 1
  (`feat/tts-core`) — всё, кроме локального сайдкара (он — этап 2).
- **Провайдер озвучивания — независимый «серверный слот»** (как эмбеддинги,
  ADR 0002): у Anthropic TTS **нет вовсе** (проверено по полной поверхности API),
  поэтому пользователь Claude озвучивает через OpenAI/Gemini/локальный сервер.
  `TtsSettings` (`mode` + под-структуры на режим + поведение), всё
  `#[serde(default)]` — без миграций. **Managed-режим в селектор не добавлен**
  (прецедент: `Claude` не показывали, пока не появился `AnthropicClient`) — нерабочий
  пункт не шипим. Ключ облака **общий с чатом** (ADR 0008), вводить заново не нужно;
  фолбэк на env-переменную сохранён.
- **Клиенты** (`shared/tts/`): `openai.rs` — `POST /v1/audio/speech`, обслуживает
  **и** облако OpenAI, **и** сторонний OpenAI-совместимый сервер (различаются base
  URL/ключ/формат ответа); у облака просим `response_format:"pcm"` (24 кГц s16le,
  мимо декодера), у стороннего — `wav` (самый переносимый; часть серверов другого
  не умеет). Незаданные поля не шлём (`skip_serializing_if`) — наборы у серверов
  разные. **`speed` у `gpt-4o-mini-tts` де-факто игнорируется** (известный дефект) →
  в UI и доках скорость просят словами в `instructions`. `gemini.rs` — нативный
  `generateContent` c `responseModalities:["AUDIO"]` (тот же транспорт, что у
  `GeminiClient` — новый не нужен); ответ base64 PCM **без WAV-заголовка**, частота
  парсится из `mimeType` (`audio/L16;codec=pcm;rate=…`), не хардкодится; блокировка
  фильтром и пустой ответ → понятная ошибка, а не тишина. `streamGenerateContent`
  для TTS не используем (он только у 3.1+, а конвейер по предложениям и так даёт
  быстрый первый звук).
- **Речевой экстрактор** (`shared/markdown/speak.rs`) — второй потребитель тех же
  событий `pulldown-cmark`, что и `Writer` (ADR 0003); `Writer` не тронут. Правила —
  таблица исследования §5: код/` ```mermaid `/таблицы/блочные формулы **пропускаются
  с короткой голосовой пометкой**, inline-код читается текстом, inline-математика
  идёт через существующий `latex_to_unicode`, у ссылки читается только текст, голый
  URL → «ссылка: домен», у изображения — alt. Пометки и префиксы ролей — на **языке
  профиля** (ось A: это речевой контент, не UI-хром). «Мысли» и tool-блоки не
  озвучиваются по построению (их нет в `Message.text`).
- **Воспроизведение** (`playback.rs`, `rodio` 0.22): очередь `Player::append` даёт
  конвейер «синтез N+1 во время проигрывания N» бесплатно. Учтены обе TUI-ловушки:
  `log_on_drop(false)` (иначе rodio пишет в stderr **поверх TUI**) и ленивое
  открытие устройства по первой команде (на Linux libasound шумит в stderr при
  энумерации, cpal#384). Нет звука → `Err` и заметка «звук недоступен», **не паника**
  (мягкая деградация, паттерн `UnavailableEmbedder`). API 0.21/0.22 переломан —
  `MixerDeviceSink`/`Player`/`DeviceSinkBuilder::open_default_sink`; `Send+Sync`
  подтверждён по исходникам cpal 0.17 (у ALSA — compile-time-ассерты), поэтому
  плеер живёт прямо в фоновой tokio-задаче, без выделенного аудио-потока.
- **Оркестрация** (`app/orchestrator/tts.rs`, зеркало `rag.rs`): снимок переписки
  берёт оркестратор (владелец `Chat`) → отбор по объёму (`build_utterances`) →
  чанкинг по предложениям (переиспользован `rag::split_sentences`) → фоновая
  отменяемая задача. Гейтов генерации нет — озвучивается снимок на момент команды.
  **Все точки остановки — в одном `stop_tts()`**: по настройке (смена чата, начало
  генерации) и **безусловно** (удаление обмена, перегенерация, удаление чата —
  озвучиваемого текста больше нет). Гонка «остановили старую → запустили новую»
  закрыта поколением задачи (`tts_gen` + внутренний канал `done`), как у
  имперсонации/авто-названия.
- **UI**: команда `/tts`/`/tts N`/`/tts all`/`/tts stop` (чистый парсер
  `features/tts_command.rs`; ошибка несёт **сам аргумент**, а текст подсказки
  строит UI локализованно — в отличие от `/rag`, где usage захардкожен по-русски),
  подсветка команды в поле ввода, оверлей помощи, тихий чип «♪ озвучка» в
  статус-баре (нота U+266A входит в WGL4 → в компат-режиме не заменяется), вкладка
  **«Озвучивание»** (четвёртая) в секции «Модель» настроек с группами «Движок»
  (mode-driven поля + статус ключа) и «Поведение» (три тумблера). Чипа статуса
  сервера у вкладки нет — клиенты stateless, пробы нет.
- **Цена по зависимостям**: `rodio` (+`cpal`/`symphonia`) и `base64` (промоутнут из
  транзитивных, как `quick-xml`). Единственная новая **системная** зависимость —
  ALSA на Linux: `libasound2-dev` добавлен в apt-шаги `ci.yml`/`release.yml`/
  `packaging.yml`, рантайм-зависимости `libasound2` (deb) / `alsa-lib` (rpm/arch) —
  в `nfpm.yaml`. `deny.toml` править не пришлось (MPL-2.0 для Symphonia уже был в
  allowlist); `cargo deny check` — advisories/bans/licenses/sources ok.
- **Тесты**: парсер команды (7), речевой экстрактор (15, включая golden на
  характерном ответе LLM и пер-локальный гейт), клиенты (wire-формы запросов,
  разбор ответа Gemini, частота из mimeType, блокировки), конвертация PCM→f32 и
  мягкая деградация плеера, отбор сообщений/префиксы ролей/чанкинг (10),
  оркестрация (7: отказы при ненастроенном движке, точки остановки по настройке и
  безусловные, идемпотентность `/tts stop`, гонка позднего `done`), настройки (2).
  **1254 юнит-теста зелёные** (+55 к main), **58 `#[ignore]`** (+5 живых смоуков:
  синтез OpenAI/Gemini, сторонний сервер, воспроизведение тона, e2e через
  оркестратор), clippy `-D warnings`/fmt/`cargo deny` чисты.
- **Живой прогон — GO**, и он же нашёл **два дефекта**, которые юнит-тестами не
  ловятся:
  1. **Gemini отвергал короткую реплику** (`400 Model tried to generate text, but
     it should only be used for TTS`) — модель принимала транскрипт за задание.
     Оказалось, префикс-директива у Gemini **обязателен**, а не косметичен:
     при пустых «Указаниях» подставляем нейтральную (`Read this text aloud
     verbatim: …`, канонический вид доков — `Say cheerfully: …`); язык директивы на
     язык речи не влияет (он определяется транскриптом).
  2. **Срез хвоста последнего чанка**: очередь `rodio` сообщает «пусто», когда
     источник исчерпан **миксером**, а звуковая карта ещё доигрывает буфер
     (замерено: клип 400 мс → `is_drained` через ~367 мс). Дроп `Playback`
     закрывает устройство, поэтому после опустошения очереди ждём `DRAIN_TAIL`
     (300 мс) перед завершением задачи.
  Прогнано на этой машине: синтез OpenAI (`gpt-4o-mini-tts`, 240 КБ PCM 24 кГц) и
  Gemini (`gemini-2.5-flash-preview-tts`, 24 кГц из `mimeType`); воспроизведение
  тона 440 Гц в реальном времени; **e2e через оркестратор** (`/tts 2` на чате с
  кодом внутри ответа: код пропущен пометкой, роли озвучены) — 14.5 с на OpenAI и
  19.5 с на Gemini, задача завершилась штатно. Звучание (в т.ч. русский и латинские
  вставки) — ручная проверка пользователя.
- **CI поймал ещё два дефекта пакетирования** (Linux локально не воспроизводим —
  разработка на Windows): (1) шаг установки `libasound2-dev` не попал в джоб сборки
  `packaging.yml` (`alsa-sys` падал на pkg-config); (2) **ловушка time_t64**: в deb
  зависимость `libasound2` после перехода (Ubuntu 24.04+/Debian trixie) стала
  **виртуальной**, и apt удовлетворял её OSS-шимом `liboss4-salsa-asound2` без части
  символов — пакет ставился, но приложение падало при запуске (`symbol lookup error:
  undefined symbol: snd_device_name_get_hint`, exit 127). Записано альтернативой
  `libasound2t64 | libasound2` (на старых системах работает второе имя). Плюс
  `pacman -U` не тянет зависимости из репозиториев — в Arch-смоуке `alsa-lib`
  ставится явно. Итог: **все 8 проверок PR зелёные** (линты, тесты ubuntu+windows,
  сборка пакетов, смоуки установки ubuntu:24.04/fedora/archlinux, инсталлятор).
- **Задел** (roadmap): этап 2 — локальный сайдкар (sherpa-onnx + piper-голоса,
  `mindfork tts setup` по образцу ADR 0005) и ADR по итогам направления;
  SSE-стриминг внутри чанка, кэш аудио, авто-выбор голоса по языку сообщения,
  маршрутизация кириллица/латиница по двум голосам, чтение таблиц построчно,
  озвучка выбранного сообщения из ленты, мульти-спикерный диалог (Gemini), ОС-TTS.

### Пост-M9: озвучивание (TTS) — этап 2 (локальный движок сайдкаром) (сделано)
- **Завершение направления** (ветка `feat/tts-sidecar`, стек на `feat/tts-core`;
  итог — **[ADR 0009](docs/decisions/0009-tts-speech-synthesis.md)**): режим
  **`managed`** — озвучивание **офлайн**, без сети, ключей и сервера. Провизия —
  `mindfork tts setup` (бинарь + голоса в `data/tts/` по lock-списку URL+sha256,
  калька с ADR 0005); в селекторе режимов появился четвёртый вариант (прежде его
  намеренно не шипили — прецедент `Claude`).
- **Спайк в начале этапа (обязательный по плану) закрыл рекомендованный подвариант
  (а).** Отслушивание кириллицы прошло на **GO** (piper-голоса `ru_RU-dmitri`/
  `denis`, данные CC0; замеры: загрузка модели ≈0.8 с, синтез RTF 0.05 — 10 с речи
  за полсекунды на CPU), но по дороге выяснилось: CLI `sherpa-onnx-offline-tts`
  принимает текст **только позиционным аргументом**, а его узкий `main()`
  заставляет Windows-CRT конвертировать argv из UTF-16 в ANSI-кодовую страницу
  (на машине разработки `1252`) — **вся кириллица превращается в `?`, синтезируется
  ...
  Латиница при этом работала бы — для русскоязычного приложения это блокер.
  Решение пользователя по развилке: **подвариант (в) — MIT-бинарь `piper`**
  (текст через **stdin**, UTF-8 байтами; сырой PCM в stdout `--output_raw`).
  Подвариант (б) (Rust-крейт in-process) отклонён: статическая линковка
  C++/onnxruntime в наш бинарь + сетевой шаг build-script + правки CI/пакетирования
  не окупают выигрыша. Спайк синтезировал сэмплы через **C API sherpa-onnx**
  (ctypes) — обход того же argv-барьера; заодно отслушан **Supertonic 3** (RTF 0.19,
  44.1 кГц) — оставлен заделом.
- **`shared/tts/sidecar.rs`** — `PiperTts: TtsEngine` (ещё одна реализация
  контракта этапа 1; чанкер/точки остановки/плеер/экстрактор не тронуты): поиск
  бинаря (настройка → env `MINDFORK_TTS_PIPER` → `data/tts/piper/piper[.exe]` →
  `data/tts/piper[.exe]`), резолв голоса (имя файла в `voices/` **или** полный путь
  к `*.onnx`; рядом обязателен `*.onnx.json`), частота сэмплов читается из конфига
  голоса (фолбэк 22050). Отмена — `kill_on_drop` + `select!` с токеном; скорость
  `speed` инвертируется в `--length_scale` (у piper больше = медленнее). У движка
  **крупный `max_input_chars`=2000**: модель грузится на каждый спавн (~0.8 с),
  поэтому чанкер складывает больше предложений в вызов.
- **Голоса — только permissive** (решение Р2): `ru_RU-dmitri-medium` (дефолт
  настроек) и `en_US-amy-medium` ставятся сразу, `ru_RU-denis-medium` — по
  `--voice`; **`ruslan` (CC BY-NC-SA) и `irina` («Unknown») не включены** (гейт в
  тестах). Голоса нужны **оригинальные с HuggingFace**: переэкспортированные ONNX
  из model zoo sherpa роняют piper (`STATUS_STACK_BUFFER_OVERRUN`) — проверено.
- **`features/provision.rs`** — общая механика провизии (скачивание потоком/в
  память, сверка sha256, распаковка tar.gz/zip) **выделена из `sandbox_setup`**,
  когда у неё появился второй потребитель; ключи бандла `sandbox.setup.*` →
  **`setup.*`** для перенесённых текстов. `features/tts_setup.rs` — lock-списки
  (4 платформенных архива piper + 3 голоса; sha256 посчитаны при заведении, GitHub
  для релиза 2023 `digest` не отдаёт) + идемпотентная установка.
- **Проводка**: `TtsMode::Managed` (+`ALL`/`label`/`cloud_provider`→`None`),
  `TtsSettings.managed { voice, binary }` (`#[serde(default)]` → без миграции),
  `engine_from_config(..., tts_dir)`, `Paths::tts_dir()`/`JsonStore::tts_dir()`,
  новые структурные ошибки `TtsSetupError::{Binary, Voice}` с подсказкой выполнить
  `tts setup`; CLI `tts setup [--force] [--voice NAME]…` (свой парсер + справка +
  гард единственного экземпляра); вкладка «Озвучивание» в managed-режиме показывает
  только голос и путь к бинарю (ни ключа, ни URL, ни модели/указаний). i18n —
  ru+en (гейты parity/no-dead накрыли автоматически).
- **Тесты**: sidecar (резолв бинаря/голоса, структурные ошибки, аргументы запуска
  и инверсия скорости, фолбэк частоты, приоритет явного пути); provision (hex/
  sha256, round-trip zip и tar.gz); tts_setup (выбор архива по платформе,
  пришпиленность и permissive-состав lock-списка, набор голосов по умолчанию,
  неизвестный голос → ошибка со списком + локализация); settings (managed-поля и
  маршрутизация правки голоса мимо облачных); config (дефолт голоса); CLI (формы
  `--voice`/`--force`, ошибки). **1264 юнит-теста зелёные** (+10), **62
  `#[ignore]`** (+4), clippy `-D warnings`/fmt чисты.
- **Живой прогон — GO** (Windows, реальные бинарь и голоса): `mindfork tts setup`
  скачал и разложил всё (piper 22 МБ + два голоса по 60 МБ; повторный запуск —
  идемпотентен), три смоука `sidecar` зелёные (синтез **3.96 с русской речи** —
  при поломке кодировки было бы ~0; воспроизведение слышно; отмена прерывает
  синтез), e2e **через оркестратор** `tts_speaks_chat_managed_e2e_live` — `/tts`
  на чате с кодом внутри ответа озвучен локально за 10.8 с.
- **Задел** (roadmap/ADR 0009): долгоживущий протокол piper (`--json-input`) вместо
  спавна на чанк; маршрутизация кириллица/латиница двумя голосами (латинские
  вставки в русском голосе читаются с акцентом — дефект espeak-фронтенда);
  Supertonic 3; SSE-стриминг внутри чанка; кэш аудио; авто-выбор голоса по языку;
  чтение таблиц построчно; мульти-спикерный диалог (Gemini); ОС-TTS.
