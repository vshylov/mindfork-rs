...
- **Режим хранения данных через маркер `location.json`** рядом с бинарником
  ...
  (нет маркера/пустой/`portable`) — данные **рядом с бинарником** (обратная
  совместимость существующих установок). `system` → стандартная ОС-папка (крейт
  `directories`: Windows `%APPDATA%\mindfork-rs\data`, Linux `~/.local/share/mindfork-rs`);
  `path` → произвольный каталог. `Paths::discover()` читает маркер, резолвит корень и
  создаёт его для не-портативных режимов; сам маркер **всегда** остаётся рядом с
  бинарником (он про установку, не пользовательские данные). **Повреждённый JSON
  маркера — ошибка запуска** (опечатка в пути не должна молча увести на пустой набор
  данных). Логи/инстанс-гард/хранилище едут за корнем автоматически (`discover` →
  `logging::init`). Новый аксессор `paths.backups_dir()`.
