...
- **Тесты**: db (supersede скрывает из списка/семантики/`is_active`; связи и соседи в
  ...
  **648 тестов зелёные**, clippy/fmt чисты.
- **Вне объёма (Ярус 3)**: консолидация `consolidate_notes` + авто-«сон»; перенос
  связей при merge на объединённую заметку; vec0 для заметок при росте объёма.
