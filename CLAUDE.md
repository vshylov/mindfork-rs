...
Сделан весь план **M0–M9** плюс обширный пост-M9 (в `main`). **1172 юнит-теста
...
- **Два гейта на инвариант** (оба **мутационно проверены** возвратом `❤️` в список):
  ...
  diff'а не целится во вторую половину широкого глифа. **1172 юнит-теста зелёные**
  (+5 к `main`), 53 `#[ignore]`.
- **Замечание на будущее**: тем же приёмом-сентинелом пользуется давний путь
  прокрутки ленты (`take_feed_scrolled`), который включается **именно при наличии
  VS16** в ленте — там та же эмиссия хвостов и тот же сдвиг ряда возможны. В ленте
  сдвиг гасится ближайшим `MoveTo` (ряды перерисовываются целиком), поэтому
  воспроизведения не было; но если всплывёт — корень тот же.
