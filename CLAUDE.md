...
- **Исходная дальняя цель связности** ([docs/narrative-as-notes.md](docs/narrative-as-notes.md)):
...
- **Полное смешение выдачи за тумблером** ([docs/narrative-as-notes.md](docs/narrative-as-notes.md),
...
- **Третий орган памяти (база знаний RAG) связан с заметками/наблюдениями**
  ([docs/narrative-as-notes.md](docs/narrative-as-notes.md), Путь 3): заметка может
...
- **Инварианты целы**: DB-only, изоляция по `profile_id`, без `ChatEffect`, без миграций
  ...
  **729 тестов зелёные**, 24 `#[ignore]`, clippy/fmt чисты. Живая оценка «сна» наблюдений —
  ручной шаг (юнит-тесты механизм покрывают).
