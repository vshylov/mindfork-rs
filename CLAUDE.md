...
- **Вордмарк — собственный пиксельный шрифт, а не SVG.** Переиспользовать
  ...
  ExtraBold): строчные, жирные штрихи, выносные у `d`/`f`/`k`. Итог — 36 колонок × 3
  строки; лockup целиком — **49×6**.
- **Пропорции взяты из метрик бренда, не на глаз** (`artwork/README.md`): высота
  слова = половина высоты глифа (`0.5227 × S`) → 3 строки против 6; базовая линия
  (`0.7418 × S`) → низ слова на **строку выше** низа глифа (`WORDMARK_TOP_ROW = 2`);
  зазор `0.3608 × S` меряется от **бокса** иконки, а мы рисуем только чернила (x 3…13),
  поэтому в терминале он равен `0.3608 × 16 − 3` ≈ **3 колонки**. Разрядка между
  буквами — 1 колонка.
...
- **Живой прогон не требуется** (чистый UI без движка/памяти; покрыт `TestBackend`).
  ...
  читается, лockup вписывается в ширину попапа с запасом (49+6 против ~78).
