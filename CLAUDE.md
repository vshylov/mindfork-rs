...
- **Вордмарк — собственный пиксельный шрифт, а не SVG.** Переиспользовать
  ...
  толщины, что бары глифа (в его сетке они тоже по 2 единицы). Итог — 51 колонка × 4
  строки; лockup целиком — **64×6**.
...
- **Правка по отзыву на первую версию** (слишком много воздуха справа; `i` вдвое
  ...
  стало ExtraBold, как бренд, и заодно выросло с 48 до 64 колонок — воздух справа
  ужался с ~26 до ~8 колонок (при попапе ~76). Обе претензии закрылись одной правкой.
...
- **Живой прогон не требуется** (чистый UI без движка/памяти; покрыт `TestBackend`).
  ...
  читается, лockup вписывается в ширину попапа с запасом (64+6 против ~78).
