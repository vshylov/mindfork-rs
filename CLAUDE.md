...
Сделан весь план **M0–M9** плюс обширный пост-M9 (в `main`). **1002 юнит-теста
зелёные, 46 `#[ignore]`-смоуков** (крупнейший счётчик — журнал ниже; последнее
направление — **мультиязычность интерфейса (ось B, язык UI)**, ветка
`feat/i18n-ui`). **Ось A (язык агента) завершена (Ярусы 1–2, все 35 инструментов);
ось B (язык интерфейса) — основные поверхности локализованы** (статус-бар, лента,
настройки, список чатов, попапы/справка, «модель себя», ошибки, экспорт; поле «Язык
интерфейса» в настройках, независимое от языка агентов). Задел: метки инструментов
`ui_label`/заголовки групп в тумблерах профиля и reasons супервайзера — русские.
...
- **Первый ярус направления i18n** (дизайн-план [docs/i18n.md](docs/i18n.md), ветка
...
- **Трейт `Tool` получил язык каркаса** (ветка `feat/i18n-tools`, docs/i18n.md Ярус 2):
...
- **Локализация UI-хрома** (тексты для **человека**) по плану
  ...
  ось A (язык агента, docs/i18n.md): переиспользует `shared/i18n` (ключи `ui.*`), но
...
- **Задел (документированные ограничения)**: (1) метки инструментов в тумблерах профиля
  (`ToolInfo.ui_label` hint + `ToolGroup::title()` заголовки групп) — русские; чистая
  локализация трогает слой `features/tools` (трейт/meta) + ~43 короткие строки, вынесена
  в отдельный шаг (тумблер показывает английский `id` инструмента, поэтому основная
  метка уже английская). (2) Reasons `ServerStatus::Disconnected(...)` из `supervisor.rs`
  (редкие ошибки конфигурации сервера) — русские: супервайзер не знает язык UI (нет
  `config`), проводка требует отдельной правки. (3) Ярус 3 оси A (внешние `data/locales`).
