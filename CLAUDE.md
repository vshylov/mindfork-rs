...
### Пост-M9: затемнение фона под попапом помощи (сделано)
- **Симптом**: оверлей помощи (`F1`/`?`) очищал только свою область (`Clear`), а чат
  за ним оставался на полной яркости и зрительно сливался с попапом, затрудняя
  восприятие.
- **Фикс** (`screens/chat.rs`): новый хелпер `dim_background(frame)` накладывает
  `Modifier::DIM` на **все** ячейки буфера экрана; вызывается **перед**
  `render_help`. `render_help` затем делает `Clear` своей области (сброс ячеек попапа
  к дефолтному, неприглушённому стилю) и рисует содержимое — так притеняется только
  фон, а сам попап остаётся ярким. Эффект `DIM` терминало-зависим (Windows Terminal
  поддерживает). Притенение намеренно ограничено помощью; попап орфографии (`Ctrl+G`)
  и оверлей выбора профиля пока не трогаются.
