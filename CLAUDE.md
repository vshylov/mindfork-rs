...
- **Дизайн-планы направлений** (банк идей + реализуемый MVP-зонд): «модель себя»
  ...
  **связность заметок** ([notes-connectivity.md](docs/notes-connectivity.md)) —
  ...
  ([narrative-as-notes.md](docs/narrative-as-notes.md)) — унификация: наблюдения
  «модели себя» переезжают в заметки с тегом `@self` (Ярус 1 сделан).
...
- План доводки — [docs/refinements.md](docs/refinements.md) (6 этапов + направление
...
- Этап 2 плана [refinements.md](docs/refinements.md): «я во времени» получает время,
...
- Этап 3 плана [refinements.md](docs/refinements.md): авто-рефлексия перестаёт
...
- Этап 4 плана [refinements.md](docs/refinements.md): соединение уже существующих
...
- Этап 5 плана [refinements.md](docs/refinements.md): рефлексия/консолидация —
...
- **Унификация органов памяти** по плану [docs/narrative-as-notes.md](docs/narrative-as-notes.md):
...
- Продолжение Яруса 1 ([docs/narrative-as-notes.md](docs/narrative-as-notes.md)):
...
### Пост-M9: нарратив как заметки — Ярус 2, шаг C (ворота почти-дублей черт) (сделано)
- **Семантические ворота почти-дублей черт `user_model`** — отложенный шаг C Яруса 2
  ([docs/narrative-as-notes.md](docs/narrative-as-notes.md)), **зеркало ворот
  ...
  до этой правки) выше порога косинусной близости `TRAIT_SIMILARITY=0.85`; при
  наличии близкой инструмент **предупреждает** о почти-дубле («Похожие черты уже
  есть… оставь одну через `remove_traits`, а не копи перефразы»). **Мягкие ворота**
  (предупреждение, не запрет — решение за моделью, обе черты сохраняются).
...
- **Черты остаются `Vec<String>`** (без миграции схемы); описание `update_user_model`
  ...
  без `ChatEffect`.
- **Тесты**: `add_trait_gate_surfaces_near_duplicate` (близкая черта поднимает ворота
  + подсказка `remove_traits`), `add_trait_gate_silent_for_dissimilar` (непохожая
  молчит, обе черты сохранены — ворота только предупреждают). На `MockEmbedder`
  («aaaa bbbb» ↔ «aaab» cosine ≈ 0.89 > порога). **718 тестов зелёные**, 20
  `#[ignore]`, clippy/fmt чисты. Живая проверка ворот черт — ручной шаг.
