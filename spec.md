...
1. **Managed (default)** — the application launches a `llama-server` child process itself with the necessary arguments (`-m` GGUF, `-ngl`, `-c`, `--jinja`, `--port`, `--no-mmap`, etc. from the model settings — and, on a CPU-only host, the **batch**: the server looks at its queue between batches of `-b` prompt tokens, so a stopped or displaced stream holds its slot for one, 23 s at the default 2048 on a CPU-only host against 6.5 s at 256 for a seventh slower prompt processing, measured in [docs/research/cpu-batch.md](docs/research/cpu-batch.md) §3.1; the *Batch (-b)* field empty means auto — `-b 256 -ub 256` when GPU layers is 0, nothing passed otherwise, the line byte for byte what it was — and a typed number is passed as is with `-ub` clamped at 512; the embedding server keeps its own batch — and where the batch is **not** at that knee, a GPU host at its defaults or an external server the user launched, the app reads the server's own timing of every prompt — the compaction roll's, the three silent loops', a page summary's a tool asked for, the automatic title's and — on the shared engine — impersonation's included, the cold prompts a server that kept its cache still processes whole: a turn on a warm prefix 31 tokens, the roll 1466, a reflection's first round 2798, a page summary's head 3236 (a stream of the turn: its timing rides the tool's outcome, [docs/research/page-summary-usage.md](docs/research/page-summary-usage.md) §3.2), impersonation's whole conversation 10 642 — the largest a session makes, since nothing of a swapped conversation is in any slot's cache ([docs/research/oneshot-samples.md](docs/research/oneshot-samples.md) §2.1), measured in [docs/research/roll-timings.md](docs/research/roll-timings.md) §2.1 and [docs/research/loop-timings.md](docs/research/loop-timings.md) §2.1 — (`llama-server`'s `timings`, net of the prefix cache) and, once per server session, says in the feed how fast prompts are processed, how long a cancelled stream would hold its slot at the batch the server runs, and the one change — the *Batch (-b)* field, or `-b 256 -ub 256` on the launch line; nothing on a GPU's second, on the clouds, or on a prompt too short to measure; [docs/research/slow-prefill-detection.md](docs/research/slow-prefill-detection.md)), waits for readiness (polling `/health`), connects. On application exit — cleanly stops the child process. Changing the model = restarting the server. Model-load progress is shown in the UI (parsing the server's stdout/stderr into a log panel — an analog of attempt #1's "View Init Logs"). **Preflight:** before launching, the model file's presence is checked — `spawn` of the child process succeeds even without the GGUF (it only fails if the binary itself is missing), and the server would then die during loading while the readiness probe waited pointlessly until timeout; so a missing/inaccessible file immediately yields `Disconnected` with a clear message instead of hanging in "connecting…". **Multi-file models:** weights too large for one file ship as `gguf-split` parts (`<name>-00001-of-00003.gguf`, …); the server is given the **first** part and reads the rest itself, by name, from the same directory, so the path is passed through untouched. What the preflight adds is the two failures the path alone hides — a part that is not the first (llama.cpp refuses it) and a part missing next to it (an interrupted download) — each named with the file to use or the file to fetch, rather than left to surface as the generic early exit. The part tail names a file, not a model, so it is also cut from the model name shown in the UI (`shared/gguf.rs`). **Early exit:** if the file is valid but the process dies *during* loading (a corrupt GGUF, out of memory), the child-process monitor task raises an `exited` signal that the readiness probe watches — it stops polling immediately with a clear error, instead of waiting for the timeout. When the process ended on llama.cpp's own refusal — its argument parser prints `error: invalid argument: --x` or `error while handling argument "-c": …` and exits at once — that line **is** the error (*"llama-server refused to start: error: invalid argument: --n-cpu-mo"*), cleaned of colour; the exit is announced only once the child's output has been read to the end (at most a second after the process is gone, so a grandchild holding the pipe cannot hold the probe), since the process's exit and its last line race. Any other exit keeps the generic *corrupt GGUF or out of memory? — see logs*.
...
- Path to the `llama-server` binary (managed mode) — in settings, and **resolved** rather than taken literally (`features::llama_setup::resolve_binary`, `app::supervisor::BinaryLookup`). A value with a directory part is used exactly as written and never second-guessed — a typo must surface as the preflight's "file not found", not as a silent launch of something else. A bare name (`llama-server`) is looked for **next to the application binary** first and otherwise handed to the OS, i.e. `PATH`; that order costs one `stat` instead of a `PATH` walk of our own, prefers what ships beside the app to whatever the machine happens to have, and makes the platforms agree (Windows' `CreateProcess` already searches the calling image's directory, Unix's `execvp` does not). **An empty field is not "unconfigured" but "find one"**: the build `mindfork llama setup` installed **last** under `data/llama/` ([3.4](#34-managing-the-server-lifecycle) above), otherwise a `llama-server` sitting next to the application, otherwise nothing — and only then `NotConfigured`. **A managed engine with no model file is `NotConfigured` too**, and for a sharper reason than symmetry: a recent `llama-server` started without `-m` does not fail but comes up in **router mode** — `/health` answers `ok`, so the app would report the engine ready, `/props` gives a context window of `0`, every completion comes back `400 "model name is missing from the request"`, and `models_autoload` would have the server fetch a model from Hugging Face if a name happened to match. Managed mode offers router mode nowhere, and `llama-server` has no flag to refuse it, so the refusal is the app's, before the process starts ([docs/research/robustness-and-defaults.md](docs/research/robustness-and-defaults.md) §2.1). Last-installed, not newest-tagged: ordering by build number would move a user from `vulkan-b10871` to `cpu-b10883` — off the GPU — because the CPU build happened to be published later, whereas what they last ran `llama setup` for is what they meant. The same resolution serves the impersonation server and the embedder, since one install runs all three.
- One loaded model instance. How many request streams the app keeps open against it at once is the engine section's **`sessions`** setting ([11.6](#116-the-settings-screen)), **1 by default** — the main agent and its subagents take turns, which is what a consumer PC wants. Above 1 a managed server is launched with `-np N --kv-unified`: N slots over the one context pool `-c` sizes, the shape llama.cpp itself picks when `-np` is not given (four slots, unified — its default since December 2025), so the pool costs no more memory; what N sessions cost is sharing it — and the app keeps that sharing within the pool: every stream of a turn **reserves** what it will occupy (its prompt, estimated and calibrated by the turn's own exact usage, plus its reply cap) and **waits** for room when the streams already open would not leave enough, since the server ends *every* running conversation at once when they outgrow the pool together ([6.3](#63-client-side-agentic-loop)). Design and measurements: [docs/research/parallel-subagents.md](docs/research/parallel-subagents.md); the reservation — [docs/research/admission-by-budget.md](docs/research/admission-by-budget.md).
...
**Permissions**: on unix the data root is `chmod 0700` at **every** start — not only at creation, since an older version's root was made under the umask (measured on Ubuntu 24.04: `0755`, with `settings.json` at `0644`) — and the JSON files are written `0600`. One mode on the root closes everything beneath it, since reaching a file needs `x` there; a failure is ignored, so a root on a filesystem without unix modes still works. Windows has none to set: a per-user root is covered by the profile's ACL, and DPAPI keeps the secrets user-bound ([ADR 0008](docs/decisions/0008-api-key-storage.md), [docs/research/safe-defaults.md](docs/research/safe-defaults.md) D8).

**Data location**: by default, in a `data/` subdirectory **next to the executable** (portable); a `defaults.json` file can move it to an OS folder/custom directory and set the scaffold/interface language (see [§12.1](#121-configuration)). The `default_language` field is optional: when absent (a fresh install, a deb/rpm package), the language is detected from the OS locale (`ru*` → Russian, otherwise English). The file tolerates a UTF-8 BOM. In non-portable mode, read-only resources (spellcheck dictionaries) are also looked up in the portable layout next to the binary — an installer/package puts them there. The data root's own `dictionaries/` is created at startup when it is missing, with a localized `README.txt` in it ([§11.5](#115-input-and-editing-spellcheck)) — that is where the user's own dictionaries go.
...
**A missing `data.db` is reported, not absorbed.** The database is created when it isn't there, which is right for a first launch and silent everywhere else — and the everywhere else that matters is a data directory carried to another machine, or restored by hand, with `chats/` copied and the database left behind. Then the conversations are on screen and the assistant remembers nothing about them, with nothing anywhere saying why: notes, the self-model, every profile's knowledge base and the semantic index over chat attachments live only here. So the fact is caught at the one moment it is still visible — `Storage::open` asks whether `data.db` is absent **while `chats/` holds chats**, before opening the database creates it — and it is said twice: a `warn` in the log, and one note in the feed at startup naming what is empty, what survived (the conversations and their attachments' text, which are the files that were copied) and the way back (put the file next to `chats/`, or `mindfork restore` an archive). A fresh install has no chats either, so it stays quiet. The supported way to move data is `mindfork backup`/`restore` ([§12.3](#123-backup-and-deletion)), which never produces this state.

**Schema versioning**: every artifact has its own schema version (`SETTINGS_SCHEMA` 3, `PROFILES_SCHEMA` 1, `CHAT_SCHEMA` 4, `DB_SCHEMA` 1) — they change at different rates, which is the whole reason there is no single number; version detection is structural (see [§12.2](#122-schema-versioning-and-migration)), so existing files aren't rewritten. **Read hardening**: a corrupt `settings.json`/`profiles.json` → startup refuses (it used to be silently overwritten with defaults); a corrupt `chats/<id>.json` → skipped with a `warn`, the file on disk is left untouched (previously one corrupt file would take down the whole startup).
...
loop:
    ...
    match finish_reason:
        ...
        Filtered             -> record the response, exit (the provider's content filter stopped it)
...
- `max_tool_rounds` — a safeguard against infinite loops; **8** by default, configurable. Unlike `mistral.rs` (which executed only the first tool call per turn), the client-side loop executes every call of a round — and a round may carry several: the model emits them unprompted when a task has several independent reads (measured 26/26 on both gate models and four clouds, [docs/research/concurrent-tools.md](docs/research/concurrent-tools.md) §3). The round is walked by **segments**: a maximal run of *consecutive* calls to tools their authors marked `concurrent()` ([9.2](#92-the-tool-contract)) runs as one segment, at most the engine section's `concurrent_calls` of them at once ([11.6](#116-the-settings-screen) — **1** on managed and external, **4** on the clouds; at 1 no segment is formed at all), every card opening first and closing as its own result lands; the results, the tool messages, the records and the effects are written in the **model's order**, so the request history and the stored message are what a sequential round would have left — no read is ever moved across a write. Every other call (a writer, a disabled or unknown name, a control call) ends the segment and runs at its own position, in order. The `call_subagent` calls of a round form its **parallel group** and run at once after the ordinary calls, at most `tools.subagent_parallel` of them, the tool results still appended in the model's order ([9.3.2](#932-call_subagent)). Every request stream of the turn — the loop's own, a subagent's round, a tool's summary — opens under the engine section's **session budget** ([11.6](#116-the-settings-screen)): a permit of `sessions`, and, when the streams share one KV pool (a managed server above one session, an external `llama-server` reporting several slots), a **reservation** of the pool — the request's prompt estimate — the system message, the messages and the tool schemas as the wire sends them, so that the ratio below is the tokenizer's density and not the schemas' overhead in disguise: measured, a fresh chat's first request had estimated 85 tokens against 4358 exact before the schemas were counted ([docs/research/roll-usage-calibration.md](docs/research/roll-usage-calibration.md) §2.1, §3.1) — scaled by the latest exact-to-estimate ratio recorded by a request **of the same kind** — the turn's rounds, a child run's, a silent loop's, the compaction roll, the automatic title, impersonation on the shared engine and a tool's page summary each record their own beside the estimate their reservation was priced from, and each kind prices with the latest ratio of its own kind, since the populations differ by a third and one ratio let a title's 0.65 erase the 1.34 a turn carrying a tool result had recorded ([docs/research/title-impersonation-usage.md](docs/research/title-impersonation-usage.md) §2.1, §3.1; the summary's kind, measured on five pages, is the one that under-counts as a rule — 1.04 to 1.28 on four of them, [docs/research/page-summary-usage.md](docs/research/page-summary-usage.md) §2.1) and floored by the loop's last exact size plus what it generated, plus the reply cap (a stream with no cap reserves the whole pool). A stream that would not fit next to the open ones waits, cancellably, until one ends; a stream that is alone is admitted whatever its size — the server, not the app, decides whether one conversation fits its window. With no pool known, nothing is priced and nothing waits: the behaviour before the budget existed, bit for bit ([docs/research/admission-by-budget.md](docs/research/admission-by-budget.md) §4). Beside that interactive lane the budget has a **silent lane** — one permit for the app's own requests (the title, reflection, the two consolidations, the compaction roll, impersonation on the shared engine, a page summary inside a silent loop) over the *same* pool sum — so they take turns among themselves and never overfill the pool beside a turn or a run; an interactive stream that would not fit beside an open silent one **displaces** it — the silent request is cancelled and made again by its own task after the turn, at most three times per task, after which it holds and the turn waits that one round, bounded by its reply cap — never the reverse; impersonation and a summary inside a silent loop are never displaced ([docs/research/silent-preemption.md](docs/research/silent-preemption.md) §4). The pool is known whenever the server runs more than one slot — at **one session too**, where a `llama-server` launched without `-np` runs four unified slots and the silent lane's requests land on them beside the turn's stream; only a server that reports one slot, or a cloud, has none ([docs/research/silent-tasks-budget.md](docs/research/silent-tasks-budget.md) §4). Design and the decided forks: [docs/research/concurrent-tools.md](docs/research/concurrent-tools.md), [ADR 0012](docs/decisions/0012-concurrent-tool-calls.md).
...
- **A turn the app starts** ([9.3.2](#932-call_subagent)): when a background run's task notification lands in the open, idle chat, the orchestrator starts a turn with no new user message — the notification is the last user-side row, sent as user text — so the assistant reports the result. `tools.subagent_background_wake` off keeps the note for the next message, which then carries it merged in front.
...
- **A task notification is a user-side row** for both ([9.3.2](#932-call_subagent)): regeneration redoes the reply it woke and keeps the notification; deletion removes the notification together with that reply and returns nothing to the input box.
...
pub struct ToolContext {
    ...
    pub sessions: Option<Arc<SessionBudget>>, // the turn's session budget (6.3, 11.6), shared with the tools; None for a background task
}
...
pub struct ToolOutcome { pub result: String, pub effects: Vec<ChatEffect>, pub wrote: bool, pub prefill: Option<Prefill> }
...
// `prefill` — the engine's timing of a request this call made on its own (a page
// summary's), `None` from every other provider and from a stream that ended short;
// the loop that called the tool folds it into the largest prefill sample it keeps for
// the slow-prefill note (docs/research/page-summary-usage.md §3.2).
...
| `web_search` | `{ query, max_results? }` | Searches DuckDuckGo (our own `reqwest` client) + extracts readable page content. Behind the `web_enabled` global switch (**off** by default, like Python and file access). See [9.3.1](#931-the-web-tool-our-own-implementation). |
...
| `fs_read` / `fs_write` / `fs_list` | `{ path, … }` | Read/write/list local files. Behind the `fs_enabled` global switch (disabled by default, like Python), and **only inside `fs_root`**: with the row empty every call refuses and says which setting to fill — the whole disk is a deliberate value (`C:\`, `/`), never the default. Whatever the root, the app's own directories (the data root and the binary's directory, `Paths::app_dirs`) are neither read nor written, and a write through a symbolic link whose target is missing is refused (`features::tools::reach`; [docs/research/safe-defaults.md](docs/research/safe-defaults.md) D1, D2, N1). A file is read in its own encoding (`shared::text_decode`, §9.12), a file not in UTF-8 says which, and a binary is refused. |
...
- **Search**: a multi-provider fallback chain (DuckDuckGo lite/html → Mojeek → Ecosia) over `reqwest`; result parsing (`scraper`); anti-bot throttling detection. Throttling is not always a status code: Mojeek serves its captcha with **HTTP 200**, so a page that parsed to **zero** results is additionally checked for an interstitial (`is_challenge_page`) — otherwise a blocked provider counts as "answered, found nothing", which suppresses the honest "all providers are throttled" error and tells the model the web has nothing on the subject. The check runs **only** on an empty parse, so a real result set can never be mistaken for a challenge. Its anchor is the **`<title>` element**, not phrases in the body: a body-phrase list was the first fix and it silently expired when Mojeek rewrote the page out of every phrase it knew, which put the false "no results" back in front of the model. A title names what a page *is* and survives a rewording; and a title **containing the query** is a results page whatever else it says, which is what keeps a fruitless search for anti-bot topics working. The body phrases remain as a second signal for generically-titled pages.
- **Keyed providers** (`tools.web_provider`, spec §11.6). The keyless chain is scraped HTML under anti-bot pressure, and measured from a residential IP it answers roughly twice before every provider blocks — with a block outliving fifteen minutes of silence, and a real browser from the same IP blocked too, so this is a rate ceiling rather than something better headers can lift. So `web_search` will use a provider with an API key when one is configured: **Tavily** (`api.tavily.com`, `Authorization: Bearer`). The choice is `auto` (use the key when there is one) or `free_only` (ignore the stored key entirely) — a choice rather than a toggle because the slot set is what a second provider would extend; the key is stored machine-encrypted under its own `SecretKey::Search` slot, and a key entered in settings **wins** over the environment variable the settings name — the precedence `api_key_env` has everywhere else. **With no key configured the request path is byte-identical to the keyless tool**, which is the invariant a test pins: configuring a key is the whole opt-in.
- **Keyed providers come first when a key exists.** A dead round trip through a blocked scraper costs more seconds than a credit costs cents. The keyless chain is never removed — it is the last fallback under every setting and the default with no key, so a fresh install still searches.
- **A keyed provider's failures are read apart from each other**: `429` is a rate limit and takes the same cooldown as an anti-bot throttle; `401`/`403`/`402` mean the key is wrong, revoked or out of credit, and that does not fail the search (the chain continues) but is carried as the error the model is shown if *nothing* answers — otherwise a stale key would take web search down while a working keyless chain sat behind it, and the model would be told it was a network problem.
- **A result that arrives with its page text is not fetched again.** Tavily returns cleaned page content on request, which is the latency the keyed backend was chosen to avoid and one fewer automated request in front of the site's own anti-bot. Reranking runs over it exactly as over text the tool fetched itself.
- **The result names which backend answered.** The chain degrades silently by design — a keyed provider, then three scrapers, then nothing — and without it the model cannot tell a thin answer from a degraded one.
- **The chain order is adaptive**: a provider family that answered with a block is remembered for a few minutes and moved to the back, so the next search in the same turn does not re-pay the dead round trip. `lite.` and `html.duckduckgo.com` are one *family* — one operator, one per-IP throttle — so the chain is three independent providers, not four. A cooling family is reordered, **never skipped**: skipping would let a stale cooldown report "everything is throttled" with no request having gone out.
...
- **A page is read in its own encoding** (`shared::http_text`, both tools). `reqwest` runs without its `charset` feature, so reading a body as a string meant reading it as UTF-8 whatever it declared — a windows-1251 page came back with U+FFFD for every letter, in the result, in the attachment made of it and in that attachment's search index ([docs/research/page-charset.md](docs/research/page-charset.md)). The body is decoded by an order, first match wins: a byte-order mark; **the bytes themselves when they read as UTF-8** — some non-ASCII, and more characters that decode than sequences that do not, since legacy text does not form UTF-8 by accident while a site moved to UTF-8 routinely keeps its old `<meta charset>`; for pure ASCII, whatever is declared; a declared encoding the bytes do not refute — the `Content-Type` charset or the document's own `<meta>`/XML declaration, looked for up to `<body>` rather than in a browser's first 1024 bytes, and where the two disagree, the one the detector agrees with; and with nothing usable declared, the detector Firefox runs on unlabelled pages (`chardetng`), hinted by the host's top-level domain. A declared UTF-8 the bytes refute is set aside rather than obeyed. A body the server compressed although the request advertises no compression (`gzip`, `deflate`) is unpacked first, its unpacked size bounded; a coding that cannot be undone is **reported** — the tool says that fetching again will fail the same way — rather than decoded as text.
...
- **A page over the budget becomes a chat attachment** (§9.7) instead of being cut: the threshold is `attachments.max_file_tokens`, exactly as `youtube_watch(transcript:)` uses it, so such an attachment is always *by reference* and is immediately paged (`attachment_read`) and, with an embedder configured, **searchable in the same turn**: its index starts at the end of the round that fetched it, and a search that meets it still being built waits for it ([§9.7](#97-chat-file-attachments-file-attach), [docs/research/attachment-birth-turn.md](docs/research/attachment-birth-turn.md)). The result says so — or, with no embedder configured, that the file is read by pages only. Below the threshold the text goes straight into the result. `summarize=true` still summarizes — from the head of the text, since the summarizer is a single-turn subagent — and attaches as well, so a partial summary is a starting point rather than the only access. A hard ceiling on one page's extracted text remains (1 000 000 characters — raised from 400 000, which cut this very specification off after §12) and, when reached, is **stated in the result and in the attachment**: silent truncation made a long page indistinguishable from a complete one, and "this is not the whole page" alone left the model hunting the attachment's end for a chapter the cut had removed. The result names the limit, the last Markdown heading before the cut and the page it starts on, for a body taken verbatim how many characters never arrived, and that no route reaches the rest (re-fetching returns the same beginning — tell the user which part is missing); it calls the attachment what was received, not the whole page. The attachment carries the cut in its header — which the pinned excerpt repeats every later turn — and a bracketed marker where its text stops. A body that is text rather than HTML (any `text/*` but HTML/XML, JSON, JavaScript) is taken verbatim and its header says so instead of claiming markup was removed; `text/markdown` used to fall to the HTML extractor, find no `<p>` and fail. Before any of that the **body** is read under a 32 MB ceiling counted as the bytes arrive — a server may send no `Content-Length`, and one that does is not obliged to be honest — and a page past it is refused in words instead of being pulled into memory for the twenty seconds the request has ([docs/research/safe-defaults.md](docs/research/safe-defaults.md) N3); `web_search`'s result fetches read through the same seam. **The attachment is named after the page, not its site** ([docs/research/page-attachment-name.md](docs/research/page-attachment-name.md)). A page states its name in `og:title`, an `<h1>` and its `<title>`, and its site's name in some of the same places — `docs.vlang.io` repeats one `<title>` on every page, `sector.biz.ua` one banner `<h1>` on every article — so the name is the one a second field confirms, first match wins: `og:title`, unless it is the site's own name (`og:site_name`, or the `<title>`'s last segment), without a trailing segment it shares with the `<title>`; an `<h1>` the `<title>` begins with, as the `<title>` spells it; the `<title>` without its last segment, where a title puts its site; the first `<h1>`; the `<title>`. Whitespace is collapsed, a heading's permalink mark trimmed, and the name clipped to 60 characters. A name a *different* page of the chat already holds gets the URL's last segment appended; re-fetching the same page keeps its name and replaces its own attachment.
- **The summary is a stream of the turn.** `summarize=true` makes one request of `ctx.engine` per page, and that request takes a permit of the engine section's `sessions` ([11.6](#116-the-settings-screen)) — and, under a shared KV pool, a reservation of it for the page's text plus the summary's cap ([6.3](#63-client-side-agentic-loop)) — through `ToolContext.sessions`, held around the summary's stream and around nothing else, so three pages fetched at once in a concurrent segment download together and, on a one-session engine, are summarised one after another; under a shared pool, as many at once as the pool holds; a background task's context carries the same budget on its silent lane ([6.3](#63-client-side-agentic-loop)). The request mutes reasoning as the title's does (`reasoning_budget = 0`): a thinking model spent its whole reply cap on thoughts and answered with nothing, on every page tried ([docs/research/page-summary-usage.md](docs/research/page-summary-usage.md) §2.1). Its stream's usage records under the summary's own kind of the budget ([6.3](#63-client-side-agentic-loop)) — a page's text is the one kind that under-counts as a rule — and the engine's timing of its prompt, the coldest the turn makes, rides the tool's outcome (`ToolOutcome.prefill`, [9.2](#92-the-tool-contract)) into the turn's largest sample for the slow-prefill note of [3.4](#34-managing-the-server-lifecycle), or a silent loop's landing ([docs/research/page-summary-usage.md](docs/research/page-summary-usage.md) §3).
...
- Global "master switches" for external tools (web — gates `web_search`, `fetch_url` and `youtube_watch`; Python; file access `fs_enabled` — gates `fs_read`/`fs_write`/`fs_list`) — in the tools settings (privacy/security). **All three are off by default**: every one of them reaches something outside this machine or outside the app's own directory. The effective set = `enabled_tools ∩ globally enabled`. Safe tools (`calculate`, `current_time`) have no switches.
...
chip, `/file list`, `/file remove`. Two things happen earlier than the landing:
the loop mirrors it into the turn's snapshot as its round ends, so `attachment_read`
reaches it in the next round, and starts its **index** there too, so
`attachment_search` can reach it in the same turn. An index board shared by the
orchestrator and the turn makes that start the only one (a sub-agent's attachment
reaches its parent's round end too; the landing starts only what nobody has), carries
the progress a search waits on, and holds the index's outcome note until the file
lands — a note in the middle of a streaming reply would split it — so the feed reads
"attached", then "searchable"
([docs/research/attachment-birth-turn.md](docs/research/attachment-birth-turn.md) §5).
...
attachment, since there is no text to attach — while a document an extractor reads (pdf,
docx, html) becomes the attachment **and** keeps its original beside it, the two linked as
**one item**. One item is the load-bearing part: listed apart, the very name the user typed
would resolve to two things and be refused by the shared-name rule. Removing such a pair
deletes our copy and both listings, never the user's own file; re-attaching replaces it,
with identical bytes moving nothing.

**One numbered list.** `/file list` numbers the attachments, then the stored files no
attachment links, then the images the chat's messages carry — and that numbering is what
`/file remove` takes, what the pinned block shows the model and what `python_exec` names in
`files` (§13.2), so `#3` means the same thing to the user and to the model. An image is
listed because code can read it; it cannot be *removed* there, since it belongs to the
message that carries it, and the refusal says which command is about images. The block
...
- **Indexing is best-effort** (the ADR 0002 pattern): with no embedder
  ...
  from "no hits", pointing at page reading in both cases. **A file whose index is
  being built** — by the round that attached it, or by a `/file attach` a moment ago —
  is **waited for**, up to 120 s and cancelled with the turn, while the banner shows the
  progress; past the bound the search covers what is indexed (the file's beginning:
  chunks are written in document order) and says how much, and that searching again
  reaches more. It names **which** files it cannot see whole and why, with each one's
  page count: no index at all, or one still being built. When some file *is* indexed
  the search runs and ends with the same list, so "nothing found" never silently covers
  a file it never looked at. A chat whose files are all inline is told they are shown
  in full; an embedder that stopped answering is said as such, not as a missing index.
  The pinned block offers search for a file still being indexed as well
...
- **Where the words go** is decided by the existing attachment budget
  ...
  its index starts at the end of the round, and a search waits for it —
...
`tools.fs_enabled`, confined to the folder `tools.fs_root` names. The workspace family is the opposite shape: it is scoped to **one chat**
...
- **Running a command** (spec-level behaviour, `features/tools/code.rs`):
  ...
  - The command runs with the project root as its working directory, with
    `NO_COLOR=1`/`CLICOLOR=0` and any surviving ANSI escapes stripped. It also
    starts **without the credentials in the app's environment** — the
    credential-shaped names and the variables the settings name as key sources
    ([`shared::child_env`], [docs/research/safe-defaults.md](docs/research/safe-defaults.md)
    D5) — because what it runs is the project's own code, which the model may have
    just edited; the local Python interpreter is filtered the same way, while
    `llama-server` and MCP servers keep the whole environment (D6, §9.6). Output is
...
- **Three contexts, strictly in turn.** The canonical state is one
  **role-encoded transcript** on the run: participant `a`'s lines are stored
  ...
  accept (measured, research §5.1–§5.2). At most **one request of the scene is
  ever in flight**: participant lines and director checkpoints take turns on
  one backend, and every one of them is priced and admitted by the engine
  section's **session budget** ([6.3](#63-client-side-agentic-loop)) like any
  other stream — which is what keeps the feature's VRAM contract once a scene
  can outlive its turn and meet the next one
  ([docs/research/background-dialogues.md](docs/research/background-dialogues.md)
  R3); on a local server each context's prefix is append-only and keeps its
  cache slot (measured).
...
- **A scene in the background — `start_dialogue`** ([docs/research/background-dialogues.md](docs/research/background-dialogues.md);
  ADR 0011, amended). The twin `start_subagent` is to `call_subagent`, behind
  the same switch (`tools.subagent_background`, off): the call **returns at
  once** with the scene's `chat://` address, the scene leaves the turn — its
  own task and cancellation token, so `Esc` ends the turn and not the scene —
  and the director's closing result arrives later as a **task notification**
  ([9.3.2](#932-call_subagent)) worded for a dialogue and carrying the same
  compact ending the foreground result returns. The record lands with the turn
  as a `kind: dialogue` placeholder holding the participants and the caller's
  opening line, and is filled in **by id** when the scene ends. Everything
  else a scene is, a background scene is: the same three contexts strictly in
  turn, the same limits and timeout, the transcript on the call's record, the
  row under its parent marked *running*, the read-only screen that streams
  each line on its speaker's side, the retake that replaces the open view, and
  `F6`/`/subagents stop [n]` to end it ([11.2](#112-the-chat-list-an-overlay)).
  Two differences are the background's own: the **director's brief is
  snapshotted at the call** — the parent's persona and the conversation as it
  was when the scene was staged, because a scene ending twenty minutes later
  has no turn to read them from — and the run counts against the shared
  `tools.subagent_background_max` with background sub-agents.
...
**Scaffold language** (`Profile.language: Lang`, i18n Tier 1, [docs/history/i18n.md](docs/history/i18n.md)): the language of background-task prompts, the "self-model" scaffold, and tool results — text that the **model** reads (axis A). This is **not** the language the model answers in (that's set by the system message) and **not** the interface language (axis B, separate). Chosen before the first conversation and then **locked** as soon as the profile has data: visible chats with **conversation content** (messages, deleted-exchange tombstones or a compaction summary — `Chat::is_pristine`), a non-empty self-model, or notes — so that the entire profile's memory stays in one language; the gate is authoritatively checked by the orchestrator (`profile_has_data`), and the UI renders the "Scaffold language" field as locked. A **pristine** chat — the first-launch default one included — binds nothing: everything language-bearing it carries is a re-derivable localized default, so a fresh install's language stays editable. A switch re-derives the untouched defaults — a profile name/system message still byte-equal to the old locale's `defaults.*` (`reseed_language_defaults`), and every pristine chat's default title plus its system-message/role-name copies, as if created now (`rederive_pristine_chats`); user-edited text never matches the old default byte-for-byte and is left alone, and a manually renamed title is kept. Resolution — `profile.language` at request time; scaffold text is fetched through `shared/i18n` from the built-in `locales/<lang>.json` bundle, with external `data/locales/*.json` layered on top at startup (Tier 3 — overriding built-in text and adding new languages with no rebuild, [docs/history/i18n-external-locales.md](docs/history/i18n-external-locales.md)). Tiers 1–2 translated the hot core and all 35 tools; `Lang` is `Ru`/`En`/`Ext(code)`.
...
alternate screen, wrote escape codes into the redirected output and waited for
keys forever
([docs/research/public-release-readiness.md](docs/research/public-release-readiness.md) §3.1).
A **second** instance is refused the same way and with the same code — it used to
exit 0, which told whatever started it that the app had run and finished. And when
a refusal is printed into a console this process owns **alone**
(`GetConsoleProcessList` = 1, which is what a double-click from Explorer gives —
measured in [docs/research/robustness-and-defaults.md](docs/research/robustness-and-defaults.md) §2.2),
the app waits for Enter before exiting, because Windows destroys that window with
the process and the message would never be read. Started from a shell, or with
output redirected, it waits for nothing.
...
- **Token counter** in the status bar: the total "conversation (prompt) + response" count,
  ...
  **estimate** (the "UTF-8 bytes / 4" heuristic over the system message, the messages and the tool schemas as the wire sends them — a fresh chat reads about 4600, not 85, until the server says 4358; [docs/research/roll-usage-calibration.md](docs/research/roll-usage-calibration.md) §3.1), marked with `~`; the exact `prompt_tokens`
...
- **A model-written title** (`Ctrl+R` in the list; `/autotitle` in the chat for the
  ...
  route's errors must land where the user is. The request is a cold prompt on the chat
  server — its own system over the digest — and its timing is offered to the
  slow-prefill rule of [3.4](#34-managing-the-server-lifecycle) after the title's
  landing, whatever the text made of it ([docs/research/oneshot-samples.md](docs/research/oneshot-samples.md) §3.2).
  **The same task also runs by itself** — `interface.auto_title`
...
- **Subagent transcripts are rows of the list, nested under the chat whose
  ...
  *its* first match, on a chat row on the chat's own (a match inside a
  ...
  count updated as its rounds file, openable, renameable — a run out in the
  **background** stays such a row after its turn has ended, and one stored
  with no outcome reads *unfinished*. A **chat** whose background run
  finished while it was not the open one is marked *unread* beside its own
  count, its dot in the accent colour, until it is opened
  ([§9.3.2](#932-call_subagent)) — the list is where a result that started no
  turn is announced. On the open transcript of a run that is still out,
  `F6` **stops it** (the same route `/subagents stop` takes) and `Esc` goes
  back to the list without touching it: the run is under no turn, so the
  footer names `F6` while it streams and neither key before or after. **Switching chats
...
- **Input/editing**: a **custom multiline widget** (`widgets/input_box.rs`, [ADR 0001](docs/decisions/0001-ui-crates-ratatui-030.md)) — for a new message and for editing existing messages in place. Off-the-shelf crates (`tui-textarea`/`ratatui-textarea`) are incompatible with ratatui 0.30 and don't support highlighting arbitrary ranges — hence a custom widget.
  ...
  - **A combining mark belongs to the letter before it, and is not a spelling.** `U+0300–U+036F` (the Combining Diacritical Marks block) counts as in-word, so a stressed `И́стинно` — the sign every Russian source uses, and what `Alt+0769` types — is one word rather than `И` plus `стинно`; and a word that fails as typed is looked up once more with its marks removed. `char::is_alphabetic` is false for every one of those marks, so without the rule a letter run ended at the stress and the check went **wrong in both directions**: correct text underlined (`чуде́сный ве́чер` drew three underlines), and a real error hidden whenever both halves happened to be words (`за́мок` passed as `за` + `мок`). The order matters — as typed first — so `en_GB`'s precomposed `café` (`U+00E9`) is answered by its own entry; the fallback is safe because the three bundled `.dic` files hold **zero** combining marks, so it can only accept what the first lookup rejected. The mark is ignored, never the spelling under it: `харашо́` is still flagged, as the whole word, once. Suggestions are asked for the unmarked word (no entry carries a mark to match), so a correction lands unstressed; "add to dictionary" stores the unmarked form, and one add therefore covers every placement of the stress. The same rule repairs a `ё` typed decomposed (`е` + `U+0308`, as pasted from a macOS-authored source). Stress **placement** is not validated — `мол́око` is as correct as `мо́локо`; that needs an accentuated dictionary. `е`/`ё` needs nothing of its own: the bundled `ru_RU` accepts the `е` spelling of every one of its 7347 `ё` stems, while still flagging the hypercorrections (`афёра`, `опёка`) — see [docs/research/spellcheck-stress-marks.md](docs/research/spellcheck-stress-marks.md). <!-- cyrillic-ok: the examples are the evidence -->
...
- **Model/server** — a tab strip **Assistant │ Impersonation │ Embeddings** (three
  ...
  FlashAttn, `--no-mmap`),
  *Speculative decoding* (`--spec-type` + draft-model fields), *Advanced* (*Extra
  arguments* — raw `llama-server` arguments appended to the line, a flag one of the
  ...
  *Provider* (model, API-key-env, base URL); and, in every mode of the
  ...
  above 1, [§3.4](#34-managing-the-server-lifecycle), and a stream that
  would not fit the shared pool next to the open ones waits for room,
  [6.3](#63-client-side-agentic-loop) — the hint says so; the app's own
  background requests take the budget's silent lane, one at a time, whatever
  the count, and yield it to a stream of the turn's that would not fit
  beside them); for a managed or
...
external), but with impersonation sampling, and on the budget's silent lane —
one of the app's own requests, waiting for room beside a turn or a run,
and never displaced by one — the preview is the user's own request
([6.3](#63-client-side-agentic-loop)), and its prompt — the whole conversation with
the roles swapped under its own system, processed cold, the largest a session
makes (measured: 10 642 tokens where the fresh chat's first turn had 4349, and a
second impersonation right after 4 — the cache's) — is timed by the engine and
offered to the slow-prefill rule of [3.4](#34-managing-the-server-lifecycle) after
`ImpersonationFinished`, on this shared server only, since a separate
impersonation server has a batch and a session of its own the rule does not know
([docs/research/oneshot-samples.md](docs/research/oneshot-samples.md) §3.1); `managed` — a separate `llama-server`
...
- **The app's own work** — the four silent tasks (reflection, notes
  consolidation, self-model consolidation, history compaction), each
  *running*, *waiting* or *idle*: *waiting* is a slot taken with no stream
  open — behind another of the app's requests on the budget's silent lane,
  or waiting for room beside a turn, or displaced by one and waiting to be
  made again ([6.3](#63-client-side-agentic-loop)); read off the budget,
  never stored. There is no last-run time and no last
  outcome, by decision (the research's fork F2).
...
a quit of the app applies the same rule before its exit flush — listening
for the cancelled task's own landing, as long as it takes by default or
up to the limit set in *Tools*, then deciding by its state —
[quit-waits-for-the-landing.md](docs/research/quit-waits-for-the-landing.md),
[quit-settle-roll-and-cap.md](docs/research/quit-settle-roll-and-cap.md),
...
again at the next landing if the conversation is still over the threshold
([docs/research/stop-silent-task.md](docs/research/stop-silent-task.md));
`Esc` goes back; `F1` is the runtime's. A silent task's row offers neither
`Enter` nor `P`, and `F6` only while it runs. Opening a transcript or a parent chat from here **stashes
...
column moves — and while a silent task runs it re-asks for the rows on that
tick, since *waiting* flips inside the task; a list that is all landed and
idle repaints on input alone (fork F5).
...
- **The Wasmer sandbox** (default) — code runs isolated in **WASIX** via a `wasmer` sidecar process (the binary sits next to the app, in `data/sandbox/`, installed by `mindfork sandbox setup`). The guest **can't see the host filesystem** (only a mounted per-call job directory with the script and `out/`; CPython and the packages run from one packed image, `packed-sandbox.webc`, whose `site-packages` volume takes a write in memory and drops it with the call — `wasmer` has no read-only mount, and a mounted directory let one call plant a `sitecustomize.py` that the next one ran; an install from before the image is refused with the command that packs it; and `sandbox setup` **starts a freshly packed image before installing it**, so a build that does not run fails the command instead of replacing a sandbox that works — the order is pack, start, install, ADR 0005 §5; every `wasmer` step it runs has a time limit — ten minutes to unpack or build, thirty for the 44.7 MB download — so a registry that stalls fails the command, naming the step, instead of hanging it), **network access is a toggle** (`python_net_enabled`; without it there are physically no sockets, and the runtime's own prompt about the missing flag is taken out of the guest's stdout so the model reads its program's output rather than an instruction it cannot follow). That network is **public-only**: `wasmer`'s rule list denies the same ranges `shared::net::is_public` denies — loopback, private, link-local and the rest, from one CIDR list a test holds against the predicate — so sandboxed code cannot reach this machine's own services or the LAN, which a bare `--net` allowed (measured: a request from the guest arrived at a server on the host). `tools.web_allow_private`, the switch that already means "the model may reach private addresses", lifts it for both the web tools and the sandbox, and the tool's description says which of the three states is in force ([docs/research/safe-defaults.md](docs/research/safe-defaults.md) D4, N4). Packages are pre-installed from a pinned lock list (numpy, pandas, sympy, networkx, requests, beautifulsoup4, lxml, feedparser, pyyaml, regex, openpyxl, pypdf, tabulate, pillow, matplotlib); the tool description names every one of them. Two WASIX shims run ahead of every script: `setsockopt` for the HTTP clients, and `MPLCONFIGDIR` + `text.hinting: default` for matplotlib (the guest has no `HOME`, and matplotlib's default autohinting traps in this FreeType build). Interruption is a **process kill** (Python runs in-process inside wasmer/V8); a **timeout** + a "one task at a time" gate. Doesn't require Python on the machine. Cross-platform (Windows/Linux/macOS). **Every call is a fresh sandbox** — the job directory is created per run and dropped afterwards, and the guest's `/tmp` dies with the process, so neither variables nor files survive between calls. The tool description says so explicitly: it is the model that pays for the assumption otherwise (measured — a 16 MB download written to `/tmp` in one call, gone by the next, then fetched again). **Its one way out is `/w/out`** ([docs/history/sandbox-file-exchange.md](docs/history/sandbox-file-exchange.md)): after the process exits — whatever the exit code, but not after a timeout, when a file may be half-written — the regular files directly in `out/` are collected in name order (10 files, 25 MB each, 50 MB per call; a link is never followed, and a folder, a link or anything past a cap is skipped and named in the result, the first twenty by name and the rest counted — the caps bound what is collected, and this bounds what is said about it; a collection that itself fails names what `out/` held as unreadable and logs why, rather than reporting that the call saved nothing) and stored with the chat (§9.7). A PNG or JPEG among them is shown to the model when `tools.python_images` is on (§9.10). **Its way in is `/w/in`**: the files a call names in its optional `files` argument — the chat's attachments (as their extracted text), its stored files and the images its messages carry, numbered exactly as `/file list` numbers them for the user — are copied there before the run, under names the system prompt states in advance, so the model can write `/w/in/<name>` into code it sends before any result exists. They are **copies**: the guest may overwrite one and nothing follows from it, and only `/w/out` comes back. So a later call reads what an earlier one saved by naming it, and a workbook or a PDF the user attached reaches the code the same way (§9.7). A handle that reaches nothing, a name two files share, a listed file whose copy is gone, or an argument that is not a list of names at all refuses the call **before** it runs, naming the valid handles — a script that asked for four files and got three would answer confidently from three. So is a call naming more than twenty files, or more than 100 MB of them in all — checked against the sizes the chat records, before anything is copied — with the numbers and the way through: save what one call makes to the output folder and name that in a later call. No single file of the chat is refused by it, attachments stopping at 32 MB. In a chat with no files at all, an unknown handle is refused saying so, not with an empty list. A single name **without** its list is taken rather than refused, being the one shape whose intent is not in doubt; one reading serves the call and the confirmation popup both, so the set stated and the set staged cannot differ. The argument is offered in **both** modes: Local runs in a job directory of its own ([ADR 0005](docs/decisions/0005-python-sandbox-wasmer.md) §3).
- **The local interpreter** — running code in a **separate process** of the system Python (the path is in settings; a venv is recommended), capturing stdout/stderr, a **timeout**, output truncation. **No OS-level sandbox** (the code runs on the user's own machine) — hence the sandbox being the default mode. The **file exchange is the same** ([docs/history/sandbox-file-exchange.md](docs/history/sandbox-file-exchange.md) F11): a job directory per call holds the script beside `in/` and `out/` and is the process's working directory, so the files a call names in `files` are copied into `in/` before it runs and what it writes to `out/` is collected under the same caps and stored with the chat. The model is told the folders in the form this mode uses — `in/` and `out/` rather than `/w/in` and `/w/out` — and the schema, the refusals and the result section are the sandbox's. What the job directory is **not** here is a boundary: the code reaches whatever the user reaches, the network included, so this mode has no network switch to honour.
- **Sandbox resource limits:** a timeout (CPU) + wasm32 (~4 GB of address space) + a "one task" gate + an **optional hard RAM cap** (`tools.python_wasm_memory_mb`, off by default). The RAM cap is **Windows-only** (a Job Object; exceeding it kills the process, protecting the host from OOM; a minimum of ~1024 MB); not applied on Unix (rlimit is unreliable with V8). Local mode has a cap of its own (`tools.python_local_memory_mb`, off by default, Windows only, through the same Job Object) — its own field because the floors are far apart, ~1024 MB for the sandbox and tens of MB for native CPython. Past it the interpreter is refused the memory, so the script sees a `MemoryError` rather than being killed, and a process the script starts is capped the same way. It is a guard, not isolation: the job is assigned just after the spawn. Local mode has no "one task" gate — that gate refuses a concurrent call rather than queueing it. The tool's contract (`python_exec`, `{ code, files }`) doesn't depend on the mode/implementation.
