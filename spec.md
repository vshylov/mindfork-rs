...
chip, `/file list`, `/file remove`. One difference in time: the loop mirrors it into
the turn's snapshot as its round ends, so `attachment_read` reaches it in the next
round, but the orchestrator applies it — and starts its index — only when the turn
lands, so it cannot be searched in the turn that produced it, and the tool's result
says so ([docs/research/attachment-birth-turn.md](docs/research/attachment-birth-turn.md)).
...
- **Indexing is best-effort** (the ADR 0002 pattern): with no embedder
  ...
  from "no hits", pointing at page reading in both cases. It names **which** files
  it cannot see and why, with each one's page count: a file a tool attached **in
  this turn** (`fetch_url`, `youtube_watch`) — indexing starts when the turn lands,
  so it is "attached in this turn, indexed after your reply", and searching again in
  the same turn is said to be pointless — or a file with no index at all. When some
  file *is* indexed the search runs and ends with the same list, marked not
  searched, so "nothing found" never silently covers a file it never looked at. A
  chat whose files are all inline is told they are shown in full; an embedder that
  stopped answering is said as such, not as a missing index
  ([docs/research/attachment-birth-turn.md](docs/research/attachment-birth-turn.md)
  F2; the turn's context records its own tools' attachments in
  `ToolContext.born_this_turn`).
...
- **Where the words go** is decided by the existing attachment budget
  ...
  from the next turn, once indexed after this one lands —
