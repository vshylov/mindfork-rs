...
- **A sub-agent transcript opened from the list is read-only** ([§9.3.2](#932-call_subagent),
  ...
  landed; the scroll follows the tail only if it already did); the sub-agent
  chip stays in the status bar, the parent's stream never reaches this feed,
  and `Esc` is navigation here — cancelling the turn is the parent's `Esc`.
