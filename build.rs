//! Build-скрипт: копирует словари спелл-чека в каталог данных рядом с бинарником.
//!
//! В портативном режиме приложение читает данные из подкаталога `data/` рядом с
//! исполняемым файлом (в dev — `target/<profile>/data/`, см. shared/paths.rs), значит
//! словари ожидаются в `data/dictionaries/`. Словари лежат в `dictionaries/` корня
//! проекта (в репозитории; их же упаковывает релизный workflow в `data/dictionaries/`);
//! этот скрипт копирует их в выходной каталог, чтобы `cargo run` сразу видел спелл-чек
//! без ручного копирования. Отсутствие словарей — не ошибка (спелл-чек просто выключится).
