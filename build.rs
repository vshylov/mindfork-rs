...
//! словари ожидаются в `data/dictionaries/`. Сами словари в репозиторий не входят
//! (см. .gitignore); этот скрипт копирует их из `dictionaries/` корня проекта в
//! выходной каталог, чтобы `cargo run` сразу видел спелл-чек без ручного копирования.
//! Отсутствие словарей — не ошибка (спелл-чек просто выключится).
