...
fn run_loop(
    ...
    while !quit {
        ...
        if dirty {
            ...
            drawn?;
            dirty = false;
            // Изменение содержимого ленты обнаруживается только в ходе отрисовки
            // (промах кэша блока), поэтому заказанная им полная перерисовка исполняется
            // СЛЕДУЮЩИМ кадром — будим петлю сами. Иначе последний чанк стрима оставил
            // бы артефакт до ближайшего постороннего события. Кадр-«догон» ровно один:
            // при неизменном содержимом кэш попадает и запрос не возобновляется.
            if active.is_chat() && screen.has_pending_full_redraw() {
                dirty = true;
            }
        }
        if event::poll(TICK)? {
