...
pub enum AppEvent {
    ...
    /// `mcp_tools` — динамический каталог инструментов MCP-серверов (снимок из
    /// `McpManager`; в статический `tool_catalog()` они не входят) для тумблеров
    /// профиля. Пуст, пока серверы не поднялись/выключены.
    Settings {
