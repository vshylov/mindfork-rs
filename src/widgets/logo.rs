...
///
/// Начертание повторяет вордмарк бренда (JetBrains Mono ExtraBold): жирные штрихи,
/// строчные буквы, выносные у `d`/`f`/`k`. Высота — **6 пикселей = 3 строки
/// терминала**, ровно половина высоты глифа-иконки (12 пикселей), как задаёт
/// горизонтальный лockup. Пиксельный шрифт нужен собственный: в SVG-вордмарке текст
/// переведён в кривые (docs/branding.md §2), растеризовать их в терминале нечем.
...
const WORDMARK_PX_ROWS: usize = 6;
...
/// строку выше низа глифа — так же, как в SVG-лockup'е.
const WORDMARK_TOP_ROW: u16 = 2;
