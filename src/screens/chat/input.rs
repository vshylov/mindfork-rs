...
impl ChatScreen {
    ...
    ///
    /// Нужна там, где с экрана **исчезает или сдвигается широкий глиф** (эмодзи).
    /// Механика дефекта (проверена на `Buffer::diff` ratatui 0.30): широкий эмодзи
    /// занимает две ячейки — саму (`символ`) и **хвостовую**, которую ratatui
    /// сбрасывает в дефолт. Когда глиф уходит, новая ячейка на его месте
    /// перерисовывается, а хвостовая — нет: в обоих буферах она дефолтный пробел,
    /// diff считает её неизменной и пропускает. Терминал же (conhost/Command Prompt)
    /// физически не очищает вторую половину широкого глифа сама — и на экране
    /// остаётся её половина (заметна, если у ячейки был фон: например выделение в
    /// попапе эмодзи). Полная перерисовка переписывает КАЖДУЮ ячейку и стирает её.
    ...
    /// Источники запроса: прокрутка ленты со «съезжающими» VS16-эмодзи
    /// ([`Self::take_feed_scrolled`]) и попап эмодзи ([`Self::request_full_redraw`]).
