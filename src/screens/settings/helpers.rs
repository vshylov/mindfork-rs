...
/// The hint panel's content height in rows (the top border is extra) — one
/// constant for **every** section, so switching sections never resizes the
/// panel. It used to grow to the field set's longest hint, which made the
/// panel (and the list above it) jump on every section switch; now content
/// that doesn't fit scrolls instead ([`super::Focus::Hint`], PgUp/PgDn).
/// Five rows fit the longest built-in descriptions at a typical pane width.
pub(super) const HINT_PANEL_ROWS: usize = 5;
...
/// what lets the panel count content rows for its scroll before layout.
