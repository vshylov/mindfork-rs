...
- **Подготовка к вводу API-ключей прямо в настройках**: в `settings.json` появилась
  секция `api_keys` — сюда попадут ключи OpenAI/Gemini/Claude, введённые в
  приложении (само поле ввода добавит следующий выпуск). Ключи хранятся
  **зашифрованными и привязанными к компьютеру**: файл настроек можно переносить
  между машинами, но на новой машине ключи потребуется ввести заново, а по
  возвращении на прежнюю — они снова читаются. Формат дополняется без миграции:
  старые настройки читаются как есть.
