...
- **Озвучивание сообщений чата** — команда `/tts` в поле ввода: `/tts` читает
  ...
  на новой вкладке «Озвучивание» секции «Модель»: **локальный движок** (без сети и
  ключей — ставится командой `mindfork-rs tts setup`), облако OpenAI, облако Gemini
  или любой сторонний OpenAI-совместимый TTS-сервер; API-ключ облака — тот же, что
  уже введён для чата. Там же — голос, указания по тону, скорость и поведение
  (озвучивать роли; прерывать при смене чата / при начале генерации). Пока идёт
  озвучка, в строке состояния виден значок «♪ озвучка». Если звуковой карты нет
  (например, вход по SSH), приложение об этом сообщит и продолжит работать.
- **Локальная озвучка без сети и ключей** — команда `mindfork-rs tts setup`
  скачивает движок и голоса (русский и английский, ~130 МБ) в `data/tts/`, после
  чего в настройках достаточно выбрать режим `managed`. Синтез идёт офлайн на
  процессоре — быстрее реального времени. Дополнительный голос ставится
  `mindfork-rs tts setup --voice ru_RU-denis-medium`, `--force` перекачивает
  заново.
