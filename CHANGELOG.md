...
- **Попап эмодзи (`Ctrl+B`) больше не оставляет след на экране**: после закрытия
  окна (и при перемещении по сетке стрелками) на месте выделенного эмодзи оставался
  цветной «огрызок» — половина широкого глифа, которую терминал не очищал. Теперь
  такой кадр перерисовывается целиком, без мигания.
...
- **Попап подсказок орфографии (`Ctrl+G`) не оставляет след подсветки**: у пункта
  «➕ добавить в словарь» после закрытия окна или перехода на другую строку могла
  остаться половина подложки выделения — тот же дефект, что и в окне эмодзи.
